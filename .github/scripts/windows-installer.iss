; osiris Windows installer (Inno Setup 6 — preinstalled on GitHub's
...
;
; Defaults to a per-user install ({localappdata}\Programs\osiris — no UAC
...
; to previous installs of osiris.
AppId={{0A17C5E1-3D42-4E9B-9C0D-05123B7F4C21}
AppName=OSIRIS.EXE
AppVersion={#AppVersion}
VersionInfoVersion={#VersionInfoVersion}
AppPublisher=DSB Labs · Disk Darián
AppPublisherURL=https://github.com/kingpiragua/osiris
AppSupportURL=https://github.com/kingpiragua/osiris/issues
AppUpdatesURL=https://github.com/kingpiragua/osiris/releases
DefaultDirName={autopf}\OSIRIS
...
UninstallDisplayIcon={app}\osiris.exe
OutputDir={#OutputDir}
OutputBaseFilename={#OutputName}
Compression=lzma2
SolidCompression=yes
WizardStyle=modern
; The persistent daemon (osiris.exe --daemon) is a detached background process
; that outlives the GUI and holds the running image of osiris.exe, so Windows
...
; Windows treat theirs — not a runtime preference, so osiris has no setting for
...
Name: "explorermenu"; Description: "Add ""Open in OSIRIS.EXE"" to the folder context menu"; GroupDescription: "Shell integration:"; Flags: unchecked

; Builds before the osiris/osiris split installed the GUI as osiris.exe. Upgrading
; only *adds* osiris.exe, so the old binary would stay on disk — and a taskbar
...
[InstallDelete]
Type: files; Name: "{app}\osiris.exe"

[Files]
Source: "{#StageDir}\osiris.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "{#StageDir}\oexe.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "{#StageDir}\osiris-updater.exe"; DestDir: "{app}"; Flags: ignoreversion skipifsourcedoesntexist
...
Source: "{#StageDir}\.osiris-inno-install"; DestDir: "{app}"; Flags: ignoreversion skipifsourcedoesntexist
...
; {app}\osiris.exe). The two files are a matched pair — never update one
...
; The Linux musl osiris-server, for serving WSL distros without a download.
...
; AppUserModelID is what lets toast notifications carry the osiris name and icon
...
[Icons]
Name: "{autoprograms}\OSIRIS.EXE"; Filename: "{app}\osiris.exe"; AppUserModelID: "com.diskdarian.osiris"
Name: "{autodesktop}\OSIRIS.EXE"; Filename: "{app}\osiris.exe"; Tasks: desktopicon; AppUserModelID: "com.diskdarian.osiris"
...
Filename: "{app}\osiris.exe"; Parameters: "--register-explorer-menu"; Tasks: explorermenu; Flags: runhidden waituntilterminated skipifsilent
Filename: "{app}\osiris.exe"; Description: "{cm:LaunchProgram,OSIRIS.EXE}"; Flags: nowait postinstall skipifsilent

[UninstallRun]
; Stop the daemon before the uninstaller deletes osiris.exe — the running daemon
...
Filename: "{app}\osiris.exe"; Parameters: "--stop-daemon --update-install-dir ""{app}"""; Flags: runhidden waituntilterminated; RunOnceId: "StopDaemon"
...
Filename: "{app}\osiris.exe"; Parameters: "--unregister-explorer-menu"; Flags: runhidden waituntilterminated; RunOnceId: "UnregisterExplorerMenu"

[Code]
(* Gracefully stop the persistent daemon before we overwrite osiris.exe. We can't
  ...
  *new* osiris.exe to {tmp} and run that. It connects to the running daemon, hangs
...
var
  ResultCode: Integer;
begin
  ExtractTemporaryFile('osiris.exe');
  Exec(ExpandConstant('{tmp}\osiris.exe'),
...
(* Take {app} back out of the user's PATH.
  ...
  users ran osiris, this clears the one uninstalling — the others keep a dead
...
(* Remove the optional Explorer verbs if this user enabled them in Settings.
  ...
  The application owns only the final `osiris` subkeys. Deleting those trees
...
begin
  RegDeleteKeyIncludingSubkeys(
    HKEY_CURRENT_USER, 'Software\Classes\Directory\shell\osiris');
  RegDeleteKeyIncludingSubkeys(
    HKEY_CURRENT_USER, 'Software\Classes\Directory\Background\shell\osiris');
end;
