...
#   dist/osiris-<version>-linux-<arch>.AppImage
#
...
NAME="osiris-${VERSION}-linux-${ARCH}"
...
cp "target/${TARGET}/release/osiris" "$APPDIR/usr/bin/osiris"
chmod +x "$APPDIR/usr/bin/osiris"
...
cp "target/${TARGET}/release/oexe" "$APPDIR/usr/bin/oexe"
chmod +x "$APPDIR/usr/bin/oexe"
...
# is gone by the time an install runs (see src/bin/osiris-updater.rs).
cp "target/${TARGET}/release/osiris-updater" "$APPDIR/usr/bin/osiris-updater"
chmod +x "$APPDIR/usr/bin/osiris-updater"
...
cat > "$TOOLS/osiris.desktop" <<'DESKTOP'
[Desktop Entry]
Type=Application
Name=osiris
...
# in src/bin/osiris-updater.rs). X-AppImage-Version is the AppImage convention
# for exactly this. Appended outside the heredoc, which is quoted on purpose.
echo "X-AppImage-Version=${VERSION}" >> "$TOOLS/osiris.desktop"
# linuxdeploy only accepts fixed icon resolutions (…256, 384, 512 — NOT the
# source's 1024), so downscale to 256×256.
convert assets/app-icon.png -resize 256x256 "$TOOLS/osiris.png"
...
"$LINUXDEPLOY" \
  --appdir "$APPDIR" \
  --executable "$APPDIR/usr/bin/osiris" \
  --desktop-file "$TOOLS/osiris.desktop" \
  --icon-file "$TOOLS/osiris.png"
