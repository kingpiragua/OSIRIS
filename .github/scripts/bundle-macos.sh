...
# Package the release binary into dist/osiris.app, then publish both:
#   dist/osiris-<version>-macos-<arch>.zip  (in-app updater)
#   dist/osiris-<version>-macos-<arch>.dmg  (drag-to-Applications install)
#
...
PACKAGE_UPDATE_ZIP="${OSIRIS_PACKAGE_UPDATE_ZIP:-1}"
APP="dist/osiris.app"

rm -rf dist
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp "target/${TARGET}/release/osiris" "$APP/Contents/MacOS/osiris"
chmod +x "$APP/Contents/MacOS/osiris"
...
# sits next to osiris under MacOS/ — that is the directory the GUI resolves
# relative to its own executable.
cp "target/${TARGET}/release/oexe" "$APP/Contents/MacOS/oexe"
chmod +x "$APP/Contents/MacOS/oexe"
if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
    ...
    cp "target/${TARGET}/release/osiris-updater" "$APP/Contents/MacOS/osiris-updater"
    chmod +x "$APP/Contents/MacOS/osiris-updater"
fi
cp assets/osiris.icns "$APP/Contents/Resources/osiris.icns"
...
<plist version="1.0">
<dict>
    <key>CFBundleName</key><string>OSIRIS.EXE</string>
    <key>CFBundleDisplayName</key><string>OSIRIS.EXE</string>
    <key>CFBundleIdentifier</key><string>com.diskdarian.osiris</string>
    ...
    <!-- osiris is a terminal workbench: panes are forked from the bundled
         executable, so macOS attributes a child process's protected-resource
         requests to osiris.app. Without these usage strings a program you run in
    ...
    <string>A program running inside osiris would like to access the camera.</string>
    <key>NSMicrophoneUsageDescription</key>
    <string>A program running inside osiris would like to access the microphone.</string>
    <key>NSContactsUsageDescription</key>
    <string>A program running inside osiris would like to access your contacts.</string>
    <key>NSCalendarsFullAccessUsageDescription</key>
    <string>A program running inside osiris would like to access your calendar data.</string>
    <key>NSRemindersFullAccessUsageDescription</key>
    <string>A program running inside osiris would like to access your reminders.</string>
    <key>NSPhotoLibraryUsageDescription</key>
    <string>A program running inside osiris would like to access your photo library.</string>
    <key>NSLocationUsageDescription</key>
    <string>A program running inside osiris would like to access your location information.</string>
    <key>NSMotionUsageDescription</key>
    <string>A program running inside osiris would like to access motion data.</string>
    <key>NSLocalNetworkUsageDescription</key>
    <string>A program running inside osiris would like to access the local network.</string>
    <key>NSBluetoothAlwaysUsageDescription</key>
    <string>A program running inside osiris would like to use Bluetooth.</string>
    <key>NSSpeechRecognitionUsageDescription</key>
    <string>A program running inside osiris would like to use speech recognition.</string>
    <key>NSSystemAdministrationUsageDescription</key>
    <string>A program running inside osiris requires elevated privileges.</string>
    <key>NSAppleEventsUsageDescription</key>
    <string>A program running inside osiris would like to control other applications via Apple Events.</string>
</dict>
</plist>
PLIST
...
if [[ -n "$SIGN_ID" && -n "${APPLE_CERTIFICATE:-}" ]]; then
    ...
    KEYCHAIN="${RUNNER_TEMP:-/tmp}/osiris-sign.keychain-db"
    CERT_PATH="${RUNNER_TEMP:-/tmp}/osiris-cert.p12"
    KEYCHAIN_PASSWORD="${KEYCHAIN_PASSWORD:-osiris-ci}"
...
<dict>
    ...
    <!-- Deliberately nothing beyond those three, and in particular no TCC
         ...
         are about a *child* process's request: macOS attributes it to osiris.app
         ...
         So camera / microphone / location / apple-events on osiris.app would do
         nothing for a pane, while widening what injected code could reach
         under osiris's identity; this bundle already carries
...
ENT
    ...
    codesign --force --options runtime --timestamp \
        --sign "$SIGN_ID" "$APP/Contents/MacOS/oexe"
    if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
        codesign --force --options runtime --timestamp \
            --sign "$SIGN_ID" "$APP/Contents/MacOS/osiris-updater"
    fi
    codesign --force --options runtime --timestamp --entitlements "$ENTITLEMENTS" \
        --sign "$SIGN_ID" "$APP/Contents/MacOS/osiris"
    codesign --force --options runtime --timestamp --entitlements "$ENTITLEMENTS" \
        --sign "$SIGN_ID" "$APP"
...
ZIP=""
if [[ "$PACKAGE_UPDATE_ZIP" != "0" ]]; then
    ZIP="dist/osiris-${VERSION}-macos-${ARCH}.zip"
    ditto -c -k --keepParent "$APP" "$ZIP"
fi

# Package the (now stapled) bundle as a drag-to-Applications DMG.
DMG="dist/osiris-${VERSION}-macos-${ARCH}.dmg"
...
# dist/osiris.app after this point — the zip above is what the updater ships and
# what nightly.yml verifies (it extracts that, not this), and release.yml only
# knows about osiris.app as an intermediate to keep out of the upload globs.
mv "$APP" "$STAGE/"
...
# that message is under /Volumes/osiris, not on the host: three nightlies died
...
hdiutil create -volname "osiris" -srcfolder "$STAGE" -ov -format UDZO \
