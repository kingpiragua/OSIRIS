...
#   dist/osiris-<version>-linux-<arch>.tar.gz
#
...
cp "target/${TARGET}/release/osiris" "$STAGE/osiris"
chmod +x "$STAGE/osiris"
...
cp "target/${TARGET}/release/oexe" "$STAGE/oexe"
chmod +x "$STAGE/oexe"
# Release builds keep symbols (thin LTO, no profile strip); drop them here so
# the archive isn't ~100 MB of debug info.
strip "$STAGE/osiris" || echo "⚠️  strip unavailable — shipping unstripped binary"
strip "$STAGE/oexe" || echo "⚠️  strip unavailable — shipping unstripped CLI"
