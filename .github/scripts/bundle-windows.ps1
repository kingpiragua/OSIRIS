...
#   dist/osiris-<version>-windows-<arch>.zip        portable (unzip anywhere)
#   dist/osiris-<version>-windows-<arch>-setup.exe  Inno Setup installer
...
# executable as a resource (see build.rs). So the payload is osiris.exe plus a
...
$Stage = "dist/$Name"
$PackageUpdater = $env:OSIRIS_PACKAGE_UPDATE_HELPER -ne '0'

Remove-Item -Recurse -Force dist -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Force -Path $Stage | Out-Null

Copy-Item "target/$Target/release/osiris.exe" "$Stage/osiris.exe"
# The CLI, staged beside the GUI so both the zip and the installer carry it.
# `core::cli_install` resolves it relative to osiris.exe and puts that
# directory on the user's PATH. It is named oexe.exe, not osiris.exe: the GUI
# owns that name and both land in this one directory.
Copy-Item "target/$Target/release/oexe.exe" "$Stage/oexe.exe"
if ($PackageUpdater) {
    ...
    Copy-Item "target/$Target/release/osiris-updater.exe" "$Stage/osiris-updater.exe"
}
...
# `osiris.exe --daemon`, so beside it is exactly here. The in-box host
...
# The Linux musl `osiris-server`, staged at server/ so a WSL distro can be handed
...
# <dir of osiris.exe>/server/<asset> first — so this directory name is not free to
...
$ServerAsset = "osiris-server-linux-x86_64-musl"
...
if ($PackageUpdater) {
    Set-Content -Path "$Stage/.osiris-portable" -Value 'portable-v1' -NoNewline -Encoding ascii
}
...
if ($PackageUpdater) {
    ...
    Remove-Item -LiteralPath "$Stage/.osiris-portable" -Force
    Set-Content -Path "$Stage/.osiris-inno-install" -Value 'inno-v1' -NoNewline -Encoding ascii
}
