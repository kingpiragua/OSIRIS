...
#   core::update::package_for_current_install    — osiris-updater.exe beside the app
#   osiris-updater `windows::verify_portable_payload`  — portable layout + versions
#   osiris-updater `windows::extract_portable_archive` — ZIP entry rules
#   osiris-updater `windows::verify_file_version`      — setup.exe PE version
#
...
$Name  = "osiris-$Version-windows-$Arch"
$Zip   = "dist/$Name.zip"
...
if (-not (Test-Path -LiteralPath $Zip)) {
    ...
    $managed = @(
        'osiris.exe', 'oexe.exe', 'osiris-updater.exe', '.osiris-portable',
    ...
    if ($entries -contains '.osiris-inno-install') {
        Fail "the portable archive carries the Inno install marker"
    }

    $unzipped = Join-Path ([System.IO.Path]::GetTempPath()) "osiris-verify-portable-$([guid]::NewGuid())"
    ...
    try {
        ...
        foreach ($required in @('osiris.exe', 'oexe.exe', 'osiris-updater.exe',
                                '.osiris-portable', 'LICENSE.txt', 'README.md')) {
        ...
        $markerPath = Join-Path $unzipped '.osiris-portable'
        ...
        Assert-BinaryVersion (Join-Path $unzipped 'osiris.exe') 'the portable osiris.exe'
        Assert-BinaryVersion (Join-Path $unzipped 'osiris-updater.exe') 'the portable osiris-updater.exe'
...
if (-not (Test-Path -LiteralPath $Stage -PathType Container)) {
    ...
    if (-not (Test-Path -LiteralPath (Join-Path $Stage '.osiris-inno-install') -PathType Leaf)) {
        Fail "the Inno payload is missing the .osiris-inno-install marker; installed copies would never be offered an in-app update"
    }
    if (Test-Path -LiteralPath (Join-Path $Stage '.osiris-portable')) {
        Fail "the Inno payload carries the portable marker, which would misroute the updater"
    }
    if (-not (Test-Path -LiteralPath (Join-Path $Stage 'osiris-updater.exe') -PathType Leaf)) {
        Fail "the Inno payload is missing osiris-updater.exe"
    }
    Assert-BinaryVersion (Join-Path $Stage 'osiris.exe') 'the installed osiris.exe'
    Assert-BinaryVersion (Join-Path $Stage 'osiris-updater.exe') 'the installed osiris-updater.exe'
...
Write-Output "Windows package verified: markers, osiris-updater.exe and versions match $Version"
