...
use osiris_core::daemon::remote_link::{RemoteEnv, remote_control_socket};
use osiris_core::daemon::router::{RemoteRouter, RouteAck, RouteHeader};
use osiris_core::host::Host;
use osiris_core::host::remote::RemoteHost;

const EXE: &str = env!("CARGO_BIN_EXE_osiris-server");
...
fn a_routed_connection_reaches_a_real_server() {
    ...
    let router = std::thread::spawn(move || {
        ...
        let (kind, payload) = osiris_core::daemon::protocol::read_frame(&mut reader).unwrap();
        assert_eq!(kind, osiris_core::daemon::router::ROUTE_KIND);
...
fn an_unreachable_target_is_reported_not_dropped() {
    ...
    let router = std::thread::spawn(move || {
        let (stream, _) = listener.accept().unwrap();
        let mut reader = stream.try_clone().unwrap();
        let (_, payload) = osiris_core::daemon::protocol::read_frame(&mut reader).unwrap();
        let header = RouteHeader::decode(&payload).unwrap();
        RemoteRouter::route(stream, &header)
    });

    let mut sock = UnixStream::connect(&hub).unwrap();
    RouteHeader::local_stdio("osiris-server-that-does-not-exist", &[])
        .write(&mut sock)
        .unwrap();
...
fn bound_control_socket(runtime_dir: Option<&str>, home: &str) -> String {
    ...
    let mut child = cmd.spawn().expect("start osiris-server --daemon");

    let stderr = BufReader::new(child.stderr.take().expect("piped"));
    let mut bound = None;
    for line in stderr.lines().map_while(Result::ok) {
        if let Some(path) = line.strip_prefix("osiris-server: control socket at ") {
