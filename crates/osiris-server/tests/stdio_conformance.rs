...
use osiris_core::daemon::control::{ControlHello, LinkShutdown};
use osiris_core::host::SharedHost;
use osiris_core::host::conformance::Sandbox;
...
fn stdio_host() -> (SharedHost, TempSandbox) {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_osiris-server"))
        .args(["--stdio", "--serve"])
        .env("OSIRIS_DATA_DIR", sandbox.path())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
...
osiris_core::host_conformance_suite!(remote_stdio, stdio_host);

#[test]
fn the_whole_suite_ran_against_the_server() {
    let names: Vec<&str> = osiris_core::host::conformance::CASES
        .iter()
        .map(|(n, _)| *n)
        .collect();
    assert!(
