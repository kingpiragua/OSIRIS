    ...
    fn await_ready(&self) {
        ...
        loop {
            ...
            assert!(
                ...
                "osiris-server did not open its pane endpoint within {READY_WITHIN:?}"
            );
...
fn send_input_reaches_the_shell_without_displacing_the_controller() {
    ...
    panes
        .send_input(pane_id, b"echo osiris_send_oneshot\r")
        .expect("one-shot input is acknowledged");
    collect_until(&mut session, b"osiris_send_oneshot");

    session
        .input(b"echo osiris_still_controller\r")
        .expect("the controller keeps its seat");
    collect_until(&mut session, b"osiris_still_controller");
...
fn a_preempting_attach_closes_the_displaced_controller_and_drops_its_input() {
    ...
    first
        ...
        .expect("bound the stream reads");
    first
        .input(b"echo osiris_first_seated\r")
        .expect("the first controller types");
    collect_until(&mut first, b"osiris_first_seated");
    ...
    let _ = first.input(b"echo osiris_stale_input\r");
    ...
    second
        .input(b"echo osiris_second_alive\r")
        .expect("the new controller types");
    let seen = collect_until(&mut second, b"osiris_second_alive");
    assert!(
        !windows_contain(&seen, b"osiris_stale_input"),
