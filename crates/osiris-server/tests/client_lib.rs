...
use osiris_core::core::machine::{LayoutDelta, PaneSeed};
use osiris_core::daemon::control::{ControlEvent, ControlHello, ControlRequest, ReplyOk, feature};
    ...
    fn await_ready(&self) {
        ...
        loop {
            ...
            assert!(
                ...
                "osiris-server did not open its endpoints within {READY_WITHIN:?}"
            );
...
fn collect_until(
    session: &mut osiris_core::client::PaneSession,
    marker: &[u8],
...
fn drain_until_exit(session: &mut osiris_core::client::PaneSession) -> Vec<u8> {
    let mut seen: Vec<u8> = Vec::new();
    loop {
        match session.recv() {
            Ok(DaemonMsg::Output(bytes)) | Ok(DaemonMsg::Snapshot(bytes)) => {
...
fn a_spawned_pane_streams_its_output_and_its_exit() {
    ...
    let mut session = daemon
        ...
        .spawn(
            None,
            size(),
            Some(one_shot_shell("echo osiris_pane_roundtrip")),
    ...
    assert!(
        windows_contain(&seen, b"osiris_pane_roundtrip"),
        "output was {:?}",
        String::from_utf8_lossy(&seen)
    );
}
...
fn input_reaches_the_shell_and_a_reattach_replays_it() {
    ...
    session
        .input(b"echo osiris_attach_replay\r")
        .expect("type into the pane");
    let (_, exit) = collect_until(&mut session, b"osiris_attach_replay");
    ...
    let (_, exit) = collect_until(&mut reattached, b"osiris_attach_replay");
