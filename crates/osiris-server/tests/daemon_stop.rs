...
use osiris_core::daemon::protocol::ClientMsg;
...
fn spawn_daemon(dir: &Path) -> Child {
    Command::new(env!("CARGO_BIN_EXE_osiris-server"))
        .arg("--daemon")
        .arg("--config-dir")
        .arg(dir)
        .env("OSIRIS_DATA_DIR", dir)
        .env("OSIRIS_CONTROL_SOCK", dir.join("control.sock"))
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
...
fn await_ready(dir: &Path) {
    ...
    while PaneClient::at(&endpoint).version().is_err() || !dir.join("daemon.pid").exists() {
        assert!(
            ...
            "osiris-server did not open its endpoint within {READY_WITHIN:?}"
        );
...
fn stop_reaps_a_lingering_daemon_even_after_the_pidfile_vanishes() {
    let dir = tempfile::TempDir::new().unwrap();
    osiris_core::core::config::set_config_dir(dir.path().to_path_buf());
