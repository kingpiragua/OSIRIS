...
use osiris_core::core::machine::{Axis, LayoutDelta, MACHINE_FILE, PaneNode, PaneSeed};
use osiris_core::daemon::control::{
...
fn connect(data_dir: &Path, token: &str) -> Client {
    let mut child = Command::new(env!("CARGO_BIN_EXE_osiris-server"))
        .args(["--stdio", "--serve"])
        .env("OSIRIS_DATA_DIR", data_dir)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
        .expect("could not start osiris-server --stdio");
    ...
    .expect("handshake with osiris-server --stdio");
...
fn the_server_advertises_the_pane_daemons_features() {
    ...
    assert!(
        client
            .peer_features
            .iter()
            .any(|f| f == osiris_core::daemon::protocol::FEATURE_RESIZE_ECHO),
...
fn an_operation_from_one_client_reaches_the_other_as_a_delta() {
    ...
    let dir = data_dir();
    let machine = osiris_core::core::machine::MachineStore::open(machine_file(&dir));
...
fn attachment_rides_the_tree_when_no_record_store_is_served() {
    ...
    // And a peer asking for the tree is told the same. `oexe ls` fills its
...
fn started_once(home: &Path, extra: &[&str]) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_osiris-server"))
        ...
        .env_remove("OSIRIS_CONFIG_DIR")
        .env_remove("XDG_DATA_HOME")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("could not start osiris-server --stdio --serve");

    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        match child.try_wait().expect("waiting on osiris-server") {
            Some(_) => return,
            None if Instant::now() >= deadline => {
                let _ = child.kill();
                let _ = child.wait();
                panic!("osiris-server kept running with its link at EOF");
            }
...
fn seed_legacy_tree(home: &Path) -> PathBuf {
    let legacy = home.join(".local").join("share").join("osiris");
...
fn a_server_started_after_the_upgrade_carries_the_legacy_tree_in() {
    ...
    assert!(
        home.path().join(".config/osiris").join(MACHINE_FILE).exists(),
...
/// And the half that keeps the upgrade from becoming the bug: a second osiris on
