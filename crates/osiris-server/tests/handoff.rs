...
//!
//! The proof is a variable. `osiris_kept=…` lives in that shell's memory and
...
impl Daemon {
    ...
    fn binary() -> PathBuf {
        PathBuf::from(env!("CARGO_BIN_EXE_osiris-server"))
    }
...
fn a_handoff_keeps_the_process_the_pty_and_the_shell_that_is_on_it() {
    ...
    session
        .input(b"osiris_kept=survivor; echo osiris_before_$osiris_kept\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"osiris_before_survivor");
    drop(session);
    ...
    let replayed = collect_until(&mut session, b"osiris_before_survivor");
    assert!(
        windows_contain(&replayed, b"osiris_before_survivor"),
    ...
    // is unambiguous.
    session
        .input(b"echo osiris_after_$osiris_kept\r")
        .expect("the shell still takes input");
    collect_until(&mut session, b"osiris_after_survivor");
...
fn a_handoff_to_something_that_will_not_exec_leaves_the_daemon_serving() {
    ...
    session
        ...
        .expect("bound the stream reads");
    session
        .input(b"echo osiris_still_here\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"osiris_still_here");

    let err = panes
        .hand_off(Path::new("/nonexistent/osiris-that-is-not-there"))
    ...
    session
        .input(b"echo osiris_unharmed\r")
        .expect("the pane still has its shell");
    collect_until(&mut session, b"osiris_unharmed");
    assert_eq!(
        session.pane_id(),
        pane_id,
