...
use osiris_core::client::PaneClient;
use osiris_core::daemon::protocol::{DaemonMsg, ShellSpec, WinSize};
...
impl Daemon {
    ...
    fn with_home(bashrc: &str, history: &str) -> Daemon {
        ...
        let child = Command::new(env!("CARGO_BIN_EXE_osiris-server"))
            .arg("--daemon")
            .arg("--config-dir")
            .arg(dir.path())
            .env("HOME", dir.path())
            .env("OSIRIS_DATA_DIR", dir.path())
            .env("OSIRIS_CONTROL_SOCK", dir.path().join("control.sock"))
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("start osiris-server --daemon");
...
fn collect_until(session: &mut osiris_core::client::PaneSession, marker: &[u8]) -> Vec<u8> {
...
fn a_pane_writes_its_own_history_and_hands_it_back_when_it_closes() {
    ...
    session
        .input(b"echo osiris_ran_here\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"osiris_ran_here");
    ...
    assert!(
        !daemon.global_history().contains("osiris_ran_here"),
...
/// The snippet seeds with `tail … > "$OSIRIS_HISTFILE"`, and a redirection
...
fn a_small_histfilesize_cannot_swallow_the_merge_back() {
    ...
    session
        .input(b"echo osiris_survives_truncation\r")
        .expect("the shell takes input");
    collect_until(&mut session, b"osiris_survives_truncation");
    ...
    wait_until(
        || daemon.global_history().contains("osiris_survives_truncation"),
...
fn two_panes_do_not_see_each_others_commands() {
    ...
    first
        .input(b"echo osiris_first_pane\r")
        .expect("the first shell takes input");
    collect_until(&mut first, b"osiris_first_pane");
    second
        .input(b"echo osiris_second_pane\r")
        .expect("the second shell takes input");
    collect_until(&mut second, b"osiris_second_pane");
    ...
    wait_until(
        || {
            daemon
                ...
                .is_some_and(|h| h.contains("osiris_first_pane"))
        },
    ...
    wait_until(
        || {
            daemon
                ...
                .is_some_and(|h| h.contains("osiris_second_pane"))
        },
