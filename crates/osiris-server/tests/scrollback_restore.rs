...
use osiris_core::core::machine::PaneSeed;
use osiris_core::daemon::control::{ControlHello, ControlRequest, ReplyOk};
use osiris_core::daemon::protocol::{ClientMsg, DaemonMsg, RestoreFrom, ShellSpec, WinSize};
use osiris_core::daemon::transport;
...
const MARKER: &[u8] = b"osiris_screen_kept";
  ...
  "panes": [
    ...
    fn start(&self) -> Running {
        let child = Command::new(env!("CARGO_BIN_EXE_osiris-server"))
            ...
            .arg("--config-dir")
            .arg(self.path())
            .env("OSIRIS_DATA_DIR", self.path())
            .env("OSIRIS_CONTROL_SOCK", self.path().join("control.sock"))
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
