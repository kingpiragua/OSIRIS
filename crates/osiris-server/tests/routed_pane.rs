...
use osiris_core::daemon::protocol::{ClientMsg, DaemonMsg, ShellSpec, WinSize};
use osiris_core::daemon::router::{RemoteRouter, RouteChannel, RouteHeader, negotiate};
...
fn a_routed_version_reply_names_the_resize_echo_feature() {
    ...
    assert!(
        version.has_feature(osiris_core::daemon::protocol::FEATURE_RESIZE_ECHO),
...
fn the_channel_decides_which_dialect_the_route_carries() {
    ...
    let (_, payload) = osiris_core::daemon::protocol::read_frame(&mut buf.as_slice()).unwrap();
