...
const USAGE: &str = "\
osiris-server — the osiris session daemon, headless

USAGE:
    osiris-server --daemon [--config-dir <dir>]
    osiris-server --stdio [--serve | --bridge] [--control-sock <path>]
    osiris-server --stdio --pane [--config-dir <dir>]
    osiris-server agent-hook <agent> <event>
...
fn main() -> ExitCode {
    ...
    if args.first().map(String::as_str) == Some("agent-hook") {
        if let (Some(agent), Some(event)) = (args.get(1), args.get(2)) {
            osiris_core::core::agent_hooks::run_agent_hook(agent, event);
        }
        return ExitCode::SUCCESS;
    }

    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("osiris-server {}", env!("CARGO_PKG_VERSION"));
        return ExitCode::SUCCESS;
    }
    if args
        .iter()
        .any(|a| a == osiris_core::daemon::install::PROTOCOL_FLAG)
    {
        println!(
            "{}",
            osiris_core::daemon::install::RemoteProtocol::of_this_build().to_line()
        );
        return ExitCode::SUCCESS;
    }
    ...
    osiris_core::core::crash::install("server");
    osiris_core::core::logfile::install("server");

    if args.iter().any(|a| a == "--stdio") {
        return match run_stdio(&args) {
            Ok(()) => ExitCode::SUCCESS,
            Err(e) => {
                eprintln!("osiris-server: stdio session ended with error: {e}");
                ExitCode::FAILURE
            }
        };
    }
    ...
    eprint!("osiris-server: nothing to do without --daemon or --stdio\n\n{USAGE}");
    ExitCode::FAILURE
}

fn run_daemon() -> ExitCode {
    if let Err(e) = osiris_core::daemon::server::run_daemon() {
        eprintln!("osiris-server: daemon exited with error: {e}");
...
fn run_stdio(args: &[String]) -> io::Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        use osiris_core::daemon::duplex::StdioDuplex;
        use osiris_core::daemon::spawn;
        use osiris_core::host::local::LocalHost;
        use osiris_core::host::server;
        ...
        match upstream {
            ...
            None => {
                ...
                server::serve_with(
                    link,
                    LocalHost::shared(),
                    osiris_core::daemon::server::control_services(),
                )
            }
        }
    }
...
fn bridge_panes() -> io::Result<()> {
    use osiris_core::daemon::{spawn, transport};
...
fn bridge(upstream: std::os::unix::net::UnixStream) -> io::Result<()> {
    ...
    let feeder = std::thread::Builder::new()
        .name("osiris-stdio-bridge-in".into())
        .spawn(move || {
