...
/// makes handle inheritance follow *that* process, so osiris cannot hand the
...
pub fn run_daemon() -> anyhow::Result<()> {
    ...
    let _seat = match crate::daemon::singleton::claim() {
        ...
        crate::daemon::singleton::Claim::Taken => {
            startup_note!("osiris-server: another server already serves this config dir; exiting");
            return Ok(());
        }
        crate::daemon::singleton::Claim::Unavailable(why) => {
            startup_note!("osiris-server: starting without the single-server lock ({why})");
            None
        }
    };
    ...
    #[cfg(any(unix, windows))]
    {
        ...
        match crate::host::server::spawn_control_listener_with(
            ...
            Ok(path) => startup_note!("osiris-server: control socket at {}", path.display()),
            Err(e) => startup_note!("osiris-server: control listener unavailable: {e}"),
        }
    }
...
fn run_adopting(inheritance: crate::daemon::handoff::Inheritance) -> anyhow::Result<()> {
    ...
    if _seat.is_none() {
        startup_note!("osiris-server: handed over without a seat descriptor; serving unprotected");
    }
    ...
    match crate::daemon::handoff::adopt(inheritance.blob_fd) {
        Some(adopted) => {
            ...
            startup_note!("osiris-server: adopted {kept} pane(s) from the previous build");
        }
        ...
        // was lost.
        None => startup_note!(
            "osiris-server: the handoff blob was unreadable; the previous build's panes are lost"
        ),
    }
...
fn reaper(registry: Arc<Registry>, id: u64) -> impl FnOnce() + Send + 'static {
    move || {
        std::thread::Builder::new()
            .name("osiris-daemon-pane-reap".to_string())
            .spawn(move || {
                registry.remove(id);
            })
            .ok();
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                std::thread::Builder::new()
                    .name("osiris-daemon-conn".to_string())
                    .spawn(move || {
...
fn serve_sigterm(registry: Arc<Registry>) {
    ...
    std::thread::Builder::new()
        .name("osiris-daemon-sigterm".to_string())
        .spawn(move || {
...
fn spawn_writer(
    ...
    std::thread::Builder::new()
        .name("osiris-daemon-writer".to_string())
        .spawn(move || {
            crate::core::threads::promote_to_user_interactive();
