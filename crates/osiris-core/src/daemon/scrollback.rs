...
// Eight bytes exactly: six of name, then the format version. Upstream spelled
// it `TTY7SB\x01\x00`; "OSIRIS" is already six characters, so the "SB" tag is
// dropped rather than the width — the header is a fixed-size record and the
// reader compares all eight.
const MAGIC: &[u8; 8] = b"OSIRIS\x01\x00";
