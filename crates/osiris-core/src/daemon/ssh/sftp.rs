...
pub fn upload_temp_name(remote: &str) -> String {
    ...
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap_or(0);
    format!("{remote}.osiris-upload-{:x}{:x}", nanos, n)
}
...
fn download_temp_path(lpath: &Path) -> PathBuf {
    ...
    os.push(format!(".osiris-download-{:x}{:x}", nanos, n));
    PathBuf::from(os)
}
...
mod tests {
    ...
    fn upload_temp_name_is_distinct_and_marked() {
        ...
        assert!(a.starts_with("/dir/file.txt.osiris-upload-"));
        assert!(b.starts_with("/dir/file.txt.osiris-upload-"));
        assert_ne!(a, b);
    }
    ...
    fn download_temp_path_is_sibling_and_distinct() {
        ...
        assert!(
            a.file_name()
                .unwrap()
                .to_string_lossy()
                .starts_with("file.bin.osiris-download-")
        );
        assert_ne!(a, b);
    }
