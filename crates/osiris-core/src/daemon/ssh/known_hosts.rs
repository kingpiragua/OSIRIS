...
pub fn delete_in_file(path: &Path, id: &KnownHostId) -> std::io::Result<()> {
    ...
    let tmp = path.with_extension("osiris-tmp");
...
mod tests {
    ...
    fn append_preserves_existing_lines_and_adds_one() {
        let dir = std::env::temp_dir().join(format!("osiris-kh-{}", std::process::id()));
    ...
    fn overriding_a_changed_key_stops_trusting_the_one_it_replaces() {
        let dir = std::env::temp_dir().join(format!("osiris-kh-supersede-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("known_hosts");
