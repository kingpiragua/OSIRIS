    ...
    pub fn global() -> &'static SshManager {
        ...
        MANAGER.get_or_init(|| {
            let runtime = tokio::runtime::Builder::new_multi_thread()
                .worker_threads(2)
                .enable_all()
                .thread_name("osiris-ssh-rt")
                .build()
                .expect("build osiris ssh runtime");
            SshManager {
                runtime,
    ...
    pub fn spawn_native_session(
        ...
        self.runtime.spawn(async move {
                ...
                .await
            {
                ...
                let line = format!("\r\n\x1b[31mosiris: SSH connection failed: {reason}\x1b[0m\r\n");
...
mod tests {
    ...
    fn a_busy_link_reads_as_connected_rather_than_down() {
        ...
        assert!(
            ...
            "a link whose slot is momentarily held is busy, not down — calling it \
             down makes `osiris -m <machine>` refuse to route over a live connection"
        );
    }
    ...
    fn live_gssapi_connects_and_opens_a_channel() {
        let host = std::env::var("OSIRIS_LIVE_SSH_HOST").expect("OSIRIS_LIVE_SSH_HOST");
        let user = std::env::var("OSIRIS_LIVE_SSH_USER").expect("OSIRIS_LIVE_SSH_USER");
        let port = std::env::var("OSIRIS_LIVE_SSH_PORT")
            .ok()
