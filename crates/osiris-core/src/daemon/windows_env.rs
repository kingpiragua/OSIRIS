...
//! at `CreateProcess` time. An osiris daemon that has been up since Tuesday
//! therefore hands Tuesday's `PATH` to a pane created today, and an installer
//! that ran in between is invisible until osiris restarts (#333) — while a
...
//! hives, the daemon's own snapshot, and osiris's configured overrides — so all
...
///
/// Precedence runs process snapshot → machine hive → user hive → osiris's
...
pub(crate) fn merge_environment(
    ...
    // Configuration is the user telling osiris what a pane must see; it outranks
...
mod tests {
    ...
    fn process_only_variables_survive_the_refresh() {
        ...
        assert_eq!(lookup(&env, "OSIRIS_PANE").as_deref(), Some("7"));
    }
    ...
    mod live_registry {
        ...
        const PROBE_NAME: &str = "OSIRIS_ISSUE333_PROBE";
        ...
        fn a_real_child_process_resolves_a_variable_added_after_startup() {
            ...
            cmd.args(["/d", "/c", "echo probe=%OSIRIS_ISSUE333_PROBE%"]);
