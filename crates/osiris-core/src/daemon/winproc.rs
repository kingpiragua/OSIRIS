...
/// case-insensitive, and only at a component boundary so `...\osiris-two` is not
/// inside `...\osiris`.
...
mod ctrl_c_tests {
    ...
    const RESULT_ENV: &str = "OSIRIS_CTRL_C_INHERITED_IGNORE_RESULT";
    const TEST_NAME: &str =
    ...
    /// The reported bug (#451, #314): nothing in an osiris pane could be
    ...
    fn a_pane_survives_an_inherited_ignore_ctrl_c() {
        ...
        let result =
            std::env::temp_dir().join(format!("osiris-pane-ctrl-c-{}.txt", std::process::id()));
...
mod tests {
    ...
    fn path_is_under_respects_component_boundaries_and_case() {
        use std::path::Path;
        let dir = Path::new(r"C:\Users\me\Apps\osiris");
        assert!(path_is_under(
            Path::new(r"C:\Users\me\Apps\osiris\OpenConsole.exe"),
            dir
        ));
        assert!(path_is_under(
            Path::new(r"c:\users\me\apps\OSIRIS\server\osiris-server"),
            dir
        ));
        assert!(!path_is_under(
            Path::new(r"C:\Users\me\Apps\osiris-two\a.exe"),
            dir
        ));
        assert!(!path_is_under(Path::new(r"C:\Users\me\Apps\osiris"), dir));
        assert!(path_is_under(
            Path::new(r"C:\Users\me\Apps\osiris\x.exe"),
            Path::new(r"C:\Users\me\Apps\osiris\"),
        ));
