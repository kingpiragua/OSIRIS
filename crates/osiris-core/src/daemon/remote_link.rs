...
pub const DEFAULT_REMOTE_SERVER_CMD: &str = "osiris-server --stdio";
...
const ENV_MARKER: &str = "__osiris_env__";

pub const REMOTE_ENV_PROBE: &str = concat!(
    "sh -c 'printf \"__osiris_env__ %s\\n\" ",
    "\"sock=${OSIRIS_CONTROL_SOCK-}\" \"cfg=${OSIRIS_CONFIG_DIR-}\" ",
    "\"xdg=${XDG_RUNTIME_DIR-}\" ",
...
pub fn remote_control_socket(env: &RemoteEnv) -> Option<String> {
    ...
    // opens its control socket in the config dir — `$OSIRIS_CONFIG_DIR` if the
    // remote sets one, otherwise `$HOME/.config/osiris`. This has to mirror
    ...
    let dir = match env.config_dir.as_deref().filter(|d| !d.is_empty()) {
        ...
        None => {
            ...
            posix_join(&posix_join(home, ".config"), "osiris")
        }
    };
...
mod tests {
    ...
    fn only_a_pane_link_asks_for_the_pane_socket() {
        let server = "/home/me/.local/share/osiris/bin/osiris-server-26.7.6";
        assert_eq!(
    ...
    fn the_entry_falls_back_exactly_when_streamlocal_cannot_be_used() {
        let cmd = "osiris-server --stdio";
        assert_eq!(
            choose_entry(Some("/run/user/1000/osiris/daemon.sock"), true, cmd),
            RemoteEntry::StreamLocal {
                socket: "/run/user/1000/osiris/daemon.sock".into()
            }
        );
        assert_eq!(
            choose_entry(Some("/run/user/1000/osiris/daemon.sock"), false, cmd),
    ...
    fn the_env_probe_survives_a_chatty_remote() {
        let out = "Welcome to Ubuntu!\n\
                   __osiris_env__ sock=\n\
                   __osiris_env__ xdg=/run/user/1000\n\
                   __osiris_env__ home=/home/me\n\
                   __osiris_env__ tmp=\n\
                   You have mail.\n";
    ...
    fn the_remote_socket_path_matches_the_servers_own_order() {
        ...
        assert_eq!(
            ...
            Some("/tmp/mine.sock"),
            "an explicit $OSIRIS_CONTROL_SOCK outranks everything"
        );
        ...
        let explicit_cfg = RemoteEnv {
            config_dir: Some("/home/me/.config/osiris".into()),
            xdg_runtime_dir: Some("/run/user/1000".into()),
            home: Some("/home/me".into()),
            ..RemoteEnv::default()
        };
        assert_eq!(
            ...
            Some("/home/me/.config/osiris/control.sock"),
            "an explicit $OSIRIS_CONFIG_DIR names the directory"
        );
        ...
        assert_eq!(
            ...
            Some("/home/me/.config/osiris/control.sock"),
            "with no $OSIRIS_CONFIG_DIR the remote server uses $HOME/.config/osiris"
        );
