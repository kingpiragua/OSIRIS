...
# --- OSIRIS pane integration (zsh) ---
if [[ -o interactive ]] && [[ -z "$OSIRIS_SHELL_INTEGRATION" ]]; then
  export OSIRIS_SHELL_INTEGRATION=1
  ...
  # first line is read.
  if [[ -n "$OSIRIS_HISTFILE" ]]; then
    if [[ ! -e "$OSIRIS_HISTFILE" ]]; then
      ...
      if [[ -n "$HISTFILE" ]]; then
        builtin printf '%s\n' "$HISTFILE" > "$OSIRIS_HISTFILE.origin" 2>/dev/null
        if [[ -r "$HISTFILE" ]]; then
          command tail -n 5000 "$HISTFILE" > "$OSIRIS_HISTFILE" 2>/dev/null
        fi
      fi
      [[ -e "$OSIRIS_HISTFILE" ]] || : > "$OSIRIS_HISTFILE" 2>/dev/null
      command wc -c < "$OSIRIS_HISTFILE" 2>/dev/null | command tr -d ' \n' \
        > "$OSIRIS_HISTFILE.seed" 2>/dev/null
    fi
    HISTFILE="$OSIRIS_HISTFILE"
  ...
  __osiris_osc() { builtin printf '\e]%s\a' "$1"; }
  ...
  # the Esc widget name.
  __osiris_report_edit_mode() {
    if [[ "$(builtin bindkey -lL main)" == *viins* ]]; then
      __osiris_osc "133;V;1"
    else
      __osiris_osc "133;V;0"
    fi
  }
  ...
  __osiris_report_cwd() { builtin printf '\e]7;file://%s%s\a' "${HOST:-localhost}" "${PWD//\%/%25}"; }

  # D (command finished + its exit code) gets its own hook, *prepended* to
  # precmd_functions rather than bundled into __osiris_precmd below: the app only
  ...
  __osiris_precmd_d() {
    local ret=$?
    if [[ -n "$__osiris_cmd_active" ]]; then
      __osiris_osc "133;D;$ret"
      unset __osiris_cmd_active
    fi
  }
  ...
  __osiris_preexec() {
    ...
    cmd=${cmd//$'\r'/%0D}
    cmd=${cmd//$'\n'/%0A}
    __osiris_osc "133;C;$cmd"
  }

  autoload -Uz add-zsh-hook
  add-zsh-hook precmd __osiris_precmd
  add-zsh-hook preexec __osiris_preexec
  ...
  precmd_functions=(__osiris_precmd_d $precmd_functions)
  ...
  __osiris_restore_zdotdir() {
    ZDOTDIR=${OSIRIS_USER_ZDOTDIR:-$HOME}
    add-zsh-hook -d precmd __osiris_restore_zdotdir
    unfunction __osiris_restore_zdotdir
  }
  add-zsh-hook precmd __osiris_restore_zdotdir
fi
# --- end OSIRIS pane integration ---
"#;

const FISH_INTEGRATION: &str = r#"
# --- OSIRIS pane integration (fish) ---
...
if status is-interactive; and test -z "$OSIRIS_SHELL_INTEGRATION"
  set -gx OSIRIS_SHELL_INTEGRATION 1

  function __osiris_osc
    printf '\e]%s\a' $argv[1]
  end

  function __osiris_report_edit_mode
  ...
  function __osiris_preexec --on-event fish_preexec
    set -g __osiris_cmd_active 1
  ...
  function __osiris_precmd --on-event fish_prompt
    set -l ret $status
    if set -q __osiris_cmd_active
      __osiris_osc "133;D;$ret"
      set -e __osiris_cmd_active
    end
    __osiris_report_cwd
    __osiris_report_edit_mode
    __osiris_osc "133;A"
  ...
  functions -c fish_prompt __osiris_original_fish_prompt
  function fish_prompt
    __osiris_original_fish_prompt
    __osiris_osc "133;B"
  end
end
# --- end OSIRIS pane integration ---
...
# --- OSIRIS pane integration (bash) ---
if [[ $- == *i* ]] && [[ -z "$OSIRIS_SHELL_INTEGRATION" ]]; then
  export OSIRIS_SHELL_INTEGRATION=1
  ...
  if [[ "$OSTYPE" == msys* || "$OSTYPE" == cygwin* ]]; then
    ...
    __osiris_report_cwd() { builtin printf '\e]7;file://%s%s\a' "${HOSTNAME:-localhost}" "${PWD//\%/%25}"; }
  fi
  ...
  precmd_functions=(__osiris_precmd_d "${precmd_functions[@]}")
  precmd_functions+=(__osiris_precmd)
  preexec_functions+=(__osiris_preexec)
fi
# --- end OSIRIS pane integration ---
"#;

const POWERSHELL_INTEGRATION: &str = r#"
# --- OSIRIS pane integration (PowerShell) ---
if (-not $env:OSIRIS_SHELL_INTEGRATION) {
  $env:OSIRIS_SHELL_INTEGRATION = '1'

  $global:__OsirisEsc = [char]0x1b
  $global:__OsirisBel = [char]0x07
  # Whatever prompt the user's profile settled on; we call through to it.
  $global:__OsirisOrigPrompt = $function:prompt
  # Gates the D marker so the first prompt (no command yet) emits no bogus exit.
  $global:__OsirisCmdActive = $false
  ...
  $global:__OsirisUser = [Environment]::UserName
  $global:__OsirisHost = [Environment]::MachineName
  $global:__OsirisHome = if ($HOME) { $HOME.Replace('\', '/').TrimEnd('/') } else { '' }
  ...
  function global:prompt {
    ...
    if ($global:__OsirisCmdActive) {
      ...
      Write-Host -NoNewline "$($global:__OsirisEsc)]133;D;$code$($global:__OsirisBel)"
    }
    ...
    if ($PWD.Provider.Name -eq 'FileSystem') {
      ...
      Write-Host -NoNewline "$($global:__OsirisEsc)]7;file://$($global:__OsirisHost)$p$($global:__OsirisBel)"
      ...
      # so without this every osiris tab running PowerShell stays generic, on every
      # platform. Forward slashes (so osiris's tab-label parser can take the last
      ...
      if ($global:__OsirisHome -and ($titlePath -eq $global:__OsirisHome -or
          $titlePath.StartsWith($global:__OsirisHome + '/'))) {
        $titlePath = '~' + $titlePath.Substring($global:__OsirisHome.Length)
      }
      Write-Host -NoNewline "$($global:__OsirisEsc)]0;$($global:__OsirisUser)@$($global:__OsirisHost):$titlePath$($global:__OsirisBel)"
    }
    ...
    $base = & $global:__OsirisOrigPrompt
    if ($base -is [array]) { $base = $base -join [char]0x0a }
    $global:LASTEXITCODE = $lastExit

    "$($global:__OsirisEsc)]133;A$($global:__OsirisBel)$base$($global:__OsirisEsc)]133;B$($global:__OsirisBel)"
  }
  ...
  if ((Test-Path Function:\PSConsoleHostReadLine) -and -not $global:__OsirisReadLineWrapped) {
    $global:__OsirisReadLineWrapped = $true
    $global:__OsirisOrigReadLine = $function:global:PSConsoleHostReadLine
    function global:PSConsoleHostReadLine {
      $line = & $global:__OsirisOrigReadLine
      if (-not [string]::IsNullOrWhiteSpace($line)) {
        $global:__OsirisCmdActive = $true
        # Carry the submitted line on the C mark (osiris extension): the daemon
        ...
        try {
          ...
          Write-Host -NoNewline "$($global:__OsirisEsc)]133;C;$cmd$($global:__OsirisBel)"
        } catch {
          Write-Host -NoNewline "$($global:__OsirisEsc)]133;C$($global:__OsirisBel)"
        }
      }
      $line
    }
...
# --- OSIRIS pane integration (nushell) ---
...
if (($env.OSIRIS_SHELL_INTEGRATION? | default "") == "") {
  $env.OSIRIS_SHELL_INTEGRATION = "1"

  # osiris restores the user's own config.nu at this placeholder — `source` is
  ...
  $env.config.hooks.pre_prompt = ($env.config.hooks.pre_prompt | default [] | append {||
    ...
    let __osiris_osc = {|__osiris_payload| print -n $"(ansi osc)($__osiris_payload)(char bel)" }
    if ($env.__osiris_cmd_active? | default false) {
      hide-env __osiris_cmd_active
      do $__osiris_osc $"133;D;($env.LAST_EXIT_CODE? | default 0)"
    }
    ...
    let __osiris_path = if (($nu.os-info.name? | default '') == 'windows') {
      ($env.PWD | str replace -a '\' '/' | str replace -a '%' '%25')
    } else {
      ($env.PWD | str replace -a '%' '%25')
    }
    let __osiris_path = if ($__osiris_path | str starts-with '/') { $__osiris_path } else { '/' + $__osiris_path }
    do $__osiris_osc $"7;file://($env.COMPUTERNAME? | default 'localhost')($__osiris_path)"
    do $__osiris_osc "133;A"
  })
  ...
  if ((($env.config.prompt_indicator? | default null) | describe) != 'nothing') {
    let __osiris_orig_indicator = $env.config.prompt_indicator
    $env.config.prompt_indicator = {||
      let __osiris_ind = match ($__osiris_orig_indicator | describe) {
        'closure' => (do $__osiris_orig_indicator)
        _ => $__osiris_orig_indicator
      }
      $"($__osiris_ind)(ansi osc)133;B(char bel)"
    }
  }
}
# --- end OSIRIS pane integration ---
...
fn zsh_redirectors() -> [(&'static str, String); 4] {
    let redirect = |name: &str, tail: &str| {
        format!(
            "__osiris_ztmp=$ZDOTDIR\n\
             if [[ -n \"$OSIRIS_USER_ZDOTDIR\" ]]; then ZDOTDIR=$OSIRIS_USER_ZDOTDIR; else unset ZDOTDIR; fi\n\
             [[ -f \"${{ZDOTDIR:-$HOME}}/{name}\" ]] && source \"${{ZDOTDIR:-$HOME}}/{name}\"\n\
             {tail}ZDOTDIR=$__osiris_ztmp\n\
             unset __osiris_ztmp\n"
        )
    };
    [
        (
            ".zshenv",
            redirect(".zshenv", "export OSIRIS_USER_ZDOTDIR=${ZDOTDIR:-$HOME}\n"),
        ),
...
const ZDOTDIR_PREFIX: &str = "osiris-zdotdir-";
...
fn real_user_zdotdir() -> Option<String> {
    if let Ok(z) = std::env::var("OSIRIS_USER_ZDOTDIR") {
        if !z.is_empty() {
            return Some(z);
        }
    }
...
fn setup_zsh() -> Option<Injection> {
    ...
    if let Some(user_zdotdir) = real_user_zdotdir() {
        env.insert("OSIRIS_USER_ZDOTDIR".to_string(), user_zdotdir);
    }
...
fn setup_nushell_with(user_config: Option<&Path>) -> Option<Injection> {
    let dir = throwaway_dir("osiris-nu-")?;
...
fn nushell_config_script_with(user_config: Option<&Path>) -> String {
    ...
    NUSHELL_INTEGRATION.replace("__OSIRIS_SOURCE_USER_CONFIG__", &source)
}
...
/// osiris keeps its own config (`~/.config/osiris` on macOS too); nu follows the
...
fn platform_config_dir() -> Option<PathBuf> {
    ...
    // same directory in every environment osiris can spawn a pane in.
...
# necessary because osiris spawns bash non-login (see shell_integration.rs) so
...
__osiris_bashrc=0
source() {{ case "${{1-}}" in */.bashrc|.bashrc) __osiris_bashrc=1;; esac; builtin source "$@"; }}
.() {{ case "${{1-}}" in */.bashrc|.bashrc) __osiris_bashrc=1;; esac; builtin . "$@"; }}
...
unset -f source .
if [[ $__osiris_bashrc == 0 && -f ~/.bashrc ]]; then source ~/.bashrc; fi
unset __osiris_bashrc
{BASH_INTEGRATION}"#
    )
}
...
fn setup_bash() -> Option<Injection> {
    let dir = throwaway_dir("osiris-bashrc-")?;
...
const WSL_RCFILE_ENV: &str = "OSIRIS_RC";
...
const WSL_ZDOTDIR_ENV: &str = "OSIRIS_ZDOTDIR";
...
/// user's own `.bashrc` down with it, because osiris starts it non-login so that
...
fn wsl_exec_script() -> String {
    format!(
        concat!(
            r#"case "${{SHELL:-}}" in "#,
            r#"*/bash) [ -r "${{OSIRIS_RC:-}}" ] && exec "$SHELL" --rcfile "$OSIRIS_RC" -i; "#,
            r#"exec "$SHELL" -l ;; "#,
            r#"*/zsh) [ -n "${{OSIRIS_ZDOTDIR:-}}" ] && [ -r "$OSIRIS_ZDOTDIR/.zshrc" ] "#,
            r#"&& export ZDOTDIR="$OSIRIS_ZDOTDIR"; exec "$SHELL" -l ;; "#,
...
///
/// `OSIRIS_USER_ZDOTDIR` is conspicuously absent, and that is the point.
...
fn setup_wsl(args: &[String]) -> Option<Injection> {
    ...
    let dir = throwaway_dir("osiris-wslrc-")?;
...
pub fn setup(program: Option<&str>, args: &[String], has_custom_args: bool) -> Option<Injection> {
    ...
    // files run. Arguments osiris's own detection supplied (Git Bash's `-i -l`,
    ...
    injection
        .env
        .insert("OSIRIS_SHELL_INTEGRATION".to_string(), String::new());
...
pub mod remote {
    ...
    pub const PROBE_COMMAND: &str = "echo __osiris_shell; echo $SHELL";

    const PROBE_MARKER: &str = "__osiris_shell";

    const HEREDOC: &str = "__OSIRIS_RC_EOF__";
    ...
    fn write_file(out: &mut String, name: &str, body: &str) {
        out.push_str(&format!(
            "command cat > \"$__osiris_d/{name}\" <<'{HEREDOC}'\n{}\n{HEREDOC}\n",
    ...
    fn zsh_bootstrap(shell_path: &str) -> String {
        ...
        out.push_str("__osiris_d=${TMPDIR:-/tmp}/osiris-zdotdir-$$\n");
        out.push_str("command mkdir -p \"$__osiris_d\" 2>/dev/null\n");
        ...
        for (name, contents) in zsh_redirectors() {
            ...
            guard.push_str(&format!("[ -s \"$__osiris_d/{name}\" ] && "));
        }
        out.push_str(&format!(
            "{guard}export ZDOTDIR=\"$__osiris_d\" OSIRIS_RM_DIR=\"$__osiris_d\"\n"
        ));
    ...
    fn bash_bootstrap(shell_path: &str) -> String {
        ...
        let mut out = String::new();
        out.push_str("__osiris_d=${TMPDIR:-/tmp}/osiris-bashrc-$$\n");
        out.push_str("command mkdir -p \"$__osiris_d\" 2>/dev/null\n");
        write_file(
            &mut out,
            "bashrc",
        ...
        out.push_str("if [ -s \"$__osiris_d/bashrc\" ]; then\n");
        out.push_str("export OSIRIS_RM_DIR=\"$__osiris_d\"\n");
        out.push_str(&format!("exec {quoted} --rcfile \"$__osiris_d/bashrc\" -i\n"));
        out.push_str("fi\n");
...
# --- osiris remote cleanup (zsh) ---
if [[ -n "$OSIRIS_RM_DIR" ]]; then
  typeset -g __osiris_rm_dir=$OSIRIS_RM_DIR
  unset OSIRIS_RM_DIR
  __osiris_rm_rcdir() {
    [[ -n "$__osiris_rm_dir" ]] || return 0
    command rm -rf -- "$__osiris_rm_dir"
    unset __osiris_rm_dir
  }
  autoload -Uz add-zsh-hook
  add-zsh-hook precmd __osiris_rm_rcdir
fi
# --- end osiris remote cleanup ---
"#;

    const BASH_CLEANUP_HOOK: &str = r#"
# --- osiris remote cleanup (bash) ---
if [[ -n "$OSIRIS_RM_DIR" ]]; then
  __osiris_rm_dir=$OSIRIS_RM_DIR
  unset OSIRIS_RM_DIR
  __osiris_rm_rcdir() {
    [[ -n "$__osiris_rm_dir" ]] || return 0
    command rm -rf -- "$__osiris_rm_dir"
  ...
  precmd_functions+=(__osiris_rm_rcdir)
fi
# --- end osiris remote cleanup ---
"#;
    ...
    mod tests {
        ...
        fn probe_declines_anything_that_isnt_a_shell_we_know() {
            assert_eq!(parse_probe("__osiris_shell\n$SHELL\n"), None);
            assert_eq!(parse_probe("__osiris_shell\n\n"), None);
            assert_eq!(parse_probe("__osiris_shell\n/bin/ksh\n"), None);
        ...
        fn zsh_bootstrap_gates_zdotdir_on_every_redirector_landing() {
            ...
            for name in [".zshenv", ".zprofile", ".zshrc", ".zlogin"] {
                assert!(
                    script.contains(&format!("[ -s \"$__osiris_d/{name}\" ] &&")),
            ...
            assert!(export < exec);
            assert!(script.contains("__osiris_report_cwd"));
        }
        ...
        fn bash_bootstrap_forces_a_non_login_shell_through_the_rcfile() {
            ...
            assert!(script.contains("exec '/bin/bash' --rcfile \"$__osiris_d/bashrc\" -i"));
...
mod tests {
    ...
    fn is_our_zdotdir_matches_only_our_prefix() {
        assert!(is_our_zdotdir("/tmp/osiris-zdotdir-1234-0"));
        assert!(is_our_zdotdir("osiris-zdotdir-x"));
        assert!(!is_our_zdotdir("/home/alice/.config/zsh"));
        assert!(!is_our_zdotdir("/tmp/other-zdotdir"));
        assert!(!is_our_zdotdir(""));
        assert!(!is_our_zdotdir("/tmp/not-osiris-zdotdir-1"));
    }
    ...
    fn wsl_setup_never_contacts_the_distro() {
        let args: Vec<String> = vec![
            "--distribution".into(),
            "osiris-no-such-distro-exists".into(),
            "--cd".into(),
            "~".into(),
        ];
        ...
        assert_eq!(
            ...
            &[
                ...
                "osiris-no-such-distro-exists".to_string(),
                "--cd".to_string(),
                "~".to_string()
            ]
        );
    ...
    fn the_wsl_bootstrap_carries_integration_for_fish_not_just_bash() {
        ...
        assert!(script.contains(
            r#"*/bash) [ -r "${OSIRIS_RC:-}" ] && exec "$SHELL" --rcfile "$OSIRIS_RC" -i; exec "$SHELL" -l ;;"#
        ));
    ...
    fn the_wsl_bootstrap_carries_integration_for_zsh_too() {
        ...
        assert!(
            script
                .contains(r#"*/zsh) [ -n "${OSIRIS_ZDOTDIR:-}" ] && [ -r "$OSIRIS_ZDOTDIR/.zshrc" ] "#)
        );
        assert!(script.contains(r#"&& export ZDOTDIR="$OSIRIS_ZDOTDIR"; exec "$SHELL" -l ;;"#));
        ...
        assert!(
            !script.contains("OSIRIS_USER_ZDOTDIR"),
    ...
    fn wsl_integration_env_writes_what_each_shell_reads_and_names_it_for_the_distro() {
        let dir = throwaway_dir("osiris-wsltest-").expect("temp dir");
        let env = wsl_integration_env(&dir, None).expect("integration files");

        let rcfile = PathBuf::from(env.get(WSL_RCFILE_ENV).expect("OSIRIS_RC"));
        assert_eq!(std::fs::read_to_string(&rcfile).unwrap(), bash_rcfile());

        let zdotdir = PathBuf::from(env.get(WSL_ZDOTDIR_ENV).expect("OSIRIS_ZDOTDIR"));
        for (name, contents) in zsh_redirectors() {
            assert_eq!(
        ...
        assert_eq!(env.get("WSLENV").unwrap(), "OSIRIS_RC/p:OSIRIS_ZDOTDIR/p");
        assert!(!env.contains_key("OSIRIS_USER_ZDOTDIR"));
    ...
    fn the_wsl_bootstrap_picks_the_arm_that_matches_the_distro_s_shell() {
        ...
        let dir = throwaway_dir("osiris-wslarm-").expect("temp dir");
    ...
    fn wslenv_preserves_the_users_own_entries() {
        assert_eq!(
            wslenv_with(Some("MYVAR/p:OTHER"), &["OSIRIS_RC/p"]),
            "MYVAR/p:OTHER:OSIRIS_RC/p"
        );
        assert_eq!(wslenv_with(None, &["OSIRIS_RC/p"]), "OSIRIS_RC/p");
        assert_eq!(wslenv_with(Some(""), &["OSIRIS_RC/p"]), "OSIRIS_RC/p");
        assert_eq!(wslenv_with(Some("OSIRIS_RC/l"), &["OSIRIS_RC/p"]), "OSIRIS_RC/l");
        assert_eq!(
            wslenv_with(None, &["OSIRIS_RC/p", "OSIRIS_ZDOTDIR/p"]),
            "OSIRIS_RC/p:OSIRIS_ZDOTDIR/p"
        );
        assert_eq!(
            wslenv_with(Some("OSIRIS_ZDOTDIR/l"), &["OSIRIS_RC/p", "OSIRIS_ZDOTDIR/p"]),
            "OSIRIS_ZDOTDIR/l:OSIRIS_RC/p"
        );
    }
    ...
    fn bash_rcfile_path_uses_forward_slashes_on_windows() {
        let rendered = bash_path(Path::new(
            r"C:\Users\a\AppData\Local\Temp\osiris-bashrc-1-0\bashrc",
        ));
        if cfg!(windows) {
            assert_eq!(
                rendered,
                "C:/Users/a/AppData/Local/Temp/osiris-bashrc-1-0/bashrc"
            );
        }
        assert_eq!(
            bash_path(Path::new("/tmp/osiris-bashrc-1-0/bashrc")),
            "/tmp/osiris-bashrc-1-0/bashrc"
        );
    }
    ...
    fn zsh_redirectors_source_user_files_and_append_integration() {
        ...
        for (name, body) in &files {
            assert!(
                body.contains("$OSIRIS_USER_ZDOTDIR"),
    ...
    fn zshenv_recaptures_a_user_relocated_zdotdir() {
        ...
        let recapture = zshenv
            .find("export OSIRIS_USER_ZDOTDIR=${ZDOTDIR:-$HOME}")
            .unwrap();
        let restore = zshenv.rfind("ZDOTDIR=$__osiris_ztmp").unwrap();
        assert!(
        ...
        for (name, body) in &files[1..] {
            assert!(
                !body.contains("export OSIRIS_USER_ZDOTDIR"),
                "{name} must not re-export OSIRIS_USER_ZDOTDIR"
            );
        }
    }
    ...
    fn zsh_integration_restores_real_zdotdir_after_startup() {
        assert!(ZSH_INTEGRATION.contains("__osiris_restore_zdotdir"));
        assert!(ZSH_INTEGRATION.contains("ZDOTDIR=${OSIRIS_USER_ZDOTDIR:-$HOME}"));
        assert!(
            ZSH_INTEGRATION.contains("add-zsh-hook -d precmd __osiris_restore_zdotdir"),
    ...
    fn bashrc_sourcings(profile: Option<(&str, &str)>) -> Option<usize> {
        let home = throwaway_dir("osiris-rcfile-home-")?;
    ...
    fn a_profile_that_never_forwards_still_gets_bashrc_once() {
        for profile in [
            Some((".bash_profile", "export OSIRIS_PROFILE=1\n")),
            Some((".profile", "export OSIRIS_PROFILE=1\n")),
    ...
    fn every_integration_guards_install_on_empty_sentinel() {
        for (shell, body) in [
            ...
            assert!(
                body.contains(r#"-z "$OSIRIS_SHELL_INTEGRATION""#),
        ...
        assert!(
            !FISH_INTEGRATION.contains("set -q OSIRIS_SHELL_INTEGRATION"),
    ...
    fn d_emitter_is_prepended_ahead_of_user_precmd_hooks() {
        assert!(
            ZSH_INTEGRATION.contains("precmd_functions=(__osiris_precmd_d $precmd_functions)"),
            "zsh must prepend the D emitter (add-zsh-hook can only append)"
        );
        assert!(
            BASH_INTEGRATION
                .contains(r#"precmd_functions=(__osiris_precmd_d "${precmd_functions[@]}")"#),
    ...
    fn setup_fish_injects_startup_command_without_files() {
        ...
        assert!(inj.args[1].contains("__osiris"));
    ...
    fn setup_dispatches_by_shell_and_sets_sentinel() {
        ...
        assert_eq!(
            inj.env.get("OSIRIS_SHELL_INTEGRATION").map(String::as_str),
            Some("")
        );
        ...
        assert!(inj.env.contains_key("OSIRIS_SHELL_INTEGRATION"));
    ...
    fn powershell_integration_emits_every_osc_133_mark_and_cwd() {
        ...
        assert!(s.contains("if (-not $env:OSIRIS_SHELL_INTEGRATION)"));
        ...
        assert!(s.contains("$global:__OsirisOrigPrompt = $function:prompt"));
        assert!(s.contains("& $global:__OsirisOrigPrompt"));
    ...
    fn powershell_integration_sets_an_osc_title() {
        let s = POWERSHELL_INTEGRATION;
        assert!(s.contains("]0;$($global:__OsirisUser)@$($global:__OsirisHost):"));
    ...
    fn powershell_integration_reads_identity_platform_neutrally() {
        ...
        assert!(s.contains("$global:__OsirisHome = if ($HOME)"));
    }
    ...
    fn powershell_home_abbreviation_requires_a_path_separator() {
        let s = POWERSHELL_INTEGRATION;
        assert!(s.contains("$titlePath.StartsWith($global:__OsirisHome + '/')"));
        assert!(s.contains("$titlePath -eq $global:__OsirisHome"));
    }
    ...
    fn throwaway_dir_is_unique_per_call() {
        let a = throwaway_dir("osiris-test-").expect("dir a");
        let b = throwaway_dir("osiris-test-").expect("dir b");
        assert_ne!(a, b);
        assert!(
            a.file_name()
                .unwrap()
                .to_string_lossy()
                .starts_with("osiris-test-")
        );
    ...
    fn nushell_integration_emits_every_osc_133_mark_and_cwd() {
        ...
        assert!(s.contains("__osiris_cmd_active"));
        assert!(s.contains("hide-env __osiris_cmd_active"));
        assert!(s.contains("prompt_indicator?"));
        assert!(
            s.contains(r##"($env.OSIRIS_SHELL_INTEGRATION? | default "") == """##),
    ...
    /// platform convention there, which is *not* where osiris keeps its own
    /// config (`~/.config/osiris`). Reading `~/.config/nushell` on a Mac finds
    ...
    fn nushell_config_dir_is_what_nu_itself_reports() {
        ...
        let xdg_absolute = std::env::temp_dir().join("osiris-nu-xdg");
        ...
        for xdg in cases {
            ...
            assert_eq!(
                ours, theirs,
                "osiris and nu disagree on the config dir for XDG_CONFIG_HOME={xdg:?}"
            );
        }
    }

    /// End to end over a real pty: a user who *has* a config.nu must still
    /// have it after osiris points `nu --config` at the wrapper. `--config`
    ...
    fn nushell_restores_the_user_config_and_still_reports_the_prompt_cycle() {
        ...
        std::fs::write(
            &user_config,
            "$env.config = { show_banner: false }\n$env.OSIRIS_USER_CONFIG_LOADED = \"yes\"\n",
        )
        ...
        let text = prompt_cycle_over_pty(
            ...
            b"print $\"probe=($env.OSIRIS_USER_CONFIG_LOADED? | default 'MISSING')\"; 1 / 0\r",
            Some(home.path()),
        );
        ...
        // Assert on osiris's *own* marks, not on the substring `133;A`: stock
        // Nushell emits its own OSC 133 cycle alongside these, so a bare
        // substring passes even when the wrapper never ran. osiris's hooks
        ...
        assert!(
            ...
            "osiris's own OSC 7 must be among the reports; got:\n{text}"
        );
        assert_eq!(
            last_osc7(&text),
