...
pub enum AuthPromptKind {
    ...
    HostKeyUnknown {
        ...
        /// `osiris-server`, either of which may be an older build; an unknown
...
mod tests {
    ...
    /// prompt is decoded by whatever GUI or `osiris-server` is at the other end,
    ...
    /// crosses both daemon↔GUI and GUI↔remote `osiris-server`, so it has to
