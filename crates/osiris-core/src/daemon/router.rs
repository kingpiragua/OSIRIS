...
async fn drive(local: Stream, header: &RouteHeader) -> io::Result<()> {
    ...
    // and last until osiris itself restarts.
...
async fn restart_server(
    ...
    match (&header.target, action) {
        ...
        _ => Err(anyhow::anyhow!(
            "restarting osiris's server is only supported for machines it serves, not {}",
            header.describe()
        )),
    }
}
...
mod tests {
    ...
    async fn a_wsl_route_refuses_a_distro_name_that_could_be_an_option() {
        for header in [
            ...
            RouteHeader {
                ...
                server_command: Some("osiris-server --stdio".to_string()),
                channel: RouteChannel::Control,
                action: RouteAction::Forward,
            },
        ] {
    ...
    fn an_unopenable_link_fails_the_route() {
        ...
        let (client, daemon_side) = UnixStream::pair().unwrap();
        let header = RouteHeader::local_stdio("osiris-no-such-binary-anywhere", &[]);
        let routed = std::thread::spawn(move || RemoteRouter::route(daemon_side, &header));
    ...
    fn a_request() -> InstallRequest {
        InstallRequest {
            ...
            source_url: "https://example.invalid/osiris-server".into(),
            remote_path: "/home/me/.local/share/osiris/bin/osiris-server-26.7.5".into(),
