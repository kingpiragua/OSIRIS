...
mod imp_unix {
    ...
    pub(crate) fn socket_path_for(config_dir: &Path) -> PathBuf {
        ...
        let name = format!("osiris-{hash:016x}.sock");
...
mod tests {
    ...
    fn socket_path_stays_in_config_dir_when_it_fits() {
        let dir = std::path::PathBuf::from("/tmp/osiris-short");
    ...
    fn a_long_runtime_dir_falls_through_to_the_temp_dir() {
        let name = "osiris-0123456789abcdef.sock";
...
mod imp_windows {
    ...
    mod tests {
        ...
        fn a_second_endpoint_gets_its_own_port_and_token() {
            ...
            let dir = std::env::temp_dir().join(format!("osiris-wintok-{}", std::process::id()));
            std::fs::create_dir_all(&dir).ok();
            config::set_config_dir(dir);
