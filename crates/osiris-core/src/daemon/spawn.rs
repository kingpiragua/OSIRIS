...
const DAEMON_EXE_STEMS: [&str; 3] = ["osiris", "osiris-server", "oexe"];
...
pub fn ensure_running() -> anyhow::Result<()> {
    ...
    // post-install "Launch osiris" click — where the launch deserves its
    // daemon, not an error.
    #[cfg(windows)]
    {
        ...
        while crate::daemon::update_guard::held() {
            if Instant::now() >= deadline {
                anyhow::bail!(
                    "an osiris update is being installed right now; the daemon will return \
...
/// level: that is where the locked images (osiris.exe, OpenConsole.exe,
...
fn spawn_detached() -> anyhow::Result<()> {
    ...
    if windows::redirection_trust_enforced() {
        ...
        match windows::spawn_detached_with_clean_parent(&exe, &args) {
            ...
            // instead of refusing to start osiris at all.
            Err(error) => log::warn!(
...
///
/// Public because `osiris-cli` launches the headless `osiris-server` the same way,
...
mod windows_spawn_tests {
    ...
    const INNER_ENV: &str = "OSIRIS_REDIRECTION_TRUST_INNER";
    const CLEAN_PARENT_ENV: &str = "OSIRIS_REDIRECTION_TRUST_CLEAN_PARENT";
    const PROBE_RESULT_ENV: &str = "OSIRIS_REDIRECTION_TRUST_PROBE_RESULT";
    const TEST_NAME: &str =
    ...
    fn daemon_spawn_does_not_inherit_redirection_trust() {
        ...
        let root = std::env::temp_dir().join(format!(
            "osiris-redirection-trust-{}-{unique}",
            std::process::id()
        ));
        ...
        assert!(
            ...
            "osiris must detect the enforcing policy before selecting the alternate spawn path"
        );
        ...
        assert_eq!(
            verdict.trim(),
            "CLEAN_OK",
            "an osiris daemon child must drop the inherited policy and retain access to user-created junctions"
        );
    }
}
...
mod exe_name_tests {
    ...
    fn windows_matches_daemon_names_case_insensitively() {
        assert!(is_reapable_daemon_name("OSIRIS-APP.EXE"));
        assert!(is_reapable_daemon_name("Osiris-Server"));
        assert!(is_reapable_daemon_name("OSIRIS"));
    }

    #[test]
    fn strip_exe_suffix_only_strips_a_trailing_exe() {
        assert_eq!(strip_exe_suffix("osiris.exe"), "osiris");
        assert_eq!(strip_exe_suffix("osiris.EXE"), "osiris");
        assert_eq!(strip_exe_suffix("osiris"), "osiris");
...
mod tests {
    ...
    fn connect_to_stale_socket_path_fails() {
        let dir = std::env::temp_dir().join(format!("osiris-spawn-test-{}", std::process::id()));
