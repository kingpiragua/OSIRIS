...
/// server's filename (`osiris-server-c{control}p{protocol}`), so moving it also
...
pub enum ControlRequest {
    ...
    GuiOpen {
        ...
        /// what `osiris [PATH]` wants, and what a workspace the CLI just made
...
impl ControlClient {
        ...
        W: Write + Send + 'static,
    {
        ...
        let reader = std::thread::Builder::new()
            .name("osiris-control-reader".into())
    ...
    pub fn call_with_deadline(
        ...
        log::debug!(target: "osiris::control", "#{req_id} {req:?}");
...
mod tests {
    ...
    fn other_connect_failures_are_not_dialect_refusals() {
        for other in [
            ...
            "the remote osiris-server did not start: exit status 127",
