...
pub const ASSET_LINUX_X86_64: &str = "osiris-server-linux-x86_64-musl";
pub const ASSET_LINUX_AARCH64: &str = "osiris-server-linux-aarch64-musl";
...
pub const ASSET_MACOS_X86_64: &str = "osiris-server-macos-x86_64";
pub const ASSET_MACOS_AARCH64: &str = "osiris-server-macos-aarch64";

pub const CHECKSUMS_ASSET: &str = "checksums.txt";

pub const RELEASE_BASE: &str = "https://github.com/kingpiragua/osiris/releases/download";

pub const INSTALL_DIR_COMPONENTS: [&str; 4] = [".local", "share", "osiris", "bin"];
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::UnsupportedSystem { raw } => write!(
                f,
                "a remote osiris workspace needs a Linux or macOS host; this machine reports \
                 `uname -sm` = {raw:?}"
            ),
            Self::UnknownMachine { raw } => write!(
                f,
                "no osiris-server is published for this architecture (`uname -sm` = {raw:?}); \
...
pub fn binary_name(control: u32, protocol: u32) -> String {
    format!("osiris-server-c{control}p{protocol}")
}
...
pub fn dialect_from_path(path: &str) -> Option<(u32, u32)> {
    ...
    let (control, protocol) = name.strip_prefix("osiris-server-c")?.split_once('p')?;
...
mod tests {
    ...
    fn download_urls_point_at_the_release_the_tag_names() {
        assert_eq!(
            ...
            "https://github.com/kingpiragua/osiris/releases/download/v26.7.5/osiris-server-linux-x86_64-musl"
        );
        assert_eq!(
            download_url(&release_tag("26.7.6-nightly.20260727"), CHECKSUMS_ASSET),
            "https://github.com/kingpiragua/osiris/releases/download/nightly/checksums.txt"
        );
    }

    #[test]
    fn asset_names_are_the_ones_the_release_workflow_publishes() {
        assert_eq!(ASSET_LINUX_X86_64, "osiris-server-linux-x86_64-musl");
        assert_eq!(ASSET_LINUX_AARCH64, "osiris-server-linux-aarch64-musl");
        assert_eq!(ASSET_MACOS_X86_64, "osiris-server-macos-x86_64");
        assert_eq!(ASSET_MACOS_AARCH64, "osiris-server-macos-aarch64");
    ...
    fn remote_paths_are_posix_and_named_by_dialect() {
        ...
        assert_eq!(p.bin_dir, "/home/me/.local/share/osiris/bin");
        assert_eq!(p.binary, "/home/me/.local/share/osiris/bin/osiris-server-c3p4");
        assert_eq!(
            p.temp,
            "/home/me/.local/share/osiris/bin/.osiris-server-c3p4.tmp"
        );
        assert_eq!(
            p.dir_chain,
            vec![
                "/home/me/.local",
                "/home/me/.local/share",
                "/home/me/.local/share/osiris",
                "/home/me/.local/share/osiris/bin",
            ]
        );
        assert!(
    ...
    fn trailing_slash_on_home_is_absorbed() {
        assert_eq!(
            ...
            "/root/.local/share/osiris/bin/osiris-server-c1p1"
        );
        assert_eq!(remote_paths("/", 1, 1).bin_dir, "/.local/share/osiris/bin");
    }

    #[test]
    fn dialects_are_recoverable_from_an_install_path() {
        assert_eq!(
            dialect_from_path("/home/me/.local/share/osiris/bin/osiris-server-c3p4"),
            Some((3, 4))
        );
        assert_eq!(dialect_from_path("osiris-server-c12p30"), Some((12, 30)));
        assert_eq!(dialect_from_path("/usr/bin/osiris-server"), None);
        assert_eq!(dialect_from_path("/bin/bash"), None);
        assert_eq!(dialect_from_path("osiris-server-c3"), None);
        assert_eq!(dialect_from_path("osiris-server-cxpy"), None);
    }

    #[test]
    fn legacy_version_named_binaries_carry_no_dialect() {
        for legacy in [
            "/home/me/.local/share/osiris/bin/osiris-server-26.7.4",
            "osiris-server-26.7.6-nightly.20260727",
            "osiris-server-0.1.0",
            "/usr/local/bin/osiris-server-",
        ] {
