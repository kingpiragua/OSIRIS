...
const BIN_DIR: &str = "/home/me/.local/share/osiris/bin";
const BINARY: &str = "/home/me/.local/share/osiris/bin/osiris-server-c3p4";
const TEMP_BASE: &str = "/home/me/.local/share/osiris/bin/.osiris-server-c3p4.tmp";

fn temp() -> String {
    unique_temp(TEMP_BASE)
}

const SERVER_BYTES: &[u8] = b"\x7fELF...a static musl osiris-server, pretend it is 6 MB";
...
impl FakeRemote {
    ...
    fn with_legacy_install(self, version: &str) -> (Self, String) {
        let path = format!("{BIN_DIR}/osiris-server-{version}");
        ...
        if let Some(exe) = cmd.strip_suffix(&format!(" {PROTOCOL_FLAG}")) {
            ...
            return match self.speaks.lock().unwrap().get(exe) {
                ...
                None => Ok(ExecOutput {
                    status: Some(1),
                    stdout: String::new(),
                    stderr: "osiris-server: nothing to do without --daemon or --stdio".into(),
                }),
            };
        }
...
fn first_install_runs_all_six_steps() {
    ...
    assert_eq!(
        release.fetched(),
        vec![
            format!("https://github.com/kingpiragua/osiris/releases/download/v{VERSION}/checksums.txt"),
            format!(
                "https://github.com/kingpiragua/osiris/releases/download/v{VERSION}/{ASSET_LINUX_X86_64}"
            ),
        ]
    );
}
...
fn restart_replaces_the_running_daemon() {
    let remote = FakeRemote::new()
        ...
        .serving(&format!("{BIN_DIR}/osiris-server-26.7.4"));
    remote.preinstall(BINARY, 0o755);
    let release = FakeRelease::new();
    let user = FakeUser::declining();
...
fn a_restart_with_nothing_to_start_leaves_the_running_daemon_alone() {
    ...
    let old = format!("{BIN_DIR}/osiris-server-c2p3");
...
fn the_launch_command_detaches_and_closes_every_stream() {
    let cmd = launch_command("/home/me/.local/share/osiris/bin/osiris-server-26.7.5");
...
fn remote_paths_are_shell_quoted() {
    ...
    assert_eq!(
        shell_quote("/home/my box/osiris-server"),
        "'/home/my box/osiris-server'"
    );
...
fn the_launch_command_quotes_its_binary() {
    let cmd = launch_command("/home/me/a b/osiris-server-1.0.0");
    assert!(cmd.contains("'/home/me/a b/osiris-server-1.0.0'"), "{cmd}");
}
...
fn the_running_exe_probe_cannot_fail_the_command() {
    ...
    assert!(TERMINATE_RUNNING_COMMAND.contains("*/osiris-server-*"));
}
...
fn a_bundle_is_used_instead_of_downloading() {
    let dir = std::env::temp_dir().join(format!("osiris-bundle-src-{}", std::process::id()));
...
fn a_bundle_that_lacks_the_asset_does_not_fall_back_to_the_network() {
    let dir = std::env::temp_dir().join(format!("osiris-bundle-empty-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn discover_falls_back_to_release_when_bundled_is_missing() {
    let dir = std::env::temp_dir().join(format!("osiris-bundle-discover-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
fn discover_uses_bundled_when_it_is_present() {
    let dir = std::env::temp_dir().join(format!(
        "osiris-bundle-discover-present-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
...
const OTHER_EXE: &str = "/home/me/.local/share/osiris/bin/osiris-server-26.7.9-nightly.20260801";
