...
const MARKER: &str = "__osiris_wsl__";
...
const HOME_SCRIPT: &str = "printf '__osiris_wsl__ home=%s\\n' \"$HOME\"\n";
...
fn settle_script(binary: &str, tries: u32, step: &str, grace: &str) -> String {
    ...
    format!(
        "__osiris_settle=0\n\
         while [ \"$__osiris_settle\" -lt {tries} ]; do\n\
         if {bin} --stdio --bridge < /dev/null > /dev/null 2>&1; then break; fi\n\
         __osiris_settle=$((__osiris_settle + 1))\n\
         sleep {step}\n\
         done\n\
         sleep {grace}\n"
    )
...
pub const BUNDLED_DIR_ENV: &str = "OSIRIS_BUNDLED_SERVER_DIR";
...
///
/// - The binary does not move. An osiris upgrade renames it, but a new build is a
...
///   stopped distro on demand, and the bridge (`osiris-server --stdio --pane`)
...
pub fn ensure_wsl_server(distro: &str) -> io::Result<String> {
    ...
    log::info!(
        "{host}: {} at {} ({}{})",
        if report.installed {
            "installed the bundled osiris-server"
        } else {
            "osiris-server already present"
        },
        report.paths.binary,
        if report.launched {
            "daemon launched"
...
mod tests {
    ...
    fn the_transport_command_line_is_the_designs() {
        assert_eq!(
            wsl_args("Ubuntu", &["osiris-server", "--stdio"]),
            vec!["-d", "Ubuntu", "--exec", "osiris-server", "--stdio"]
        );
        assert_eq!(
            wsl_args(
                "Ubuntu-22.04",
                &[
                    "/home/me/.local/share/osiris/bin/osiris-server-26.7.5",
                    "--stdio"
                ]
            ),
            vec![
    ...
    fn no_transport_command_is_re_parsed_by_the_distro_s_default_shell() {
        ...
        for argv in [
            vec!["osiris-server", "--stdio"],
            vec!["osiris-server", "--stdio", "--pane"],
            vec!["sh", "-s"],
            vec!["tee", "/home/me/.local/share/osiris/bin/osiris-server"],
    ...
    fn sh_scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-wsl-sh-{name}-{}", std::process::id()));
    ...
    fn the_settle_asks_the_control_dialect_with_the_quoted_binary() {
        let script = settle_script("/home/a b/osiris-server-26.7.6", 4, "0.2", "0.3");
        assert!(
            script.contains("'/home/a b/osiris-server-26.7.6' --stdio --bridge < /dev/null"),
            "{script}"
        );
    ...
    fn the_settle_stops_as_soon_as_the_daemon_answers() {
        ...
        let serving = fake_server(&dir, "osiris-server-answers", 0);
    ...
    fn the_settle_gives_up_rather_than_hanging() {
        ...
        let silent = fake_server(&dir, "osiris-server-silent", 1);
    ...
    fn the_list_script_really_runs_and_answers_correctly() {
        ...
        std::fs::write(dir.join("osiris-server-26.7.5"), b"x").unwrap();
        std::fs::write(dir.join(".osiris-server-26.7.5.tmp"), b"x").unwrap();
        ...
        assert_eq!(
            got,
            vec![".osiris-server-26.7.5.tmp", "a b c", "osiris-server-26.7.5"],
    ...
    fn awkward_paths_survive_into_a_running_shell() {
        ...
        std::fs::create_dir_all(&dir).unwrap();
        let file = dir.join("osiris-server-1.0.0");
        ...
        assert_eq!(
            ...
            Some(vec!["osiris-server-1.0.0".to_string()])
        );
    ...
    fn a_mixed_encoding_pipe_keeps_both_halves_readable() {
        ...
        let mut mixed = Vec::from(&b"__osiris_wsl__ home=/home/me\n"[..]);
    ...
    fn the_mutating_scripts_really_run() {
        ...
        let from = nested.join(".osiris-server-1.0.0.tmp");
        let to = nested.join("osiris-server-1.0.0");
    ...
    fn script_paths_are_shell_quoted() {
        let script = stat_script("/home/o'brien/my dir/osiris-server-1.0.0");
        assert!(
            script.contains(r"p='/home/o'\''brien/my dir/osiris-server-1.0.0'"),
            "{script}"
        );
    ...
    fn the_stat_probe_round_trips_its_three_answers() {
        assert_eq!(parse_stat("__osiris_wsl__ stat=none 0 0\n").unwrap(), None);

        let dir = parse_stat("__osiris_wsl__ stat=dir 0 0\n").unwrap().unwrap();
        assert!(dir.is_dir);

        let exe = parse_stat("__osiris_wsl__ stat=file 755 6291456\n")
            .unwrap()
            .unwrap();
        assert!(!exe.is_dir);
        ...
        let half = parse_stat("__osiris_wsl__ stat=file 644 6291456\n")
            .unwrap()
            .unwrap();
    ...
    fn the_stat_probe_ignores_noise_but_not_silence() {
        let noisy = "Welcome to Ubuntu 22.04\n__osiris_wsl__ stat=file 755 10\nlast login: …\n";
        assert_eq!(parse_stat(noisy).unwrap().unwrap().size, 10);
        assert!(parse_stat("").is_err());
        assert!(parse_stat("bash: sh: command not found\n").is_err());
        assert!(parse_stat("__osiris_wsl__ stat=weird 0 0\n").is_err());
    }

    #[test]
    fn a_listing_distinguishes_empty_from_absent() {
        assert_eq!(parse_list("__osiris_wsl__ nolisting\n").unwrap(), None);
        assert_eq!(
            parse_list("__osiris_wsl__ listing\n").unwrap(),
            Some(Vec::new())
        );
        assert_eq!(
            parse_list(
                "__osiris_wsl__ listing\n\
                 __osiris_wsl__ entry=osiris-server-26.7.5\n\
                 __osiris_wsl__ entry=.osiris-server-26.7.5.tmp\n"
            )
            .unwrap(),
            Some(vec![
                "osiris-server-26.7.5".to_string(),
                ".osiris-server-26.7.5.tmp".to_string()
            ])
        );
        assert_eq!(
            parse_list("__osiris_wsl__ listing\n__osiris_wsl__ entry=a b c\n").unwrap(),
    ...
    fn the_search_order_is_specific_first_and_deduplicated() {
        let exe = PathBuf::from("/opt/osiris/bin/osiris.exe");
        let dirs = bundled_search_dirs(Some(&exe), None);
        assert_eq!(dirs[0], PathBuf::from("/opt/osiris/bin/server"));
        assert_eq!(dirs[1], PathBuf::from("/opt/osiris/bin"));
        assert!(dirs.contains(&PathBuf::from("/opt/osiris/Resources/server")));
        ...
        let same = PathBuf::from("/opt/osiris/bin/server");
    ...
    fn a_missing_bundled_binary_names_every_place_it_looked() {
        let tmp = std::env::temp_dir().join(format!("osiris-wsl-src-{}", std::process::id()));
        ...
        assert!(msg.contains("osiris-server-linux-x86_64-musl"), "{msg}");
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-wsl-{name}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        dir
    }
    ...
    fn a_wsl_install_copies_the_bundled_binary_and_launches_a_daemon() {
        ...
        assert_eq!(
            report.paths.binary,
            format!(
                "/home/me/.local/share/osiris/bin/osiris-server-c{}p{}",
    ...
    fn nowhere(test: &str) -> String {
        format!("osiris-no-such-distro-{test}")
    }
    ...
    fn a_remembered_distro_is_answered_without_asking_wsl_anything() {
        ...
        let binary = "/home/me/.local/share/osiris/bin/osiris-server-c5p5";
    ...
    fn forgetting_sends_the_next_caller_back_to_the_distribution() {
        ...
        remember(&distro, note("/somewhere/osiris-server"));
    ...
    fn what_is_remembered_is_per_distro_and_replaceable() {
        ...
        remember(&a, note("/a/osiris-server"));
        remember(&b, note("/b/osiris-server"));
        assert_eq!(remembered_wsl_server(&a).as_deref(), Some("/a/osiris-server"));

        forget_wsl_server(&a);
        assert_eq!(remembered_wsl_server(&a), None);
        assert_eq!(
            remembered_wsl_server(&b).as_deref(),
            Some("/b/osiris-server"),
            "forgetting one distro must not forget another"
        );

        remember(&b, note("/b/osiris-server-newer"));
        assert_eq!(
            remembered_wsl_server(&b).as_deref(),
            Some("/b/osiris-server-newer"),
    ...
    fn a_restart_forgets_first_so_a_failed_one_leaves_no_stale_note() {
        ...
        remember(&distro, note("/x/osiris-server"));
    ...
    fn a_remembered_answer_waits_for_an_install_to_let_go_of_the_binary() {
        ...
        assert_eq!(answered, "/x/osiris-server");
        let _ = asking.join();
    ...
    fn a_remembered_mismatch_is_told_to_every_later_pane() {
        ...
        let entry = crate::daemon::install::MismatchedRemoteDaemon {
            ...
            running_exe: Some("/x/osiris-server-someone-elses".to_string()),
            wanted_version: "9.9.9".to_string(),
        };
        remember(
            &distro,
            Proved {
                binary: "/x/osiris-server".to_string(),
