    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingBundled { asset, searched } => write!(
                f,
                "this build of osiris does not ship a Linux server binary, so it cannot \
            ...
            Self::Declined { host, path } => write!(
                f,
                "installing osiris-server at {path} on {host} was not confirmed; nothing was written"
            ),
            Self::Write { path, reason } => {
                write!(f, "could not write {path} on the remote machine: {reason}")
            }
            Self::Launch { reason } => write!(f, "the remote osiris-server did not start: {reason}"),
            Self::NoServerToRestart { host, path } => write!(
                f,
                "{host} has no osiris-server at {path} for this build to start, so nothing was \
            ...
            Self::DialectMismatch {
                ...
                write!(
                    f,
                    "this build needs an osiris-server speaking control v{} and protocol v{}, \
...
impl<'a> Installer<'a> {
    ...
    fn is_first_install(&self, paths: &RemotePaths) -> bool {
        match self.ops.list_dir(&paths.bin_dir) {
            Ok(Some(entries)) => !entries.iter().any(|name| name.starts_with("osiris-server-")),
            Ok(None) => true,
            Err(_) => true,
        }
    }
    ...
    /// Stop whatever osiris-server is running over there and start the one this
    /// build speaks to.
    ///
    /// Which is not the same binary: the path is named after *our* dialect
    /// (`osiris-server-c{control}p{protocol}`), while the kill matches any
    /// `osiris-server-*` — see [`TERMINATE_RUNNING_COMMAND`]. When
...
/// `osiris-server-c7p5` — which is why the fallback stays a fallback and `/proc`
/// keeps first refusal.
///
...
const RUNNING_EXE_COMMAND: &str = r#"if [ -d /proc ]; then for p in /proc/[0-9]*; do e=$(readlink "$p/exe" 2>/dev/null) || continue; case "$e" in */osiris-server-*) printf '%s' "${e% (deleted)}"; break;; esac; done; else ps -xwwo pid=,comm= 2>/dev/null | while read -r pid e; do case "$e" in */osiris-server-*) printf '%s' "$e"; break;; esac; done; fi; true"#;

const TERMINATE_RUNNING_COMMAND: &str = r#"if [ -d /proc ]; then for p in /proc/[0-9]*; do e=$(readlink "$p/exe" 2>/dev/null) || continue; case "$e" in */osiris-server-*) kill -TERM "${p#/proc/}" 2>/dev/null; break;; esac; done; else ps -xwwo pid=,comm= 2>/dev/null | while read -r pid e; do case "$e" in */osiris-server-*) kill -TERM "$pid" 2>/dev/null; break;; esac; done; fi; true"#;
