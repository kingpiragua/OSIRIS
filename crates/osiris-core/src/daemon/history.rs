...
//! like, long after the pane's environment was decided. osiris's integration
...
//!
//! A shell with no osiris integration is simply not covered: nothing sets the
...
pub const FILE_ENV: &str = "OSIRIS_HISTFILE";

/// Shells whose integration snippet knows what to do with [`FILE_ENV`].
///
/// The list is exactly the shells that get a `HISTFILE` *and* an osiris snippet to
...
mod tests {
    ...
    fn a_global_history(name: &str, body: &str) -> PathBuf {
        let path = std::env::temp_dir().join(format!("osiris-history-{}-{name}", std::process::id()));
    ...
    fn the_sweep_keeps_only_the_panes_that_still_exist() {
        ...
        let dir = std::env::temp_dir().join(format!("osiris-sweeptest-{}", std::process::id()));
