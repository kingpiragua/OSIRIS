...
struct ChosenShell {
    program: String,
    args: Vec<String>,
    args_are_osiris_defaults: bool,
}

fn has_custom_args(chosen: Option<&ChosenShell>) -> bool {
    chosen.is_some_and(|c| !c.args.is_empty() && !c.args_are_osiris_defaults)
}
...
fn choose_shell(
    ...
    spawn_override
        .map(|s| ChosenShell {
            program: s.program,
            args: s.args,
            args_are_osiris_defaults: s.args_are_osiris_defaults,
        })
        .or_else(|| {
            configured.map(|(program, args)| ChosenShell {
                program,
                args,
                args_are_osiris_defaults: false,
            })
        })
}
...
fn build_spawn_config(
    ...
    let shell = configured.as_ref().map(|c| ShellSpec {
        ...
        args: c.args.clone(),
        args_are_osiris_defaults: c.args_are_osiris_defaults,
    });
...
///
/// `resolved_program` is what osiris *decided* to run before shell integration
...
fn initial_working_directory(cwd: Option<PathBuf>) -> Option<PathBuf> {
    ...
    // mistyped path reads as an osiris bug (#601). Settings refuses to save one
...
const TERM_PROGRAM_NAME: &str = "osiris";
...
const OSIRIS_CONFIG_DIR_ENV: &str = "OSIRIS_CONFIG_DIR";
const OSIRIS_PANE_ENV: &str = "OSIRIS_PANE";
const OSIRIS_WS_ENV: &str = "OSIRIS_WS";
...
/// shell osiris was told to run *is* the user's shell, so that is what we name.
///
...
fn pane_environment(
    ...
    if let Some(ws) = workspace {
        env.push((OSIRIS_WS_ENV.to_string(), ws.to_string()));
    }
    if let Some(dir) = config_dir_env() {
        env.push((OSIRIS_CONFIG_DIR_ENV.to_string(), dir));
    }
...
fn drain_then_report(
    ...
    std::thread::Builder::new()
        .name("osiris-daemon-pane-pty-close".to_string())
...
impl DaemonPane {
    ...
    fn spawn_reader(
        ...
        std::thread::Builder::new()
            .name("osiris-daemon-pane-reader".to_string())
            .spawn(move || {
    ...
    fn spawn_exit_monitor(
        ...
        if handle.is_null() {
            ...
            std::thread::Builder::new()
                .name("osiris-daemon-pane-exit-fallback".to_string())
                .spawn(move || {
        ...
        std::thread::Builder::new()
            .name("osiris-daemon-pane-exit-monitor".to_string())
            .spawn(move || {
...
fn join_bounded(handle: JoinHandle<()>, timeout: Duration) -> bool {
    ...
    if std::thread::Builder::new()
        .name("osiris-daemon-pane-join".to_string())
        .spawn(move || {
            let _ = handle.join();
            let _ = tx.send(());
        })
...
mod tests {
    ...
    fn initial_working_directory_skips_paths_that_are_not_directories() {
        ...
        let file = real.join("osiris-iwd-probe");
    ...
    fn only_user_authored_args_block_shell_integration() {
        let chosen = |args: Vec<&str>, osiris: bool| ChosenShell {
            program: r"C:\Program Files\Git\bin\bash.exe".to_string(),
            args: args.into_iter().map(str::to_string).collect(),
            args_are_osiris_defaults: osiris,
        };
    ...
    fn env_only_integration_keeps_default_login_shell_builder() {
        ...
        env.insert("ZDOTDIR".to_string(), "/tmp/osiris-zdotdir-test".to_string());
        let injection = shell_integration::Injection {
            env,
            args: Vec::new(),
            replaces_argv: false,
    ...
    fn at_prompt_covers_prompt_draw_gap_across_chunks() {
        ...
        let chunk = s.feed(
            b"\x1b]133;A\x07\x1b]7;file://host/repo/osiris\x07\r\nosiris git:(main) \xe2\x9e\x9c ",
        );
        assert!(
    ...
    fn sentinel_events_drive_agent_session_state() {
        ...
        let mut sniffer = OscSniffer::new();
        let stream = concat!(
            "\x1b]777;notify;osiris://cli-agent;",
    ...
    fn probed_cwd_corrects_a_pane_whose_shell_never_reports_osc7() {
        ...
        apply_probed_cwd(&mut st, Some(PathBuf::from("/Users/alice/dev/osiris")));

        assert_eq!(st.cwd.as_deref(), Some(Path::new("/Users/alice/dev/osiris")));
        assert!(
            matches!(rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/dev/osiris"))
        );
    }
    ...
    fn probed_cwd_keeps_the_shells_spelling_for_a_symlinked_path() {
        let tmp = std::env::temp_dir().join(format!("osiris-cwd-{}", std::process::id()));
    ...
    fn probed_cwd_matching_the_reported_one_says_nothing() {
        let mut st = test_state(true);
        st.cwd = Some(PathBuf::from("/Users/alice/dev/osiris"));
        let (tx, rx) = mpsc::channel();
        st.subscriber = Some(tx);
    ...
    fn attach_replays_initial_cwd_even_before_shell_reports_osc7() {
        let mut st = test_state(true);
        st.cwd = Some(PathBuf::from("/Users/alice/clone/osiris"));
        ...
        assert!(
            matches!(rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/clone/osiris"))
        );
    }
    ...
    fn reader_poll_applies_the_probed_cwd() {
        ...
        let handle = DaemonPane::spawn_reader(
            ...
            ForegroundProbes {
                remote: Box::new(|| None),
                agent: Box::new(|| None),
                cwd: Box::new(|| Some(PathBuf::from("/Users/alice/dev/osiris"))),
            },
            Arc::new(DeathReporter::new(|| {})),
        );
        handle.join().unwrap();
        ...
        assert_eq!(
            ...
            Some(Path::new("/Users/alice/dev/osiris"))
        );
        assert!(matches!(sub_rx.try_recv(), Ok(DaemonMsg::Output(_))));
        assert!(
            matches!(sub_rx.try_recv(), Ok(DaemonMsg::Cwd(p)) if p == PathBuf::from("/Users/alice/dev/osiris"))
        );
    }
    ...
    fn pane_environment_advertises_the_terminal_under_the_standard_names() {
        ...
        assert_eq!(env.get("TERM_PROGRAM").map(String::as_str), Some("osiris"));
        assert_eq!(
            env.get("TERM_PROGRAM_VERSION").map(String::as_str),
            Some(version)
        );
        assert_eq!(
            env.get(crate::core::agent_hooks::OSIRIS_ENV_MARKER)
                .map(String::as_str),
            Some(version)
        );
        assert_eq!(
    ...
    fn pane_environment_hands_the_shell_its_own_address() {
        ...
        match config_dir_env() {
            ...
            None => assert!(
                !env.contains_key(OSIRIS_CONFIG_DIR_ENV),
                "no resolvable config dir must not inject an empty OSIRIS_CONFIG_DIR"
            ),
        }
        ...
        .collect();
        assert!(
            !unfiled.contains_key(OSIRIS_WS_ENV),
    ...
    #[cfg(unix)]
    #[test]
    fn a_configured_shell_variable_outranks_the_shell_osiris_launched() {
        // Same precedence contract as TERM_PROGRAM: osiris describes the pane,
    ...
    fn the_shell_we_announce_is_the_argv_we_are_about_to_exec() {
        ...
        assert_eq!(
            ...
            "/opt/homebrew/bin/fish",
            "a builder with an argv has already overruled whatever osiris resolved"
        );
    ...
    #[test]
    fn spawned_shell_carries_the_osiris_marker() {
        let cmd = build_shell_command(None, &Some(PathBuf::from("/tmp")), 42, Some("ws-main"))
            .expect("build default shell command")
            .0;
        let osiris = cmd
            .get_env(crate::core::agent_hooks::OSIRIS_ENV_MARKER)
            .and_then(|v| v.to_str());
        assert_eq!(
            osiris,
            Some(env!("CARGO_PKG_VERSION")),
            "the daemon must inject OSIRIS into every spawned shell"
        );
        assert_eq!(
            cmd.get_env(OSIRIS_PANE_ENV).and_then(|v| v.to_str()),
            Some("42"),
            "the daemon must tell every spawned shell which pane it is"
        );
        assert_eq!(
            cmd.get_env(OSIRIS_WS_ENV).and_then(|v| v.to_str()),
            Some("ws-main"),
            "the daemon must tell every spawned shell which workspace filed it"
        );
        if let Some(dir) = config_dir_env() {
            assert_eq!(
                cmd.get_env(OSIRIS_CONFIG_DIR_ENV).and_then(|v| v.to_str()),
                Some(dir.as_str()),
    ...
    fn the_two_endpoints_are_siblings_derived_from_one_config_dir() {
        ...
        // and sit in ~/.local/share/osiris as `daemon.sock`, the same basename the
        ...
        let dir = std::path::Path::new("/tmp/osiris-endpoint-test");
