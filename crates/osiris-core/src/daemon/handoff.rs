...
fn anonymous_file() -> std::io::Result<std::fs::File> {
    ...
    for attempt in 0..8 {
        let path = dir.join(format!("osiris-handoff-{}-{attempt}", std::process::id()));
...
mod tests {
    ...
    fn carried(id: u64, fd: RawFd, output: &[u8]) -> Carried {
        Carried {
            ...
            master_fd: fd,
            child_pid: 4242,
            integration_dir: Some(PathBuf::from("/tmp/osiris-int")),
            size: size(),
    ...
    fn the_staged_file_has_no_name_to_read_it_by() {
        ...
        let leaked: Vec<_> = std::fs::read_dir(&dir)
            ...
            .flatten()
            .filter(|e| e.file_name().to_string_lossy().starts_with("osiris-handoff-"))
            .collect();
        assert!(
            leaked.is_empty(),
    ...
    fn a_failed_exec_puts_close_on_exec_back_on_every_descriptor() {
        ...
        let err = take_over(
            Path::new("/nonexistent/osiris-that-cannot-exec"),
