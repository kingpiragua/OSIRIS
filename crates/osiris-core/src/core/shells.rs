...
pub struct DetectedShell {
    ...
    pub program: String,
    pub args: Vec<String>,
    /// Marks arguments supplied by osiris's shell detection rather than by the user.
    /// Older peers omit this field, and their inventory arguments were all treated
    /// as osiris defaults, so `true` preserves the previous protocol behavior.
    #[serde(default = "default_true")]
    pub args_are_osiris_defaults: bool,
    /// Marks a row the user wrote into `custom_shells` rather than one osiris
...
fn append_custom(inventory: &mut ShellInventory, custom: &[crate::core::config::CustomShell]) {
    for (index, entry) in custom.iter().enumerate() {
        ...
        inventory.shells.push(DetectedShell {
            ...
            // osiris chose none of this, so none of it is an osiris default: the
...
fn inventory_from(
        ...
        .position(|shell| same_shell_program(&shell.program, default_program))
    {
        ...
        if let Some((program, args)) = configured.as_ref() {
            ...
            shells[default_index].args_are_osiris_defaults = false;
        }
...
/// osiris anything, so we take them all; skipping them here would hide the most
...
mod tests {
    ...
    fn a_unique_configured_shell_is_added_first_with_its_args() {
        ...
        assert!(!inventory.shells[0].args_are_osiris_defaults);
    }
    ...
    #[test]
    fn inventories_from_older_peers_treat_arguments_as_osiris_defaults() {
    ...
    fn a_custom_entry_joins_the_menu_with_its_arguments_intact() {
        ...
        assert!(
            !added.args_are_osiris_defaults,
            "osiris contributed none of this command, so none of it may be replaced as a default"
        );
        assert_eq!(
