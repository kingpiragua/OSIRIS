...
mod tests {
    ...
    fn untracked_and_ignored_records_parse_without_fields() {
        ...
        assert_eq!(
            by_path(&parsed, "target/debug/osiris").kind,
            EntryKind::Ignored
        );
    }
    ...
    fn scratch(name: &str) -> Option<Scratch> {
        ...
        let dir =
            std::env::temp_dir().join(format!("osiris-scm-status-{name}-{}", std::process::id()));
    ...
    fn outside_a_repository_there_is_no_status() {
        ...
        assert_eq!(
            probe_status(&*host, Path::new("/no/such/osiris/path")),
            StatusProbe::Unreachable,
