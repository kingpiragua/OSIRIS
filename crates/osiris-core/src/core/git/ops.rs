...
pub struct GitOpError {
    ...
    /// notification offers to re-run this in a pane — osiris is a terminal, and a
...
impl GitOp {
    ...
    /// in over a decade. osiris's whole point is that a remote host behaves like
    ...
    impl TempRepo {
        fn new(tag: &str) -> TempRepo {
            ...
            let dir = std::env::temp_dir()
                .join(format!("osiris-git-ops-{tag}-{}-{nanos}", std::process::id()));
