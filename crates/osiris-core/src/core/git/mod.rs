//! Everything osiris knows about git, split by question:
//!
...
//!
//! Nothing here depends on gpui: the headless `osiris-server` answers the same
...
fn emit(line: &[u8], dropped: usize, on_line: &mut impl FnMut(&str)) {
    ...
    text.push_str(&format!(
        " …[{dropped} more bytes on this line dropped: past osiris's {MAX_LINE}-byte line cap]"
    ));
    on_line(&text);
}
...
mod tests {
    ...
    #[test]
    fn non_repo_is_none() {
        let dir = std::env::temp_dir().join("osiris-git-status-not-a-repo-xyz");
    ...
    fn missing_path_is_none() {
        assert_eq!(probe(&*h(), Path::new("/no/such/osiris/path/here")), None);
    }
