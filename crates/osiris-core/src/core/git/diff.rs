...
impl DiffSource {
    ...
    /// The whole argv, minus pathspecs. Every diff osiris runs is built here so
...
/// - the line is in the first parent — the side osiris numbers — iff its own
...
/// shape. The first `-` is the first parent's, which is the side osiris numbers.
...
";
    ...
    fn merge_repo(name: &str) -> Option<PathBuf> {
        let dir = std::env::temp_dir().join(format!("osiris-diff-{name}-{}", std::process::id()));
