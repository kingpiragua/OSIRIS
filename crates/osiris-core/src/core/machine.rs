...
pub const DATA_DIR_ENV: &str = "OSIRIS_DATA_DIR";
...
pub struct PaneRecord {
    ...
    /// switcher listing a workspace it does not own, `oexe tab ls` across a
...
pub struct PaneSeed {
    ...
    /// tree as a seed — a split, a `osiris` CLI call — names its shell from the
...
impl MachineStore {
    ...
    fn ensure_flusher(self: &Arc<Self>) {
        ...
        let spawned = std::thread::Builder::new()
            .name("osiris-machine-flush".into())
            .spawn(move || {
                loop {
...
/// osiris's default preset is: a daemon that spawns a pane before the GUI has
...
/// two osiriss pointed at different config directories — each holding its own
...
///
/// Deliberately still resolved the old way, `OSIRIS_DATA_DIR` included: this
...
fn adopt_into(current: Option<&Path>, legacy: Option<&Path>, machines_instance: bool) {
    ...
    // makes `OSIRIS_DATA_DIR` a no-op: both getters answer with it.
    if legacy == current {
        return;
    }
...
/// Whether this process is the machine's osiris rather than an instance somebody
...
/// `$OSIRIS_CONFIG_DIR` when the box sets one, `$HOME`'s otherwise. Anything
/// aimed elsewhere is a second osiris by definition, and a second osiris starts on
/// an empty tree.
///
...
/// launch a real `osiris-server --config-dir <TempDir>` under the developer's own
/// `HOME`: a start-order rule moves the developer's real `machine.json` into a
/// scratch directory and deletes it with the `TempDir`.
///
/// Counting `$OSIRIS_CONFIG_DIR` as the machine's is what keeps remote hosts
/// upgrading: a remote `osiris-server` is launched with no `--config-dir` and
...
mod tests {
    ...
    fn tabs_rename_move_and_regroup_in_place() {
        ...
        store
            ...
            .unwrap();
        store
            .tab_set_group(ws, first.id, Some("/repo/osiris".into()), None)
            .unwrap();
    ...
    fn an_attachment_travels_by_name_and_never_by_token() {
        ...
        // What `MachineGet` hands a peer: `oexe ls` reads its ATTACHED column
    ...
    /// behind, so two osiriss that each believed they were the only server on
    ...
    /// must not be what picks. A second osiris renaming the tree into its own
    ...
    fn only_the_machines_own_instance_inherits_the_legacy_tree() {
        let machines = PathBuf::from("/home/u/.config/osiris");
    ...
    /// `OSIRIS_DATA_DIR` answers both getters, so the sandboxes every test
