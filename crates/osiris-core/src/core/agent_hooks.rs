...
pub const OSIRIS_ENV_MARKER: &str = "OSIRIS";
...
pub fn run_agent_hook(agent: &str, event: &str) {
    detach_console();
    if std::env::var_os(OSIRIS_ENV_MARKER).is_none() {
        return;
    }
...
fn write_to_controlling_tty(bytes: &[u8]) -> bool {
    ...
    let shell = ancestors.iter().copied().find(|&pid| {
        procs
            ...
            .is_some_and(|n| is_osiris_host_exe(&n))
    });
...
fn is_osiris_host_exe(name: &str) -> bool {
    matches!(name, "osiris.exe" | "osiris-server.exe" | "osiris.exe")
}
...
impl HookAgent {
    ...
    fn target_path(self, target: &HookTarget) -> PathBuf {
        match self {
            ...
            HookAgent::Pi => target.under_home(&[".pi", "agent", "extensions", "osiris", "index.ts"]),
            HookAgent::Grok => target.under_home(&[".grok", "hooks", OWNED_FILE_STEM_JSON]),
            HookAgent::OhMyPi => {
                target.under_home(&[".omp", "agent", "extensions", "osiris", "index.ts"])
            }
        }
    }
...
impl<'a> HookTarget<'a> {
    ...
    /// Build a local target for a known osiris hook runner.
    ///
    ...
    /// `osiris` it would launch instead of comparing configs to `osiris`.
...
pub enum HookOutcome {
    ...
    NothingInstalled,
    /// The config is there, but holds no osiris hooks.
    NoOsirisHooks,
}
...
const OWNED_FILE_STEM_JSON: &str = "osiris.json";
const OWNED_FILE_STEM_JS: &str = "osiris.js";
...
fn hook_map_state(
    ...
    for (hook_event, osiris_event) in events {
        let ours = root
            .get("hooks")
        ...
        match ours {
            Some(cmd) => {
                any = true;
                if cmd != target.hook_command(agent, osiris_event) {
                    complete = false;
                }
            }
            None => complete = false,
...
fn hook_map_install(
    ...
    let marker = agent.marker();
    for (hook_event, osiris_event) in events {
        let command = target.hook_command(agent, osiris_event);
        let entries = hooks
            .as_object_mut()
            .unwrap()
            .entry(*hook_event)
...
fn hook_map_uninstall(
    ...
    if removed == 0 {
        return Ok(HookOutcome::NoOsirisHooks);
    }
    target.write(path, serde_json::to_string_pretty(&root)?.as_bytes())?;
...
fn owned_file_install(
    ...
    {
        return Err(anyhow::anyhow!(
            "{} exists but wasn't written by osiris; not touching it",
            path.display()
        ));
    }
...
fn owned_file_uninstall(
    ...
    if !contents.contains(marker) {
        return Err(anyhow::anyhow!(
            "{} wasn't written by osiris; not touching it",
            path.display()
        ));
    }
    target.host.remove(path, false)?;
    if let Some(parent) = path.parent()
        && parent.file_name().is_some_and(|n| n == "osiris")
    {
...
fn opencode_plugin_js(target: &HookTarget) -> Option<String> {
    ...
    .ok()?;
    Some(format!(
        r#"// osiris agent-hook opencode bridge — generated by osiris, do not edit.
// Bridges OpenCode plugin events onto `osiris agent-hook opencode <event>`,
// which is inert outside osiris (gated on the OSIRIS env var).
export const OsirisPresence = async ({{ $ }}) => {{
  if (!process.env["OSIRIS"]) return {{}}
  const cmd = {prefix}
  let sessionId = ""
  let announced = ""
...
fn pi_extension_ts(target: &HookTarget, agent: HookAgent) -> Option<String> {
    ...
    Some(format!(
        r#"/* osiris agent-hook {slug} bridge — generated by osiris, do not edit. */
...
mod tests {
    ...
    #[test]
    fn every_osiris_daemon_host_takes_the_console_fast_path() {
        for name in ["osiris.exe", "osiris-server.exe", "osiris.exe"] {
            assert!(is_osiris_host_exe(name), "{name} hosts OSIRIS panes");
        }
        for name in ["explorer.exe", "cmd.exe", "osiris", "osiris", "wt.exe"] {
            assert!(!is_osiris_host_exe(name), "{name} is not an osiris host process");
        }
    }
    ...
    fn marker_detection_matches_our_entries_only() {
        let ours = serde_json::json!({
            "hooks": [{ "type": "command", "command": "\"/x/osiris\" agent-hook claude stop" }]
        });
    ...
    fn remote_paths_are_built_in_the_remote_machine_s_spelling() {
        ...
        for (agent, expected) in [
            ...
            (HookAgent::Copilot, "/home/me/.copilot/hooks/osiris.json"),
            (
                HookAgent::OpenCode,
                "/home/me/.config/opencode/plugins/osiris.js",
            ),
            (HookAgent::Pi, "/home/me/.pi/agent/extensions/osiris/index.ts"),
            (HookAgent::Grok, "/home/me/.grok/hooks/osiris.json"),
            (
                HookAgent::OhMyPi,
                "/home/me/.omp/agent/extensions/osiris/index.ts",
            ),
        ] {
            assert_eq!(
    ...
    fn the_hook_command_names_the_binary_on_that_machine() {
        ...
        let target = HookTarget::remote(&*host, PathBuf::from("/home/me"));
        let dialect = crate::daemon::install::RemoteProtocol::of_this_build();
        let name = format!("osiris-server-c{}p{}", dialect.control, dialect.protocol);
        assert_eq!(
            target.hook_command(HookAgent::Claude, "stop"),
            format!("\"/home/me/.local/share/osiris/bin/{name}\" agent-hook claude stop")
        );
    ...
    fn a_remote_install_round_trips_through_the_host() {
        let dir = std::env::temp_dir().join(format!("osiris-remote-hooks-{}", std::process::id()));
    ...
    fn owned_file_contents_carry_marker_and_exe() {
        ...
        assert!(opencode.contains(r#"process.env["OSIRIS"]"#));
        ...
        for (agent, slug, package) in [
            ...
            assert!(bridge.contains(r#"process.env["OSIRIS"]"#));
    ...
    fn owned_file_round_trip_and_ownership_guard() {
        let dir = std::env::temp_dir().join(format!("osiris-owned-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("osiris.json");
    ...
    fn install_is_idempotent_and_preserves_user_hooks() {
        let dir = std::env::temp_dir().join(format!("osiris-hooks-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        ...
        assert_eq!(
            stop.iter()
                ...
                .count(),
            1,
            "exactly one osiris entry after two installs"
        );
        assert!(
            stop.iter()
        ...
        assert!(
            ...
            "an event list that held only the osiris hook is dropped"
        );
