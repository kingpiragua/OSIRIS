...
pub struct Config {
    ...
    /// Whether the GUI puts the bundled `osiris` CLI on PATH at launch (see
    ...
    /// Off is for people who keep their own `osiris` — a `cargo install` build, a
    ...
    #[serde(default, deserialize_with = "de_lenient")]
    pub bell: BellMode,
    /// Whether osiris edits the shell prompt itself. On by default: the inline
    ...
    /// dotfile work exactly as they do outside osiris. Shell integration itself
    /// stays on: prompt boundaries, cwd, exit status and notifications are
    /// unaffected. `tab_completion` and `history_search` are osiris's own menus,
    ...
    /// Optional HTTP/SOCKS proxy for osiris's *own* update checks and release
...
/// crosses as user-authored: osiris has no defaults to contribute to a command it
/// did not choose.
#[derive(Debug, Clone, Default, PartialEq, Eq, Deserialize, Serialize)]
#[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            // OSIRIS.EXE has one register: phosphor on duat black. There is no
            // light half to follow the system into, so both sides of the
            // follow-system pair name the same theme — turning it on changes
            // nothing rather than dropping the archive into a stock light mode.
            theme: "osiris".to_string(),
            theme_preset: "osiris".to_string(),
            theme_follow_system: false,
            theme_preset_light: "osiris".to_string(),
            theme_preset_dark: "osiris".to_string(),
...
/// redirects it — `$OSIRIS_CONFIG_DIR` where the box names one, the default under
/// `$HOME` otherwise.
///
/// [`config_dir`] with the `--config-dir` override left off, which is the
/// question "is this the machine's osiris or a second one somebody pointed
...
pub fn machine_config_dir() -> Option<PathBuf> {
    if let Some(dir) = std::env::var_os("OSIRIS_CONFIG_DIR").filter(|d| !d.is_empty()) {
...
pub fn default_config_dir() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".config/osiris"))
}

#[cfg(windows)]
pub fn default_config_dir() -> Option<PathBuf> {
    if let Some(appdata) = std::env::var_os("APPDATA").filter(|d| !d.is_empty()) {
        return Some(PathBuf::from(appdata).join("osiris"));
    }
    let profile = std::env::var_os("USERPROFILE").filter(|d| !d.is_empty())?;
    Some(PathBuf::from(profile).join(".config").join("osiris"))
}
...
pub enum LinkFileOpen {
    /// osiris's own editor. The default, and the only one of the three that can
...
mod tests {
    ...
    fn theme_follow_system_defaults_and_round_trips() {
        ...
        assert_eq!(cfg.theme_preset_light, "osiris");
        assert_eq!(cfg.theme_preset_dark, "osiris");
    ...
    fn sanitize_clamps_to_the_same_bounds_the_gui_steps_within() {
        ...
        // wrong way. The bounds are one shared set now: `OsirisApp::set_font_size`
    ...
    impl TestDir {
        fn new(name: &str) -> Self {
            let dir = std::env::temp_dir().join(format!("osiris-test-{name}-{}", std::process::id()));
