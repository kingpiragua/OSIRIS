...
//!   that survives osiris's socket + SSH topology.
...
impl GraphicsSniffer {
    ...
    /// is local when osiris spawned a local shell and no foreground `ssh` owns the
...
/// The kitty action key (`a=`). Only the variants osiris acts on are named; the
...
///
/// Only the keys osiris needs are surfaced; unknown keys are ignored so forward
...
/// osiris targets — most often `d=A` (delete everything) — actually uses.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl GraphicsParser {
    ...
    pub fn feed(&mut self, command: &[u8]) -> Option<Event> {
        ...
        match control.action {
            ...
            // get nothing; `a=T` (the shape every sender osiris targets uses)
            // is unaffected.
            Action::Display => None,
...
mod tests {
    ...
    fn file_transfer_resolves_and_temp_file_is_removed() {
        ...
        let path = dir.join(format!("osiris-kitty-test-{}.rgba", std::process::id()));
