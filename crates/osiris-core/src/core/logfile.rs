...
fn level_from_env() -> LevelFilter {
    let raw = std::env::var("OSIRIS_LOG")
...
fn log_path() -> Option<PathBuf> {
    crate::core::config::config_path("osiris.log")
}
...
mod tests {
    ...
    fn logging_is_off_unless_asked_for() {
        ...
        assert_eq!(parse_level("osiris_core::daemon=debug"), LevelFilter::Off);
    }
    ...
    fn the_file_is_rewritten_once_it_passes_the_cap() {
        let path = std::env::temp_dir().join(format!("osiris-logfile-{}.log", std::process::id()));
    ...
    fn a_record_names_its_role_and_target() {
        let path = std::env::temp_dir().join(format!("osiris-logrec-{}.log", std::process::id()));
        let _ = std::fs::remove_file(&path);
        ...
        logger.log(
            &Record::builder()
                .args(format_args!("remote build-box: installed osiris-server"))
                .level(Level::Info)
                .target("osiris_core::daemon::install")
                .build(),
        );
        let written = std::fs::read_to_string(&path).unwrap();
        assert!(written.contains("daemon"), "{written}");
        assert!(written.contains("osiris_core::daemon::install"), "{written}");
        assert!(written.contains("installed osiris-server"), "{written}");
