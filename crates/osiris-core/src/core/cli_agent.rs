...
pub const AGENT_EVENT_SENTINEL: &str = "osiris://cli-agent";
...
mod tests {
    ...
    fn parses_sentinel_events() {
        let ev = parse_agent_event(
            br#"777;notify;osiris://cli-agent;{"v":1,"agent":"claude","event":"permission-request","session_id":"abc-123","message":"Claude needs your permission to use Bash"}"#,
        )
        ...
        assert_eq!(
            parse_agent_event(br#"777;notify;osiris://cli-agent;{"event":"quantum-leap"}"#),
            None
        );
        assert_eq!(
            parse_agent_event(b"777;notify;osiris://cli-agent;{oops"),
            None
        );
    }
