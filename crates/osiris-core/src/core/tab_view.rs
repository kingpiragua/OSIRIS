...
//! it does not own, `oexe tab ls` on the other side of a socket — has only
...
/// the paths that arrive here are not all `/…` or `~/…`: osiris's own PowerShell
...
mod tests {
    ...
    fn a_host_prefix_is_only_cut_when_a_path_follows_it() {
        ...
        assert_eq!(
            ...
            "C:/src",
            "osiris's own pwsh title names a drive when the cwd is off the home drive"
        );
