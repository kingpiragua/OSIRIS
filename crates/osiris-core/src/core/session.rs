...
impl RemoteTarget {
    ...
    /// Whether the far end is served by an osiris daemon this computer installed
...
mod tests {
    ...
    fn a_local_stdio_target_is_its_own_machine() {
        ...
        assert_eq!(a.connection_key(), "local-stdio:/opt/osiris-server --stdio");
        assert_ne!(a.host_id(), b.host_id());
        assert!(
            !a.host_id().is_local(),
            "a routed target is never the local host"
        );
        assert_eq!(a.to_string(), "local:osiris-server");
    }
