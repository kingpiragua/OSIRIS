...
impl Conn {
    ...
    fn start_git_stream(&self, id: u64, cwd: PathBuf, args: Vec<String>) {
        ...
        let spawned = std::thread::Builder::new()
            .name("osiris-control-git-stream".into())
            .spawn(move || {
                let _slot = slot;
                let borrowed: Vec<&str> = args.iter().map(String::as_str).collect();
...
fn spawn_layout_forwarder(
    ...
    let spawned = std::thread::Builder::new()
        .name("osiris-control-layout".into())
        .spawn(move || {
...
fn spawn_watch_forwarder(id: u64, rx: smol::channel::Receiver<Vec<PathBuf>>, sink: Arc<Sink>) {
    let spawned = std::thread::Builder::new()
        .name("osiris-control-watch".into())
        .spawn(move || {
    ...
    fn submit(&self, job: impl FnOnce() + Send + 'static) -> bool {
        ...
        if st.wants_another_worker() {
            ...
            match std::thread::Builder::new()
                .name("osiris-control-worker".into())
...
pub const CONTROL_SOCK_ENV: &str = "OSIRIS_CONTROL_SOCK";
...
mod sock {
    ...
    ///
    /// It used to sit in `$XDG_RUNTIME_DIR/osiris` or `~/.local/share/osiris`,
    ...
    pub fn spawn_control_listener_with(
        ...
        std::thread::Builder::new()
            .name("osiris-control-listener".into())
    ...
    fn machine_get_overlays_live_pane_titles() {
        ...
        let services = Services {
            ...
            panes: Some(Arc::new(ThreePanesOneAgent {
                panes: vec![PaneInfo {
                    pane_id: 7,
                    cwd: Some(PathBuf::from("/repo/osiris")),
                    title: "nvim".into(),
                    osc_title: Some("nvim ~/repo/osiris".into()),
                    alive: true,
                    owner: None,
                }],
            })),
        ...
        assert_eq!(pane.osc_title.as_deref(), Some("nvim ~/repo/osiris"));
        assert!(pane.live);
    }
...
mod tests {
    ...
    fn binding_does_not_re_permission_a_directory_it_did_not_create() {
        ...
        assert_eq!(
            ...
            0o1777,
            "a directory osiris did not create must keep its own permissions"
        );
        drop(listener);
    }
