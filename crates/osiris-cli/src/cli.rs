...
#[command(
    name = "oexe",
    version,
    about = "OSIRIS.EXE from the command line — the archive without the window",
    long_about = "One name, three artifacts: this CLI (oexe), the OSIRIS.EXE GUI\n\
                  (osiris), and osiris-server.\n\n\
                  ...
                  protocol of its own. Inside an OSIRIS pane, $OSIRIS_CONFIG_DIR points at\n\
                  the server, and $OSIRIS_PANE / $OSIRIS_WS let the address-taking verbs run\n\
...
pub enum Command {
    #[command(about = "This machine on one screen (= oexe ws ls)")]
    Ls,
    ...
    #[command(about = "Split a pane (= oexe pane split)")]
    Split(SplitArgs),
    ...
    #[command(
        about = "Block until a pane's agent needs input, finishes its turn, or the pane \
                 exits — the orchestration primitive: `oexe wait %3 && oexe capture %3 --plain`"
    )]
    Wait(WaitArgs),
    ...
    #[command(about = "Server health at a glance (= oexe server status)")]
    Status,
    ...
    #[command(subcommand, about = "The osiris-server process on this machine")]
    Server(ServerCmd),
}
...
pub struct RunArgs {
    ...
    #[arg(
        ...
        help = "Sets the pane's OSIRIS_WS and, with --keep, the workspace the kept pane is \
                filed into; defaults to $OSIRIS_WS inside an OSIRIS pane"
    )]
    ...
    #[arg(
        ...
        required = true,
        value_name = "CMD",
        help = "The command, after `--`: oexe run -- cargo test"
    )]
    pub cmd: Vec<String>,
}
...
pub struct SplitArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to split; defaults to $OSIRIS_PANE inside an OSIRIS pane"
    )]
...
pub struct SendArgs {
    ...
    #[arg(
        ...
        help = "Send a keystroke instead of text; repeat for a sequence \
                (C-c, escape, up, enter, …). See `oexe send --help`"
    )]
...
pub struct WaitArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to watch; defaults to $OSIRIS_PANE inside an OSIRIS pane"
    )]
    pub target: Option<String>,
...
pub struct CaptureArgs {
    #[arg(
        value_name = "%PANE",
        help = "Pane to read; defaults to $OSIRIS_PANE inside an OSIRIS pane"
    )]
    pub target: Option<String>,
...
pub enum PaneCmd {
    ...
    Close {
        #[arg(value_name = "%PANE", help = "Panes to close; defaults to $OSIRIS_PANE")]
        targets: Vec<String>,
...
mod tests {
    ...
    #[test]
    fn bare_osiris_is_the_gui_launcher() {
        let cli = parse(&["osiris"]);
    ...
    fn a_path_that_is_not_a_verb_still_launches_the_gui() {
        let cli = parse(&["osiris", "C:\\Users\\me\\proj"]);
    ...
    fn a_gui_path_preserves_native_windows_arguments() {
        ...
        let cli = Cli::try_parse_from([OsString::from("osiris"), native_path.clone()])
    ...
    fn every_top_level_verb_parses() {
        assert!(matches!(parse(&["osiris", "ls"]).command, Some(Command::Ls)));
        assert!(matches!(
            parse(&["osiris", "new"]).command,
            Some(Command::New { path: None, .. })
        ));
        assert!(matches!(
            parse(&["osiris", "new", "C:\\proj"]).command,
            Some(Command::New { path: Some(p), .. }) if p == "C:\\proj"
        ));
        assert!(matches!(
            parse(&["osiris", "agents"]).command,
            Some(Command::Agents)
        ));
        assert!(matches!(
            parse(&["osiris", "events"]).command,
            Some(Command::Events)
        ));
        assert!(matches!(
            parse(&["osiris", "status"]).command,
            Some(Command::Status)
        ));
        assert!(matches!(
            parse(&["osiris", "doctor"]).command,
            Some(Command::Doctor)
        ));
        assert!(matches!(
            parse(&["osiris", "procs", "%3"]).command,
    ...
    fn run_takes_the_command_after_a_double_dash() {
        let cli = parse(&["osiris", "run", "--", "cargo", "test", "--keep"]);
        ...
        assert!(
            !args.keep,
            "--keep after -- belongs to the child, not to osiris"
        );

        let cli = parse(&["osiris", "run", "--keep", "--cwd", "C:\\proj", "--", "make"]);
        let Some(Command::Run(args)) = cli.command else {
            panic!("run did not parse");
        };
        assert!(args.keep);
    ...
    fn run_without_a_command_is_a_usage_error() {
        let err = Cli::try_parse_from(["osiris", "run"]).unwrap_err();
        assert_eq!(err.kind(), ErrorKind::MissingRequiredArgument);
    }

    #[test]
    fn split_requires_exactly_one_axis() {
        let cli = parse(&["osiris", "split", "--v"]);
        let Some(Command::Split(args)) = cli.command else {
            panic!("split did not parse");
        };
        assert!(args.vertical && !args.horizontal);
        assert!(args.target.is_none(), "the pane comes from $OSIRIS_PANE");
        assert_eq!(args.ratio, 0.5);

        let cli = parse(&["osiris", "pane", "split", "%42", "--h", "--ratio", "0.3"]);
        ...
        let err = Cli::try_parse_from(["osiris", "split", "%1"]).unwrap_err();
        assert_eq!(
            err.kind(),
            ErrorKind::MissingRequiredArgument,
            "an axis must be chosen"
        );
        let err = Cli::try_parse_from(["osiris", "split", "--h", "--v"]).unwrap_err();
        assert_eq!(
            err.kind(),
    ...
    fn send_takes_an_optional_address_then_text() {
        let cli = parse(&["osiris", "send", "%42", "make -j8", "--enter"]);
        ...
        let cli = parse(&["osiris", "send", "make -j8"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
    ...
    fn send_accepts_keys_with_or_without_text() {
        let cli = parse(&["osiris", "send", "%42", "--key", "C-c"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
        assert_eq!(args.first.as_deref(), Some("%42"));
        ...
        let cli = parse(&["osiris", "send", "--key", "down", "--key", "enter"]);
        let Some(Command::Send(args)) = cli.command else {
            panic!("send did not parse");
        };
        assert!(args.first.is_none(), "the pane comes from $OSIRIS_PANE");
        ...
        let err = Cli::try_parse_from(["osiris", "send", "--key", "f7"]).unwrap_err();
    ...
    fn every_pane_verb_parses() {
        ...
        assert!(matches!(
            parse(&["osiris", "pane", "close", "%9"]).command,
        ...
        assert!(matches!(
            parse(&["osiris", "pane", "close", "%9", "%10"]).command,
            Some(Command::Pane(PaneCmd::Close { targets, .. })) if targets == ["%9", "%10"]
        ));
        assert!(matches!(
            parse(&["osiris", "pane", "close", "--orphans"]).command,
        ...
        let err = Cli::try_parse_from(["osiris", "pane", "close", "%9", "--orphans"]).unwrap_err();
    ...
    fn every_machine_and_server_verb_parses() {
        assert!(matches!(
            parse(&["osiris", "machine", "ls"]).command,
            Some(Command::Machine(MachineCmd::Ls))
        ));
        assert!(matches!(
            parse(&["osiris", "machine", "connect", "devbox"]).command,
            Some(Command::Machine(MachineCmd::Connect { profile })) if profile == "devbox"
        ));
        assert!(matches!(
            parse(&["osiris", "machine", "disconnect", "devbox"]).command,
        ...
        for (verb, want) in [
            ...
            let cli = parse(&["osiris", "server", verb]);
    ...
    fn global_flags_are_accepted_anywhere() {
        let cli = parse(&["osiris", "-m", "devbox", "ls", "--json"]);
        assert_eq!(cli.machine.as_deref(), Some("devbox"));
        assert!(cli.json);
        assert!(matches!(cli.command, Some(Command::Ls)));

        let cli = parse(&["osiris", "ws", "ls", "-q", "-m", "devbox"]);
        assert!(cli.quiet);
        assert_eq!(cli.machine.as_deref(), Some("devbox"));

        let cli = parse(&["osiris", "capture", "%3", "--scrollback", "--json"]);
        assert!(cli.json);
    ...
    fn capture_plain_is_its_own_flag_and_composes_with_scrollback() {
        ...
        let Some(Command::Capture(args)) = parse(&["osiris", "capture", "--plain"]).command else {
            panic!("capture did not parse");
        };
        ...
        let Some(Command::Capture(args)) =
            parse(&["osiris", "capture", "%3", "--plain", "--scrollback"]).command
        else {
            panic!("capture did not parse");
        };
        assert!(args.plain && args.scrollback);
    ...
    fn usage_errors_exit_with_code_2() {
        let err = Cli::try_parse_from(["osiris", "ws", "frobnicate"]).unwrap_err();
        assert_eq!(
            err.exit_code(),
            2,
            "clap's usage-error exit code is the CLI contract"
        );
        let err = Cli::try_parse_from(["osiris", "tab", "move", "@7", "not-a-number"]).unwrap_err();
        assert_eq!(err.exit_code(), 2);
    }
}
