...
pub const ENV_PANE: &str = "OSIRIS_PANE";
pub const ENV_WS: &str = "OSIRIS_WS";
/// The server's config dir, not a socket path — the server publishes the
/// directory so both endpoints resolve through osiris_core's own derivation
...
pub const ENV_CONFIG_DIR: &str = "OSIRIS_CONFIG_DIR";

pub const OUTSIDE_SHELL: &str = "not inside an OSIRIS pane — pass an explicit %pane/@tab/workspace";
...
pub fn parse_pane(s: &str) -> Result<u64> {
    // The `%` is optional: `oexe pane ls --json` hands back bare ids, so a
...
pub fn parse_tab(s: &str) -> Result<TabAddress> {
    ...
    bail!("'{s}' is not a tab address — @7 as numbered by `oexe ls`, or @<full tab id>");
}
...
mod tests {
    ...
    fn shell_context() -> Context {
        Context {
            ...
            config_dir: Some("C:\\Users\\me\\AppData\\Roaming\\osiris".into()),
        }
    }
    ...
    fn a_bare_pane_id_addresses_the_same_pane_as_the_marked_one() {
        // `oexe pane ls --json` prints bare ids; refusing them here pushed
        // people into "%${OSIRIS_PANE#%}" contortions (#538).
        assert_eq!(parse_pane("42").unwrap(), 42);
        assert_eq!(parse_pane("%42").unwrap(), 42);
    }
    ...
    #[test]
    fn outside_a_osiris_shell_the_error_names_the_fix() {
    ...
    fn a_broken_env_pane_is_reported_not_silently_ignored() {
        ...
        let err = pane_or_context(None, &ctx).unwrap_err().to_string();
        assert!(err.contains("OSIRIS_PANE"), "{err}");
    }
}
