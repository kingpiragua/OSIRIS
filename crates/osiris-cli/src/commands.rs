...
use osiris_core::core::agent_hooks::{HookAgent, HooksState};
use osiris_core::core::machine::{Axis, Machine, PaneSeed, Workspace};
use osiris_core::core::session::WorkspaceId;
use osiris_core::core::tab_view::tab_views_of;
use osiris_core::daemon::control::{CONTROL_VERSION, ControlEvent, ControlRequest, ReplyOk};
use osiris_core::daemon::protocol::PROTOCOL_VERSION;
...
pub fn execute(cli: Cli, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    match cli.command {
        None => match cli.path {
            ...
            // is: launching the GUI for `osiris statu` would hide the typo.
            Some(word) if !looks_like_a_path(&word) => bail!(
                "unknown subcommand '{}' — run `oexe --help` for the list. \
        ...
        Some(Command::Ws(WsCmd::Stop { .. })) => bail!(
            "`oexe ws stop` is not implemented yet — the control dialect has no \
...
fn local_server(
    ...
    if let Some(machine) = machine {
        bail!(
            "`oexe server {verb}` manages only the server on THIS machine — with -m {machine} \
...
fn launch_gui(
    ...
    if let Some(machine) = machine {
        bail!(
            "`osiris [PATH]` controls the GUI on this machine and cannot be combined with -m {machine}"
        );
    }
    ...
    let opened = match open {
        ...
        true => match backend.control(ControlRequest::GuiOpen {
            ...
            Ok(ReplyOk::Bool(opened)) => {
                ...
                if !opened {
                    eprintln!(
                        "osiris: no GUI is running on this machine; \
            ...
            Err(error) => {
                eprintln!(
                    "osiris: could not ask the GUI to open it ({error:#}); \
                     the workspace was made all the same"
                );
                false
            }
...
fn run(args: RunArgs, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if args.keep && workspace.is_none() {
        bail!(
            "`run --keep` keeps the pane alive, so it must be filed into a workspace — \
             pass --ws, or run inside an OSIRIS pane where $OSIRIS_WS names one"
        );
    }
    ...
    let (code, exact) = match backend.run_wait()? {
        Some(code) => (code, true),
        None => {
            eprintln!("osiris: {EXIT_CODE_UNKNOWN}");
            (1, false)
        }
    };
...
/// interrupted `oexe run` leaves its pane running with nothing referencing it,
...
fn pane_ls_all(backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    if orphans > 0 {
        human.push_str(&format!(
            "\n{orphans} pane(s) held by no workspace — `oexe pane close %<id>` stops one, \
             `oexe pane close --orphans` stops all of them\n"
        ));
    }
...
fn pane_close(
    ...
    if !failures.is_empty() {
        ...
        eprintln!(
            "osiris: closed {} pane(s); {} could not be closed — {}",
            closed.len(),
            failures.len(),
            failures.join("; ")
        );
...
/// control request per tick — the same request `oexe agents` makes once.
fn wait(args: WaitArgs, ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    use std::time::{Duration, Instant};
    use osiris_core::core::cli_agent::AgentStatus;
    ...
    loop {
        ...
        if matched || current == WaitState::Exit {
            ...
            if !matched {
                ...
                eprintln!("osiris: pane %{pane} exited before reaching the awaited state");
                return Ok(Outcome::Exit(
                    1,
                    Report {
        ...
        if deadline.is_some_and(|d| Instant::now() >= d) {
            ...
            if current == WaitState::NoAgent {
                human.push_str(
                    "\nnothing is reporting agent status in this pane — for a plain command \
                     wait `--until free`, and for an agent check `oexe agents` for a missing \
                     status hook",
                );
            }
            ...
            eprintln!("osiris: pane %{pane}: still {} — timed out", current.name());
            return Ok(Outcome::Exit(
                124,
                Report {
                    human,
...
/// the command — which is what `oexe run` spawns — reads free for as long as it
...
fn agent_hook_diagnostics(
    states: &[osiris_core::daemon::control::PaneAgentState],
    machine: &Machine,
    backend: &mut dyn Backend,
...
fn agents_human(
    ...
    for diagnostic in diagnostics {
        ...
        human.push_str(&format!(
            "{} is running, but its osiris agent-status hooks are {state}. Open Settings → Agents to {} the hooks, then start a new {} session.\n",
...
fn doctor(ctx: &Context, backend: &mut dyn Backend) -> Result<Outcome> {
    ...
    match backend.hello() {
        Ok(hello) => {
            ...
            // Without hooks an agent reports no status, which means `osiris
            // agents` shows it standing still and `oexe wait` never wakes. That
    ...
    if ctx.config_dir.is_none() && ctx.pane.is_none() {
        human.push_str(
            "\nnot inside an OSIRIS pane — address commands need an explicit %pane/@tab/workspace\n",
        );
    }
    let report = Report {
        human,
    ...
    if report.json["server"]["reachable"] == false {
        ...
        // `oexe doctor || alert` has to fire (#592). The table and JSON go
        // out all the same, and stderr carries the headline under `-q`.
        eprintln!("osiris: doctor: the server is unreachable");
...
fn hooks_summary(hooks: &[(HookAgent, HooksState)]) -> String {
    ...
    // osiris knows about would bury it. "Up to date" rather than "installed":
...
mod tests {
    ...
    use clap::Parser;
    use osiris_core::core::cli_agent::CLIAgent;
    use osiris_core::core::machine::Tab;
    ...
    fn segment(bytes: &[u8]) -> crate::backend::CaptureSegment {
        crate::backend::CaptureSegment {
            size: osiris_core::daemon::protocol::WinSize {
                cols: 20,
                rows: 10,
                cell_w: 8,
                cell_h: 16,
    ...
    fn pane_info(pane_id: u64, owner: Option<&str>) -> osiris_core::daemon::protocol::PaneInfo {
        osiris_core::daemon::protocol::PaneInfo {
            pane_id,
            cwd: None,
            title: "sh".into(),
            osc_title: None,
    ...
    fn pane_ls_all_surfaces_the_panes_no_workspace_holds() {
        ...
        // interrupted `oexe run` leaves behind.
        backend.registry = vec![
            pane_info(1, None),
            pane_info(3, None),
            pane_info(77, Some("osiris-cli")),
        ];
        ...
        assert_eq!(orphan["owner"], serde_json::json!("osiris-cli"), "{orphan}");
    ...
    fn pane_ls_without_all_cannot_see_an_orphan() {
        let mut backend = mock();
        backend.registry = vec![pane_info(77, Some("osiris-cli"))];
    ...
    fn ls_and_ws_ls_are_the_same_request() {
        ...
        let mut a = mock();
        run_cli(&["osiris", "ls"], &ctx, &mut a);
        let mut b = mock();
        run_cli(&["osiris", "ws", "ls"], &ctx, &mut b);
    ...
    fn ws_rename_rm_attach_detach_build_their_requests() {
        ...
        run_cli(&["osiris", "ws", "rename", "api", "core"], &ctx, &mut backend);
        assert_eq!(
        ...
        run_cli(&["osiris", "ws", "rm", "web"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
        ...
        let out = run_cli(&["osiris", "ws", "attach", "api"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
        ...
        backend.control_calls.clear();
        run_cli(&["osiris", "ws", "detach", "api"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
    ...
    fn tab_verbs_resolve_machine_wide_ordinals_to_real_ids() {
        ...
        run_cli(&["osiris", "tab", "close", "@3"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls,
            vec![
                ControlRequest::MachineGet,
        ...
        backend.control_calls.clear();
        run_cli(&["osiris", "tab", "move", "@2", "0"], &ctx, &mut backend);
        assert_eq!(
            backend.control_calls[1],
    ...
    fn tab_new_uses_the_workspace_from_the_environment() {
        ...
        run_cli(
            &["osiris", "tab", "new", "--cwd", "C:\\elsewhere"],
            &ctx,
            &mut backend,
        );
        assert_eq!(
    ...
    fn pane_split_builds_the_split_and_spawns_the_new_shell() {
        ...
        let api = backend.machine.workspaces[0].id;
        let out = run_cli(
            &["osiris", "pane", "split", "%2", "--v", "--ratio", "0.3"],
            &Context::default(),
            &mut backend,
        );
        assert_eq!(
    ...
    fn split_without_an_address_uses_the_pane_from_the_environment() {
        ...
        run_cli(&["osiris", "split", "--h"], &ctx, &mut backend);
        let web = backend.machine.workspaces[1].id;
    ...
    fn pane_close_orphans_reaps_only_what_no_workspace_holds() {
        ...
        backend.registry = vec![
            ...
            pane_info(3, None),
            pane_info(77, Some("osiris-cli")),
            pane_info(78, Some("osiris-cli")),
        ];

        let json = json_of(run_cli(
            &["osiris", "pane", "close", "--orphans"],
            &Context::default(),
            &mut backend,
        ));
    ...
    fn send_reaches_the_pane_socket_seam_not_the_control_socket() {
        ...
        backend.sent.clear();
        run_cli(&["osiris", "send", "echo hi"], &ctx, &mut backend);
    ...
    fn send_key_presses_keys_in_order() {
        let mut backend = mock();
        run_cli(
            &["osiris", "send", "%1", "--key", "down", "--key", "enter"],
            &Context::default(),
            &mut backend,
        );
        assert_eq!(
    ...
    /// otherwise `oexe send %42` would silently do nothing at all.
    #[test]
    fn send_still_refuses_a_bare_address_when_there_is_nothing_to_press() {
        ...
        // And outside an OSIRIS pane, with neither text nor keys, the complaint
        // is about the missing input rather than the missing pane.
        let mut backend = mock();
        let err = execute(cli(&["osiris", "send"]), &Context::default(), &mut backend)
    ...
    /// $OSIRIS_PANE the fallback errors OUTSIDE_SHELL before the guard matters
    /// (#538).
    #[test]
    fn a_broken_address_errors_instead_of_typing_into_the_callers_pane() {
        ...
        let mut backend = mock();
        let err = execute(cli(&["osiris", "send", "%42a"]), &ctx, &mut backend)
    ...
    fn a_lone_bare_id_refuses_loudly_rather_than_retargeting() {
        ...
        let mut backend = mock();
        let err = execute(cli(&["osiris", "send", "83"]), &ctx, &mut backend)
        ...
        let mut backend = mock();
        let err = execute(cli(&["osiris", "send", "83", "--enter"]), &ctx, &mut backend)
    ...
    fn enter_alone_presses_enter_at_the_address_it_was_given() {
        ...
        let mut backend = mock();
        run_cli(&["osiris", "send", "--enter"], &ctx, &mut backend);
        ...
        let mut backend = mock();
        run_cli(
            &["osiris", "send", "83", "--key", "enter"],
            &ctx,
            &mut backend,
        );
        assert_eq!(backend.sent, vec![(83, b"\r".to_vec())]);
        let mut backend = mock();
        run_cli(&["osiris", "send", "%83", "--enter"], &ctx, &mut backend);
        assert_eq!(backend.sent, vec![(83, b"\r".to_vec())]);
    }
    ...
    fn percent_led_text_still_types_into_the_current_pane() {
        ...
        let mut backend = mock();
        run_cli(&["osiris", "send", "%s/a/b/"], &ctx, &mut backend);
        assert_eq!(backend.sent, vec![(5, b"%s/a/b/".to_vec())]);

        let mut backend = mock();
        run_cli(&["osiris", "send", "%!sort", "--enter"], &ctx, &mut backend);
        assert_eq!(
            backend.sent,
        ...
        for text in ["3x", "+5", "50%"] {
            let mut backend = mock();
            run_cli(&["osiris", "send", text], &ctx, &mut backend);
            assert_eq!(
                backend.sent,
    ...
    fn capture_and_procs_are_wired_through_the_backend() {
        ...
        run_cli(&["osiris", "procs", "%1"], &Context::default(), &mut backend);
    ...
    fn run_passes_the_command_and_its_exit_code_through() {
        ...
        let out = execute(
            cli(&["osiris", "run", "--keep", "--", "cargo", "test"]),
            &ctx,
            &mut backend,
        )
        .unwrap();
    ...
    fn run_keep_spawns_first_then_files_the_pane_into_the_workspace() {
        ...
        let out = execute(
            cli(&[
                "osiris", "run", "--keep", "--ws", "api", "--cwd", "C:\\proj", "--", "cargo", "watch",
            ]),
            &Context::default(),
            &mut backend,
        )
    ...
    fn run_answers_json_even_though_it_carries_an_exit_code() {
        ...
        let out = execute(
            cli(&["osiris", "run", "--json", "--", "cargo", "test"]),
            &ctx,
            &mut backend,
        )
        .unwrap();
    ...
    fn a_machine_flag_refuses_the_local_server_verbs() {
        for verb in ["start", "stop", "restart", "logs"] {
            let mut backend = mock();
            let err = execute(
                cli(&["osiris", "-m", "devbox", "server", verb]),
                &Context::default(),
                &mut backend,
            )
    ...
    fn a_mistyped_subcommand_is_named_as_one_not_offered_to_the_gui() {
        for typo in ["tree", "statu", "pnae", "workspace"] {
            let err = execute(cli(&["osiris", typo]), &Context::default(), &mut mock())
    ...
    fn a_path_asks_the_running_gui_to_open_an_absolute_directory() {
        ...
        let out = run_cli(&["osiris", "."], &Context::default(), &mut backend);
    ...
    fn an_invalid_gui_path_fails_before_touching_the_wire() {
        let mut backend = mock();
        let missing =
            std::env::temp_dir().join(format!("osiris-cli-missing-path-{}", std::process::id()));
        let arg = missing.to_str().unwrap().to_owned();
        let err = execute(cli(&["osiris", &arg]), &Context::default(), &mut backend)
    ...
    fn a_non_utf8_gui_path_stays_off_the_string_protocol() {
        ...
        let path = std::env::temp_dir().join(OsString::from_vec(b"osiris-\xff".to_vec()));
    ...
    fn the_still_missing_verbs_say_so_without_touching_the_wire() {
        for (args, needle) in [
            (vec!["osiris", "ws", "stop", "api"], "not implemented"),
            (
                vec!["osiris", "machine", "connect", "devbox"],
                "not implemented",
            ),
            (
                vec!["osiris", "machine", "disconnect", "devbox"],
                "not implemented",
            ),
        ] {
            let mut backend = mock();
    ...
    fn agent_state(
        pane_id: u64,
        status: osiris_core::core::cli_agent::AgentStatus,
    ) -> osiris_core::daemon::control::PaneAgentState {
        agent_state_at(pane_id, status, 0)
    }

    /// A pane sitting at its prompt: the shell, and nothing in front of it.
    fn idle_procs() -> osiris_core::daemon::protocol::PaneProcs {
        osiris_core::daemon::protocol::PaneProcs {
    ...
    fn busy_procs() -> osiris_core::daemon::protocol::PaneProcs {
        osiris_core::daemon::protocol::PaneProcs {
            procs: vec![
    ...
    fn proc_entry(
        ...
        depth: u8,
        foreground: bool,
    ) -> osiris_core::daemon::protocol::ProcEntry {
        osiris_core::daemon::protocol::ProcEntry {
            pid,
            name: name.into(),
            depth,
    ...
    fn agent_state_at(
        ...
        osiris_core::daemon::control::PaneAgentState {
            pane_id,
            agent: None,
            state: osiris_core::core::cli_agent::AgentSessionState {
                status,
    ...
    fn detect_agent(backend: &mut MockBackend, pane_id: u64, agent: CLIAgent) {
        use osiris_core::core::machine::AgentFacts;
    ...
    fn wait_returns_the_moment_the_state_matches() {
        ...
        let out = run_cli(&["osiris", "wait", "%3"], &Context::default(), &mut backend);
        let json = json_of(out);
    ...
    fn wait_reads_agentless_panes_from_the_tree() {
        ...
        let mut backend = mock();
        backend.replies.push_back(ReplyOk::AgentStates(Vec::new()));
        let out = run_cli(&["osiris", "wait", "%9"], &Context::default(), &mut backend);
        let json = json_of(out);
        assert_eq!(json["status"], "exit");
        assert_eq!(json["matched"], true);
    ...
    fn wait_does_not_call_a_busy_shell_idle() {
        ...
        let out = execute(
            cli(&["osiris", "wait", "%3", "--until", "idle", "--timeout", "0"]),
            &Context::default(),
            &mut backend,
        )
        .expect("a timeout is an exit code, not an error");
    ...
    fn wait_free_ends_when_the_foreground_command_exits() {
        ...
        let json = json_of(run_cli(
            &["osiris", "wait", "%3", "--until", "free", "--interval", "50"],
            &Context::default(),
            &mut backend,
        ));
    ...
    fn wait_free_does_not_overrule_a_state_the_caller_asked_for() {
        ...
        let json = json_of(run_cli(
            &["osiris", "wait", "%3", "--until", "waiting,free"],
            &Context::default(),
            &mut backend,
        ));
    ...
    fn wait_free_does_not_read_an_empty_process_tree_as_finished() {
        let mut backend = mock();
        backend.replies.push_back(ReplyOk::AgentStates(Vec::new()));
        backend.procs_reply = osiris_core::daemon::protocol::PaneProcs::default();

        let out = execute(
            cli(&["osiris", "wait", "%3", "--until", "free", "--timeout", "0"]),
            &Context::default(),
            &mut backend,
        )
        .expect("a timeout is an exit code, not an error");
    ...
    fn wait_only_reads_the_process_tree_when_free_is_asked_for() {
        ...
        run_cli(&["osiris", "wait", "%3"], &Context::default(), &mut backend);
        assert!(
            backend.procs_calls.is_empty(),
    ...
    fn wait_timeout_exits_124() {
        ...
        let out = execute(
            cli(&["osiris", "wait", "%3", "--until", "done", "--timeout", "0"]),
            &Context::default(),
            &mut backend,
        )
        .expect("a timeout is an exit code, not an error");
    ...
    fn agents_distinguishes_no_agent_from_a_healthy_agent() {
        ...
        let mut backend = mock();
        backend.replies.push_back(ReplyOk::AgentStates(Vec::new()));
        let out = run_cli(&["osiris", "agents"], &Context::default(), &mut backend);
        assert_eq!(
            backend.control_calls,
    ...
    fn agents_reports_outdated_hooks_with_the_update_action() {
        ...
        let report = match run_cli(&["osiris", "agents"], &Context::default(), &mut backend) {
    ...
    fn status_and_machine_ls_are_single_aggregate_requests() {
        use osiris_core::daemon::control::{RouteInfo, ServerStatus};
        ...
        let out = run_cli(&["osiris", "status"], &Context::default(), &mut backend);
    ...
    fn doctor_backend() -> MockBackend {
        use osiris_core::daemon::control::ServerStatus;
    ...
    fn doctor_reports_the_injected_context_and_the_server_half() {
        ...
        assert!(out.contains("OSIRIS_CONFIG_DIR"), "{out}");
        ...
        let ctx = Context {
            pane: Some("7".into()),
            ws: None,
            config_dir: Some("/cfg/osiris".into()),
        };
        let out = human(run_cli(&["osiris", "doctor"], &ctx, &mut doctor_backend()));
        assert!(out.contains("set (/cfg/osiris)"), "{out}");
    }

    /// Missing hooks are the reason a perfectly healthy-looking agent never
    /// reports and `oexe wait` sits there until it times out. `doctor` is the
    ...
    fn doctor_reports_where_the_agent_status_hooks_stand() {
        use osiris_core::core::agent_hooks::HookAgent;
        ...
        backend.agent_hooks_states = HookAgent::ALL
            ...
            .collect();
        let out = run_cli(&["osiris", "doctor"], &Context::default(), &mut backend);
    ...
    /// exits non-zero over it — `oexe doctor || alert` has to fire — while
    ...
    fn doctor_exits_nonzero_when_the_server_is_unreachable() {
        ...
        assert!(r.human.contains("OSIRIS_CONFIG_DIR"), "{}", r.human);
