...
use serde_json::json;
use osiris_core::client::{ControlClient, PaneClient, PaneSession};
use osiris_core::core::agent_hooks::{HookAgent, HookTarget, HooksState, hooks_state};
...
use osiris_core::daemon::protocol::{DaemonMsg, PaneInfo, PaneProcs, ShellSpec, WinSize};
use osiris_core::daemon::router::RouteTarget;
...
const NOT_RUNNING: &str =
    "could not reach the osiris server on this machine — `oexe server start` brings one up";
...
impl RealBackend {
    ...
    /// `OSIRIS_CONFIG_DIR`, which is what `osiris_core`'s own endpoint derivation
    ...
    fn hello_msg() -> ControlHello {
        ControlHello::host_rpc(format!("osiris-cli-{}", std::process::id()), hostname())
    }
        ...
        let code = loop {
            match session.recv() {
                Ok(DaemonMsg::Output(bytes)) | Ok(DaemonMsg::Snapshot(bytes)) => {
                    ...
                    // (`oexe run -- … | head`) must end the pipeline, not turn
...
fn hostname() -> String {
    std::env::var("COMPUTERNAME")
        ...
        .unwrap_or_else(|_| "osiris-cli".to_string())
}
