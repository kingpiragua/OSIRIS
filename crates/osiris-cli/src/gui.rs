...
/// The bundled `osiris` this CLI belongs to: `OSIRIS`, then the file next
...
pub fn find_executable() -> Result<PathBuf> {
    if let Some(explicit) = std::env::var_os("OSIRIS") {
        let path = PathBuf::from(explicit);
        if path.is_file() {
            return Ok(path);
        }
        bail!(
            "OSIRIS points to {}, but that file does not exist",
            path.display()
        );
    }
    ...
    bail!("could not find {name} next to this CLI or on PATH — install osiris, or set OSIRIS")
}
