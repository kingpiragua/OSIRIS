...
pub const SERVER_EXE_ENV: &str = "OSIRIS_SERVER_EXE";
...
/// `$OSIRIS_CONFIG_DIR`: `spawn::stop` dials `transport::connect`, which derives
...
/// endpoint these verbs reach and the one `oexe status` reports on are one and
...
pub fn logs() -> Result<Outcome> {
    let Some(path) = config::config_path("osiris.log") else {
    ...
    match std::fs::read_to_string(&path) {
        ...
        Err(_) => {
            human.push_str("no log file yet — set OSIRIS_LOG=info before starting the server\n");
        }
    }
    report(
        human,
