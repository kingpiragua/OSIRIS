...
use osiris_core::core::machine::{Machine, PaneNode, Workspace};
use osiris_core::core::session::WorkspaceId;
use osiris_core::core::tab_view::{TabLabel, TabView, strip_host_prefix, tab_views_of};
use osiris_core::daemon::control::{PaneAgentState, RouteInfo, ServerStatus};
use osiris_core::daemon::protocol::{PaneInfo, PaneProcs};
...
pub fn workspace_table(machine: &Machine) -> String {
    if machine.workspaces.is_empty() {
        return "no workspaces — `oexe new <path>` starts one\n".to_string();
    }
...
fn render_node(out: &mut String, node: &PaneNode, machine: &Machine, depth: usize) {
    ...
    match node {
        ...
        PaneNode::Split { axis, ratio, a, b } => {
            let axis = match axis {
                osiris_core::core::machine::Axis::Horizontal => "h",
                osiris_core::core::machine::Axis::Vertical => "v",
            };
...
mod tests {
    ...
    use osiris_core::daemon::protocol::{PortEntry, ProcEntry};
    ...
    fn an_empty_machine_says_how_to_start() {
        ...
        assert!(rendered.contains("oexe new"), "{rendered}");
    }
    ...
    fn an_unnamed_tab_borrows_its_title_then_an_agent_then_a_place_then_its_process() {
        let view = |f: &dyn Fn(&mut TabView)| {
            let mut v = TabView {
                id: osiris_core::core::machine::TabId::new(),
                name: None,
                title: String::new(),
                osc_title: None,
                cwd: None,
        ...
        assert_eq!(
            tab_label(&view(
                &|v| v.agent = Some(osiris_core::core::cli_agent::CLIAgent::Claude)
            )),
            "Claude Code"
        );
        ...
        assert_eq!(
            tab_label(&view(&|v| {
                ...
                v.agent = Some(osiris_core::core::cli_agent::CLIAgent::Claude);
            })),
            "✳ fixing the switcher"
        );
        assert_eq!(
            tab_label(&view(
                &|v| v.osc_title = Some("user@host:~/repo/osiris".into())
            )),
            "osiris",
        ...
        assert_eq!(
            tab_label(&view(&|v| v.cwd = Some("/Users/me/repo/osiris".into()))),
            "osiris"
        );
        assert_eq!(
