...
use osiris_core::core::machine::{Machine, TabId, Workspace};
    ...
    match addr {
        ...
        WorkspaceAddress::Named(text) => {
            ...
            match by_prefix.as_slice() {
                [one] => Ok(one),
                [] => bail!("no workspace named '{text}' — `oexe ls` lists them"),
                many => bail!(
...
pub fn tab(machine: &Machine, addr: &TabAddress) -> Result<(WorkspaceId, TabId)> {
    match addr {
        TabAddress::Ordinal(n) => tab_index(machine)
            ...
            .ok_or_else(|| anyhow::anyhow!("no tab @{n} — `oexe ls` shows the @ numbers")),
        TabAddress::Id(text) => {
            for ws in &machine.workspaces {
                for tab in &ws.tabs {
...
pub fn workspace_of_pane(machine: &Machine, pane: u64) -> Result<&Workspace> {
    machine
        ...
        .ok_or_else(|| {
            anyhow::anyhow!("no pane %{pane} on this machine — `oexe pane ls` lists them")
        })
}
