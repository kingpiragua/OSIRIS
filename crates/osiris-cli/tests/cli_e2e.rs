...
use osiris_core::client::{ControlClient, PaneClient};
use osiris_core::daemon::control::ControlHello;
use osiris_core::daemon::protocol::{DaemonMsg, PROTOCOL_VERSION, ShellSpec, WinSize};

const DAEMON_ENV: &str = "OSIRIS_CLI_E2E_DAEMON";
const PASTE_AWARE_FIXTURE_ARG: &str = "--osiris-e2e-paste-aware-fixture";
const PASTE_AWARE_TEXT: &str = "osiris paste aware input";
...
fn main() {
    ...
    if std::env::var(DAEMON_ENV).as_deref() == Ok("1") {
        if let Err(e) = osiris_core::daemon::server::run_daemon() {
...
fn run_paste_aware_fixture() {
    ...
    println!("OSIRIS_PASTE_AWARE_READY");
    ...
    loop {
        ...
        for &byte in &chunk[..read] {
            if byte == b'\r' {
                ...
                if submitted {
                    println!("OSIRIS_PASTE_AWARE_SUBMITTED");
                } else {
                    println!("OSIRIS_PASTE_AWARE_NOT_SUBMITTED");
                }
...
impl Daemon {
    fn start() -> Daemon {
        ...
        let child = Command::new(own)
            .env(DAEMON_ENV, "1")
            .env("OSIRIS_CONFIG_DIR", dir.path())
            .env("OSIRIS_DATA_DIR", dir.path())
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("start the in-test osiris server");
        #[cfg(windows)]
    ...
    fn cli(&self, args: &[&str]) -> Command {
        let mut cmd = Command::new(env!("CARGO_BIN_EXE_oexe"));
        cmd.args(args)
            .env("OSIRIS_CONFIG_DIR", self.dir.path())
            .env("OSIRIS_DATA_DIR", self.dir.path())
            .env_remove("OSIRIS_PANE")
            .env_remove("OSIRIS_WS")
            .env_remove("OSIRIS_SOCKET");
        cmd
    }
    ...
    fn run(&self, args: &[&str]) -> Output {
        self.cli(args)
            .output()
            .unwrap_or_else(|e| panic!("could not run osiris {args:?}: {e}"))
    }
    ...
    fn run_json(&self, args: &[&str]) -> serde_json::Value {
        ...
        serde_json::from_str(&out)
            .unwrap_or_else(|e| panic!("osiris {args:?} --json printed no JSON ({e}): {out}"))
    }
}
...
/// "osiris-cli", which left a CLI-built workspace rebuilt from scratch the
...
fn run_streams_output_and_passes_the_exit_code(daemon: &Daemon) {
    let echo = one_shot("echo osiris_e2e_run_marker");
    ...
    assert!(stdout.contains("osiris_e2e_run_marker"), "{stdout}");
...
fn run_keep_files_the_pane_so_ls_shows_it(daemon: &Daemon) {
    ...
    args.extend(one_shot("echo osiris_e2e_keep_marker"));
...
fn config_dir_alone_resolves_both_endpoints(daemon: &Daemon) {
    let out = Command::new(env!("CARGO_BIN_EXE_oexe"))
        .args(["status", "--json"])
        .env_remove("OSIRIS_DATA_DIR")
        .env_remove("OSIRIS_CONTROL_SOCK")
        .env_remove("OSIRIS_PANE")
        .env_remove("OSIRIS_WS")
        .env_remove("OSIRIS_SOCKET")
        .env("OSIRIS_CONFIG_DIR", daemon.dir.path())
        .output()
        .expect("run oexe status with only OSIRIS_CONFIG_DIR");
    assert!(
        out.status.success(),
        "status over OSIRIS_CONFIG_DIR failed ({}): {}",
        out.status,
        String::from_utf8_lossy(&out.stderr)
    );
    ...
    assert_eq!(
        ...
        "the answer must come from the daemon OSIRIS_CONFIG_DIR names: {status}"
    );
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_oexe"))
        ...
        .env("OSIRIS_CONFIG_DIR", daemon.dir.path())
        .output()
        .expect("run oexe run with only OSIRIS_CONFIG_DIR");
    assert_eq!(
        out.status.code(),
        Some(9),
...
fn send_then_capture_round_trip(daemon: &Daemon) {
    ...
    daemon.run_ok(&["send", &address, "echo osiris_e2e_capture_marker", "--enter"]);

    let deadline = Instant::now() + SETTLE_WITHIN;
    loop {
        let seen = daemon.run_ok(&["capture", &address, "--scrollback"]);
        if seen.contains("osiris_e2e_capture_marker") {
            return;
        }
        assert!(
            Instant::now() < deadline,
...
fn send_enter_submits_in_a_paste_aware_raw_mode_tui(daemon: &Daemon) {
    ...
    collect_pane_output_until(&mut pane, b"OSIRIS_PASTE_AWARE_READY");

    let address = format!("%{}", pane.pane_id());
    daemon.run_ok(&["send", &address, PASTE_AWARE_TEXT, "--enter"]);

    collect_pane_output_until(&mut pane, b"OSIRIS_PASTE_AWARE_SUBMITTED");
}

fn collect_pane_output_until(session: &mut osiris_core::client::PaneSession, marker: &[u8]) {
...
/// `osiris … | head -1` must end the way `cat … | head -1` ends. Rust ignores
...
fn a_reader_that_hung_up_ends_the_pipeline_quietly(daemon: &Daemon) {
    ...
    let printer = one_shot("echo osiris_e2e_pipe_marker");
    ...
    for args in [vec!["ls"], streaming] {
        let mut child = daemon
            ...
            .stderr(Stdio::piped())
            .spawn()
            .unwrap_or_else(|e| panic!("could not spawn osiris {args:?}: {e}"));
        ...
        child
            ...
            .expect("reading osiris's stderr");
        let status = child.wait().expect("waiting for osiris");

        assert!(
            !stderr.contains("panicked"),
            "osiris {args:?} panicked when its reader hung up: {stderr}"
        );
        assert!(
            !stderr.to_lowercase().contains("broken pipe"),
            "a hung-up reader is how a pipeline ends, not something to report: \
             osiris {args:?} said {stderr}"
        );
        ...
        assert_ne!(
            status.code(),
            Some(1),
            "osiris {args:?} treated a hung-up reader as a failure: {stderr}"
        );
    }
}
...
fn capture_plain_returns_text_not_escapes(daemon: &Daemon) {
    ...
    daemon.run_ok(&["send", &address, "echo osiris_e2e_plain_marker", "--enter"]);
    ...
    loop {
        ...
        if plain.contains("osiris_e2e_plain_marker") && raw.contains('\r') {
            assert!(
                !plain.contains('\r'),
