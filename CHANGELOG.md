...
- **Give the prompt back to the shell** — a new **Settings → Input → Prompt →
  ...
  Turned off, osiris stops editing the shell prompt: every keystroke there —
  ...
  osiris, history traversal included. Previously the only way to get there was to
  hide the shell's own name from osiris so integration never armed. Shell
  integration is untouched by the switch: prompt boundaries, working directory,
  exit codes, notifications and `oexe procs` keep working. Tab completion and
  history search are menus osiris opens inside that editor, so both grey out
...
- **A zsh or fish you gave your own arguments to is no longer injected into.**
  Custom arguments have always been the line where osiris backs off — the bash,
...
- **An SSH pane is named after its host.** A fresh SSH tab reads the host's
  ...
  `~/.ssh/config` arrives nameless, and a window full of them all read `osiris`
...
- **A workspace reads its own name from the first frame, and stops changing
  ...
  `keen-marten` — and that is the name `oexe ws ls` prints and `oexe ws rename
  ...
  what `oexe new` leaves behind, still reads the directory it is working in, and
  a name you chose yourself still wins (#604).
- **File links open in osiris instead of leaving it.** A ⌘/Ctrl-clicked file path
...
- **A pane address may now be written without its `%`** — `oexe pane ls
  --json` prints bare ids, so `83` addresses the same pane as `%83`
  everywhere an address is taken. One behaviour goes with it: a lone `osiris
  ...
  the pane too (`oexe send %5 83 --enter`) (#538).
...
- **Orphan panes are visible in the GUI, and closable from it.** A shell
  left running after its workspace went away — what an interrupted `osiris
  run` leaves behind — showed up nowhere in the GUI; only the CLI's `osiris
...
- **`oexe pane close %99` fails when no pane 99 exists.** The orphan path
...
- **A mistyped "Start in" path is refused at save instead of silently
  ...
  in my project" read as an osiris bug rather than a typo. Settings now marks a
...
- **`oexe doctor` exits 1 when the server is unreachable.** Doctor is the
  verb people run when something is not working, so an unreachable server is
  *the* finding — not a row to exit 0 over while `oexe doctor || alert`
...
- **The `owner` field of `pane ls --all` is documented the same way in both
  ...
  literal `"osiris-cli"` owner, the behaviour that was removed because an owner
  names the workspace allowed to attach. Both now describe the workspace id,
  or its absence while a pane is unfiled (#591).
- **A failed `oexe wait` now says so on stderr even under `-q`.** Timeout and
...
- **A timed-out `oexe wait` now answers in the same JSON shape as a finished
...
- **A broken config.json can no longer be silently replaced by defaults** —
  ...
  no trace at all. (#537)
- **A mistyped `oexe send` address no longer types itself into your own
  pane** — `oexe send %3x --key C-c` used to degrade the unparseable `%3x`
...
- **`oexe send --enter` now presses Enter when there is nothing to type** —
  `--enter` is shorthand for `--key enter`, but it was never counted as a key,
  so `oexe send %42 --enter` answered "needs TEXT … or a --key to press"
  instead of running what pane 42 already had typed. It counts now, with an
  address or without one (`oexe send --enter` presses Enter in your own pane).
  An *unmarked* id is deliberately left out: `oexe send 83 --enter` reads as
...
- **`oexe wait` can wait for a command, not just an agent** — a new `free`
...
- **`oexe send --key` presses keys instead of typing characters** — `C-c` to
...
- **`oexe pane close` takes several panes, and `--orphans` clears the lot** —
...
- **`oexe doctor` reports where the agent status hooks stand** — it has claimed
  to check hooks in its own `--help` for a while without doing so. Missing or
  outdated hooks are why an agent can look frozen in `oexe agents` and why
  `oexe wait` on it only ever times out, so the check belongs in the verb
...
- **Put your own entries in the new-tab menu** — `custom_shells` in
  ...
  row in the menu behind the sidebar's **+**, after the shells osiris detected. A
  distro, a container, a REPL, the same shell against a different profile: they
  are launched exactly as written, since osiris chose none of the command and has
...
- **A Windows install for all users now updates itself** — previously an
  ...
  osiris with the user's own token. The update dialog says the prompt is coming
...
- **`oexe pane close --json` now reports `{"closed": [ids]}`** rather than a
...
- **Ctrl+C works in a pane on Windows again** — the daemon was created with
  ...
  so an osiris launched from a shell that already had the bit set is covered too.
  (#459, #451, #314)

- **Two osiriss on different config directories no longer erase each other's
...
- **The CLI and the GUI agree on what exists** — five places where a workspace,
  ...
  back under the same id and haunting the switcher until a restart; `oexe ls`
...
- **A pane the CLI created can be attached to** — a pane's owner names the
  workspace allowed to attach to it, and the CLI wrote a literal `osiris-cli`
...
- **A WSL pane starts when the distro's default shell is fish** — `wsl.exe --`
  ...
  `oexe wait` and busy/idle status dead in the pane. There is a fish arm now,
...
- **A remembered WSL server path repairs itself** — the note saying where a
  ...
  but restarting osiris. A bridge that closed without ever sending a byte never
...
- **Overlay scrollbars fade out again** — macOS reports that scrollbars should
  ...
  scrollbars, which take a gutter out of the layout; osiris's are overlay bars
...
- **Switching workspaces no longer wipes the tab titles of the one you left** —
  ...
  *process* name and never the terminal's title, so the naming fell through to
  ...
  than whichever shell happens to be first. `oexe tab ls` and workspaces listed
...
- **A WSL distro that cannot reach `/mnt` keeps its own `.bashrc`** — the
  ...
  without a word, and osiris starts it non-login precisely so that `--rcfile` is
  ...
  osiris's, and not their `.bashrc` or `.bash_profile` either. The arm now checks
...
- **An idle window stops re-reading the repository on Linux** — with a
  repository open, osiris ran `git status -uall` about two and a half times a
...
- **An ssh config import says what it did** — the button was silent three ways
  ...
  already current, and names the options osiris has no field for — `IdentityAgent`,
  `CertificateFile`, `IdentitiesOnly` and the rest — grouped with the hosts that
  set them, so a host that behaves differently under osiris than under `ssh` has
...
- **`oexe wait` no longer calls a busy shell `idle`** — a pane with nothing
  reporting agent status was reported as `idle`, so `oexe wait %3 --until idle`
...
- **Opening a folder from Explorer no longer costs you your layout** — "Open in
  osiris", "Open osiris here", and `osiris <PATH>` restore the last window's tabs and
...
- **An SFTP upload no longer sits in the browser under its temporary name** —
  an upload is written as `<name>.osiris-upload-<hex>` and renamed into place at
...
- **A failed update install says so instead of asking again** — the GUI hands
  the install to the `osiris-updater` helper and quits, so a failure inside the
...
- **Update osiris without leaving the app** — the launch check and
  **Settings → About → Check Now** now offer **Update and Relaunch** instead of
  sending the user to GitHub Releases. A dedicated `osiris-updater` helper
...
- **A proxy for osiris's own network traffic** — update checks, release downloads
...
- **`oexe wait` — the primitive an agent team was missing** — block until a
  pane's agent needs input, finishes its turn, or dies:
  `oexe wait %3 --until waiting,done --changed --timeout 600`. The daemon
  ...
  *previous* turn's state before the worker has even read the input; the
  ...
  stateless question composes into scripts (`oexe wait %3 && oexe capture %3
  --plain`), survives a server restart mid-wait, and needs no cursor. The
  cost is one aggregate request per tick — the same one `oexe agents` makes
  once. (by @yetone in #248)

- **An agent-facing skill for the CLI** — the repo carries
  [`skills/osiris`](skills/osiris/SKILL.md), which teaches a coding agent the
  ...
  `npx skills add kingpiragua/osiris` — osiris writes nothing into `~/.claude` for
...
- **Oh My Pi (`omp`) is a first-class agent** — the eighteenth CLI osiris
  ...
  **Settings → Agents** installs its hooks like the others.
  ...
  landing at `~/.omp/agent/extensions/osiris/index.ts`.
...
- **Every shell osiris can find is in the new-terminal menu, plus one you name** —
...
- **Windows Explorer context menus** — an optional **Open in osiris** verb for a
...
- **`osiris` opens directories in new tabs** — the CLI accepts one or more
...
- **The About page is about the app again** — it had grown three sections that
  ...
  registry write, and a daemon restart. The `osiris` CLI toggle moves to
  **Agents**, which already describes osiris ↔ agent integration in the other
...
- **Chinese terminology, corrected throughout** — the worst one collided two
  concepts: a git worktree was translated as 工作区, the same word osiris uses for
  ...
  osiris's own server is now written `server`, the way `shell`, `pane` and `agent`
...
- **macOS declares its privacy intents** — osiris shipped no
...
- **A Windows update that reported success and left the old build** — the
  ...
  `ensure_running` refuse to spawn a daemon mid-install, so a `osiris` CLI call
...
- **A newly installed command works in the next pane** — Windows hands every
  ...
  freshly installed command was unresolvable until osiris restarted. Windows
...
- **Windows panes can answer a background-color query** — the in-box conhost
  swallows a pane process's OSC 11 query, so it never reached osiris's emulator
  and no reply was written back: applications that pick a light or dark UI from
  the terminal background rendered a dark UI under a light theme. osiris already
...
- **A second cursor no longer blinks in the wrong place on Windows** —
  ...
  it, and the cursor blinks there until conhost's next frame moves it back. osiris
...
- **Windows toasts are osiris's, not PowerShell's** — notifications showed the
  PowerShell icon and name because osiris had no AUMID of its own registered. It
  now brands the process and refreshes the single per-user `osiris.lnk` Inno's
  ...
  toasts keep the PowerShell identity for half a minute after osiris writes one:
...
- **Windows panes are told whether the window is light or dark** — the daemon
  ...
  query before osiris's emulator can answer it. It was reading that from a dead
...
- **Scoop shims work again when osiris is launched from a hardened Windows
  ...
  it: when osiris detects the enforcing bit, it creates the daemon with
  ...
  desktop shell cannot be borrowed, in which case osiris logs a warning and
  starts degraded rather than not starting at all. Note the daemon's token on
  the clean-parent path derives from Explorer, so an elevated osiris starts a
...
- **A version-mismatched remote server is described as old, not as
  not-osiris** — a handshake refused on the control dialect was shown with the
  protocol layer's own wording: "java answered, but not as an osiris server:
  control peer (build …) speaks control v4, this build speaks v5". The far end
  *is* osiris; it is a build on the other side of a dialect bump, and the reader
...
- **Pasting a screenshot into a remote pane works** — the clipboard-image paste
  ...
  Staging is hardened on the remote side. `/tmp/osiris-clipboard-<user>` is a
  predictable name in a world-writable directory: any local account on the
  remote host could pre-create it, and the `Mkdir` result was discarded, so osiris
  ...
  Images now stage in `$HOME/.cache/osiris/clipboard` resolved from the session's
...
- **A right-click goes to the application when mouse reporting is on** — a TUI
  ...
  own right-button menus, and osiris was delivering one right-click to both
  consumers: the press was forwarded to the application *and* osiris's own context
  ...
  escape hatch that reaches osiris, the same override it already provides for
...
- **`$SHELL` names the shell the pane actually runs** — a pane configured to run
  ...
  `TERM`, the `OSIRIS_*` markers and `TERM_PROGRAM` but never touched `SHELL`, so
  ...
  shell osiris resolved — an argv-replacing integration injection and the
...
- **A forwarding `.bash_profile` no longer sources `.bashrc` twice** — the
  rcfile osiris hands bash replays the login-shell startup chain, but then sourced
...
- **The macOS locale fallback sets `LANG`, not `LC_CTYPE`** — when a pane
  ...
  osiris derives an installed UTF-8 locale and injects it — but as `LC_CTYPE`,
...
- **`oexe send` submits its Enter outside a paste burst**, so an agent does not
  read the newline as part of pasted text. (#322)

- **Closing a tab from the CLI terminates its panes.** (#319)

- **`osiris` diagnoses an unavailable agent hook** instead of failing silently.
  (#321)

- **osiris no longer panics on launch under Wayland on a slow VM** — the
...
- **A remote workspace is a window that is one machine, not a pane that
  ...
  Host** (or the workspace switcher) and osiris opens a window bound to that
  ...
  *that* machine rather than read off the client's disk. Before this, reaching
  ...
  ends. `osiris-server` — a new headless binary built on `osiris-core`, the crate
...
- **`osiris` is now a CLI, not just the app you launch by clicking an icon** —
  a new scriptable command-line client, built for coding agents to drive
  panes without a window in the way. The GUI binary is renamed `osiris`
  (bundle id, icons and display name unchanged); `osiris`, the name you type,
  is now the CLI. Both are clients of the same `osiris-server` a remote
  ...
  | Global | `-m <machine>` routes over the server's existing SSH/WSL links; `--json` everywhere; implicit context from `OSIRIS_PANE` / `OSIRIS_WS` / `OSIRIS_SOCKET`, injected into every spawned shell |
  ...
  two writers on one pane could leave it looking frozen. `osiris_core::client`
  ...
  release, so there was no previous `osiris attach` to remove out from under
  ...
  `osiris` launching the GUI) says so in `--help` rather than only failing at
...
- **The CLI ships in every install and lands on PATH automatically** —
  ...
  scripts copied only `osiris`, and the upload glob never reached the CLI
  ...
  server, which inherits it from the GUI — can run `osiris` immediately. It
  writes nothing to disk and cannot fail. The **on-disk half** covers typing
  `osiris` in some other terminal, and is allowed to fail safely: on Unix it
  ...
  lookup (`InstalledShadowed` names whichever `osiris` beats it on PATH).
  Debug builds and cargo build trees only get the environment half, so a
  `cargo run` can't repoint your real `osiris` at a binary the next `cargo
...
- **Pi is a first-class agent, not a fallback one** — Pi panes drew the generic
  ...
  The mark is Pi's own, from pi.dev, rescaled to osiris's 24x24 icon grid — its
  `prefers-color-scheme` stylesheet dropped, since these avatars are tinted by
  the app. Restoring a Pi pane also resumes its conversation now: the osiris
...
- **Fork an agent session** — branch a live agent conversation into a second,
  ...
  that got you there. OSIRIS panes the agent's *own* fork command rather than
  ...
  Agents with no fork osiris could verify simply don't offer the action, rather
...
- **Your WSL distributions are machines you can open a workspace on** — the
  ...
  (`wsl.exe -d <distro> -- osiris-server --stdio`: no SSH, no address, no
  ...
  A distribution is served the Linux `osiris-server` this client shipped with
  rather than one downloaded from a release, so the first connect writes it into
  `~/.local/share/osiris/bin` inside the distro — with the same one-time
...
- **The machine that runs your panes now owns their layout** — the workspace,
  ...
  - **Saved layouts do not carry over.** The tree is a new file
    (`~/.local/share/osiris/machine.json`) and the old `session.json` is not read;
...
- **A remote workspace no longer loses or crosses its own layout on reopen
  ...
  `osiris-server` had been replaced, could land on permanently `disconnected`
...
- **A captured pane reads like text, not a stream of raw escape codes, and a
  broken pipe ends quietly** — `oexe capture --plain` replays a pane's bytes
  ...
  verbs produced on something as ordinary as `oexe ls | head -1`. Rust
...
- **The IME candidate window follows the actual caret, not a parked
  ...
  older `CANDIDATEFORM` call osiris was already making once per composition —
  ...
  landing the only real anchor osiris had at the right edge of the input box's
...
- **osiris gets a taskbar icon and identity on Linux** — the window now sets
  `app_id: "osiris"` on every platform, which X11 uses as `WM_CLASS` and
  Wayland matches against the packaged `osiris.desktop` entry, so taskbars and
...
- **⌃R on a remote machine searches that machine's history** — osiris owns ⌃R at
  ...
  workspace osiris also reads the far end's own `~/.zsh_history` and
...
- **A Windows clipboard pastes like every other clipboard** — text copied on
  ...
  blank line under every line — bad enough to make osiris unusable for editing.
...
- **Rounded UI controls no longer square off their corners**
  ([#236](https://github.com/kingpiragua/osiris/issues/236)) — the cursor-shape
...
- **Return no longer confirms the file tree's delete prompt** — it was the only
  destructive prompt in osiris with the destructive action first, and on macOS
...
- **Panes are told which terminal they're running in** — every pane now carries
  `TERM_PROGRAM=osiris` and `TERM_PROGRAM_VERSION`, the de-facto standard pair
  ...
  conservative behaviour. osiris's own `OSIRIS` marker doesn't help them — it exists
...
- **One fewer duplicate SVG stack in the build** — the resvg 0.47 bump (#227)
  ...
  stacks. The fork now pins 0.47 too, re-unifying its stack with the one osiris
  uses for the tray icon. (#237)

- **The last duplicate SVG stack is gone** — after #227 and #237 unified osiris
...
- **Box-drawing characters no longer break into dashes between rows** — they
  ...
  Alacritty, WezTerm and iTerm2 all ship, and what osiris's Powerline separators
...
- **Tab completes remote paths in an SSH pane** — path candidates came off the
  ...
  osiris-owned connection to ask. (#217)
...
- **Foreground applications can negotiate the Kitty keyboard protocol** — the
  ...
  enhancement fell back to `modifyOtherKeys`, which osiris doesn't implement. That
...
- **Interaction state and status colours are derived from the active theme**
  ([#197](https://github.com/kingpiragua/osiris/issues/197)) — a segmented control's
...
- **Splitting or un-maximizing a pane you were hovering no longer kills the app**
  ([#201](https://github.com/kingpiragua/osiris/issues/201)) — a pane remembers the
...
- **Emoji written with a variation selector take their real width and
  presentation** ([#203](https://github.com/kingpiragua/osiris/issues/203)) — `🗂️`,
...
- **A config file saved with a UTF-8 BOM no longer wipes your settings** — every
  ...
  an absent one, and osiris came up on defaults with nothing in the log to explain
...
- **New tabs and splits open in the right directory even when the shell can't be
  instrumented** ([#187](https://github.com/kingpiragua/osiris/issues/187)) — a pane
  learned its directory from `OSC 7`, which only shells osiris injects its
...
- **Links open on Ctrl+click on Windows and Linux**
  ([#183](https://github.com/kingpiragua/osiris/issues/183)) — the link modifier was
...
- **Option-as-Meta works with a CJK input source**
  ([#177](https://github.com/kingpiragua/osiris/issues/177)) — macOS gives Option two
...
- **Grok Build gets the full hook integration** — Settings → Agents grows a
  sixth row, installing osiris's hooks to `~/.grok/hooks/osiris.json`. A grok pane
...
- **Edit, File and Help menus** — copy/cut/paste/select-all, undo/redo and
  find/find-next now live in a real Edit menu; `About osiris`, `Check for
  Updates…`, `Hide osiris` and Services sit in the app menu; Docs, Discord and
...
- **The menu bar follows the HIG** — `osiris · File · Edit · View · Window ·
  Help` replaces `osiris · Shell · Window · View`. The `Shell` menu is gone: its
...
- **Non-ASCII output survives a Finder/Dock launch** — macOS GUI launches can
  ...
  `LC_*` by default.
  ([#178](https://github.com/kingpiragua/osiris/issues/178), #173, #180)
...
- **Tab completion falls through to the shell** — a Tab osiris has no
...
- **Per-tab context menu with worktree tabs** — right-clicking a tab chip or
  ...
  `.osiris/worktrees/<name>` (kept out of `git status` by a self-ignoring
...
- **README rewritten as a minimal index** — feature details, keybindings,
  ...
  tagline now positions osiris as a terminal workbench.
...
- **Click a sidebar git line to open a working-tree diff overlay** — the
  ...
  live. (#92)
- **Window size and position are remembered across launches** — osiris saves
...
- On Windows, a pane no longer hangs open when its shell exits on its own.
  ...
  output pipe never reports EOF on a natural exit — and osiris detected a shell's
...
- Nerd Font prompt icons no longer render sliced off on the right. A non-Mono
  ...
  ink up to ~1.9 cells wide, and osiris clipped every lone glyph to exactly one
...
- New tabs and splits no longer stall for seconds while a zsh plugin manager
  reinstalls itself. osiris launches zsh through a throwaway `ZDOTDIR` (so it can
  ...
  dropped osiris's integration. (#15)
...
- Tab completion (and other line editing) now stays out of the way over `ssh`.
  ...
  most visibly, which ships OSC 133 on by default — used to engage osiris's
  *local* line editor, so Tab ran completion against the local machine's
  filesystem instead of reaching the remote shell. osiris now only drives the
...
- The "+" button now opens a shell picker: osiris detects the shells installed
...
- Fish, nushell, pwsh and other shells installed by Homebrew or nix now appear
  ...
  entries. (#18)
- Upgrading osiris while an older daemon is still running in the background no
...
- Windows releases now ship an Inno Setup installer
  (`osiris-<version>-windows-x86_64-setup.exe`) alongside the portable zip. It
...
- Startup update check: osiris asks GitHub once, in the background, whether a
  ...
  About. Both open the Releases page; osiris never downloads or updates itself —
...
- Windows: launching osiris no longer opens a stray console window behind the
...
[26.8.3]: https://github.com/kingpiragua/osiris/compare/v26.8.2...v26.8.3
[26.8.2]: https://github.com/kingpiragua/osiris/compare/v26.8.1...v26.8.2
[26.8.1]: https://github.com/kingpiragua/osiris/compare/v26.8.0...v26.8.1
[26.8.0]: https://github.com/kingpiragua/osiris/compare/v26.7.6...v26.8.0
[26.7.6]: https://github.com/kingpiragua/osiris/compare/v26.7.5...v26.7.6
[26.7.5]: https://github.com/kingpiragua/osiris/compare/v26.7.4...v26.7.5
[26.7.4]: https://github.com/kingpiragua/osiris/compare/v26.7.3...v26.7.4
[26.7.3]: https://github.com/kingpiragua/osiris/compare/v26.7.2...v26.7.3
[26.7.2]: https://github.com/kingpiragua/osiris/compare/v26.7.1...v26.7.2
[26.7.1]: https://github.com/kingpiragua/osiris/compare/v26.7.0...v26.7.1
[26.7.0]: https://github.com/kingpiragua/osiris/compare/v0.17.0...v26.7.0
[0.10.0]: https://github.com/kingpiragua/osiris/compare/v0.9.0...v0.10.0
[0.9.0]: https://github.com/kingpiragua/osiris/compare/v0.8.0...v0.9.0
[0.8.0]: https://github.com/kingpiragua/osiris/compare/v0.7.0...v0.8.0
[0.7.0]: https://github.com/kingpiragua/osiris/compare/v0.6.2...v0.7.0
[0.6.2]: https://github.com/kingpiragua/osiris/compare/v0.6.1...v0.6.2
[0.6.1]: https://github.com/kingpiragua/osiris/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/kingpiragua/osiris/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/kingpiragua/osiris/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/kingpiragua/osiris/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/kingpiragua/osiris/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/kingpiragua/osiris/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/kingpiragua/osiris/releases/tag/v0.1.0
