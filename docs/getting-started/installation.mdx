...
[**GitHub Releases**](https://github.com/kingpiragua/osiris/releases). There is no
...
<Tabs>
  <Tab title="macOS">
    Download the DMG that matches your Mac and drag **osiris** into Applications.

    | Mac | File |
    |---|---|
    | Apple silicon (M1 and later) | `osiris-<version>-macos-arm64.dmg` |
    | Intel | `osiris-<version>-macos-x86_64.dmg` |
  ...
  <Tab title="Windows">
    ...
    | File | Use it when |
    |---|---|
    | `osiris-<version>-windows-x86_64-setup.exe` | You want a normal install with Start-menu entries and an uninstaller. |
    | `osiris-<version>-windows-x86_64.zip` | You want it portable — unzip anywhere and run `osiris.exe`. |

    The installer offers one optional setup task, off by default: **Add "Open in
    osiris" to the folder context menu**. It writes shell verbs under `HKCU`, so
    ...
    ```powershell
    osiris.exe --register-explorer-menu
    osiris.exe --unregister-explorer-menu
    ```

    <Note>
      The Windows package also carries a Linux `osiris-server` binary so a WSL
  ...
  <Tab title="Linux">
    | File | Use it when |
    |---|---|
    | `osiris-<version>-linux-x86_64.AppImage` | Almost always. `chmod +x` and run — the X11, Wayland, xkb, and font libraries are bundled, so it works on Fedora, Arch, Debian and friends, not just Ubuntu. |
    | `osiris-<version>-linux-x86_64.tar.gz` | You would rather unpack the plain binary and place it yourself. |

    ```bash
    chmod +x osiris-*-linux-x86_64.AppImage
    ./osiris-*-linux-x86_64.AppImage
    ```
  </Tab>
</Tabs>
...
Every installer ships the `osiris` CLI beside the app, and the app puts it on your
...
A `osiris` you installed yourself — a `cargo install` build, a package manager's
copy — is never replaced. To turn the whole thing off, uncheck **Settings →
Agents → Install the osiris command on PATH**.

<Tip>
  Inside an osiris pane the CLI works regardless of PATH, because panes inherit the
  app's environment.
</Tip>

## Updating

osiris checks for updates every six hours and can update itself: **Settings →
...
<Tabs>
  <Tab title="macOS / Windows">
    ```bash
    git clone https://github.com/kingpiragua/osiris
    cd osiris
    cargo build --release
    ```
  </Tab>
...
<AccordionGroup>
  <Accordion title="macOS">
    Quit osiris (use **Quit and Stop Server** from the tray menu so the background
    server stops too), then drag the app to the Trash. Your settings live in
    `~/.config/osiris` and are left alone; delete that folder to remove them.
  </Accordion>
