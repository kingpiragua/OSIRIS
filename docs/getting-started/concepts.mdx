...
Four words explain most of osiris. Three of them you can see; the fourth is the
reason the other three survive a reboot.

## Pane

A **pane** is one terminal: one shell (or one program) attached to one PTY. It
is the only thing in osiris that actually runs something.

Panes have stable ids — `%42` — for their whole life. That id is what the
[CLI](/cli/overview) addresses, and what `$OSIRIS_PANE` holds inside the pane
itself.
...
label is the best evidence osiris has: a name you set, else the coding agent
...
Workspaces are how osiris keeps ten repositories from becoming forty
...
<Frame caption="Placeholder — screenshot: the workspace switcher, workspaces on the left, that workspace's tabs on the right">
  <img src="/images/placeholder.svg" alt="The osiris workspace switcher" />
</Frame>
...
Quitting osiris closes the window and leaves that server running. Your build keeps
building, your agent keeps working, your SSH session stays up. Open osiris again
and it reattaches to exactly what was there.

This is also why:

- **`osiris` works from any terminal.** The CLI talks to the same server. The GUI
...
| | Close a tab | Quit osiris | Stop the server | Reboot |
|---|:--:|:--:|:--:|:--:|
...
|---|---|
| `OSIRIS_PANE` | This pane's id — the default target of `oexe split`, `send`, `capture`, `procs`. |
| `OSIRIS_WS` | This pane's workspace id. |
| `OSIRIS_CONFIG_DIR` | The config directory, which is how the CLI finds the right server. |

`echo $OSIRIS_PANE` is the fastest way to tell whether you are inside osiris at all.
