...
Open osiris and you get a window with one tab and one shell, and a tab sidebar
...
<Frame caption="Placeholder — screenshot: the Settings window, Appearance section">
  <img src="/images/placeholder.svg" alt="osiris Settings" />
</Frame>
...
osiris then follows the OS appearance live.
...
**Start in** decides what a *fresh* shell opens in: osiris's launch directory
...
**Settings → Agents.** osiris detects 18 coding CLIs by process name on its own —
you get brand avatars and tab labels for free. The *status dots*, the "needs
your permission" notifications, and `oexe wait` all need one more thing: a small
...
Your shells, builds, and agent turns keep going, and reopening osiris reattaches
to them.
...
**Settings → Window & Tabs → Notifications.** By default osiris posts a desktop
...
| | macOS / Linux | Windows |
|---|---|---|
| Settings file | `~/.config/osiris/config.json` | `%APPDATA%\osiris\config.json` |
| Custom themes | `~/.config/osiris/themes/` | `%APPDATA%\osiris\themes\` |
...
Set `OSIRIS_CONFIG_DIR` to move the whole directory somewhere else.
