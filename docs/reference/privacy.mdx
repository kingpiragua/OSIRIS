...
description: "What macOS asks you, why, and what osiris itself holds."
---

## Why macOS asks osiris for permission

Panes are forked from osiris's own bundled executable, so when a program you run
asks macOS for a protected resource, macOS attributes the request to **osiris.app**
— not to the program.

If osiris declared no usage strings, that request would be **denied outright with
no prompt at all**, and the program would look broken for no visible reason.

So osiris declares the matching usage strings, and you get the normal one-time
prompt:
...
<Warning>
  **Declaring a usage string is not the same as holding the permission.**
  osiris.app itself is granted none of these. Every prompt you see belongs to
...
| **Remote server installs** | Downloading a `osiris-server` binary for a machine you connected to — or, for WSL, copying the one already bundled with your install. |
...
| | |
|---|---|
| Settings, themes, window state | `~/.config/osiris/` (`%APPDATA%\osiris\` on Windows) |
