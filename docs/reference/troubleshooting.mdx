...
- Check **Settings → Agents → Install the osiris command on PATH** is on.
...
- A `osiris` you installed yourself is never replaced, so an old one earlier in
  PATH will win.

Inside an osiris pane it works regardless, since panes inherit the app's
environment.

## The server is unreachable

`oexe doctor` says so, and the GUI cannot open panes.

Start it with `oexe server start`. If you are an agent or a script,
...
```bash
OSIRIS_LOG=info   # must be set before the server starts
oexe server logs
```
...
unavoidable. The *screens* should come back: osiris keeps a capped tail of each
...
osiris updated in place, so the app is new and your panes are still served by the
...
|---|---|
| *"running an old osiris server that this copy cannot talk to"* | The server there predates your client's protocol. Let osiris update it — this ends every session on that machine. |
| *"running a newer osiris server than this copy"* | Update osiris here instead, or replace the server there. |
| *"answered, but not as an osiris server"* | Something else is listening, or the binary is not what osiris expects. |
| *"osiris no longer has a way to reach &lt;machine&gt;"* | The SSH link dropped. Reconnect from the switcher. |

`osiris -m <machine>` never dials a fresh connection by design — it uses a link
...
If the file cannot be parsed, osiris starts on defaults and keeps your original at
...
## `oexe capture … | head -1` printed a Rust panic
...
<CardGroup cols={2}>
    <Card title="Report an issue" icon="github" href="https://github.com/kingpiragua/osiris/issues/new">
    Include `oexe doctor` output and your platform.
  </Card>
</CardGroup>
