---
title: "config.json"
description: "Every key osiris reads, its type, and its default."
---

| | |
|---|---|
| macOS / Linux | `~/.config/osiris/config.json` |
| Windows | `%APPDATA%\osiris\config.json` |
| Override the whole directory | `OSIRIS_CONFIG_DIR` |
...
| Key | Type | Default | |
|---|---|---|---|
| `font_family` | string | `"Geist Mono"` | Primary face. Geist Mono and Hack are both bundled. |
...
| Key | Type | Default | |
|---|---|---|---|
| `theme_preset` | string | `"osiris"` | Active theme id — `osiris`, `eye_of_horus`, `signal_bleed`, `pale_horse`, `flyerhead`, or any of the nine upstream ids. |
| `theme_follow_system` | bool | `false` | Follow the OS appearance. |
| `theme_preset_light` | string | `"osiris"` | Used when following the system. |
| `theme_preset_dark` | string | `"osiris"` | Used when following the system. |
...
| `crt_scanlines` | bool | `false` | Phosphor scanlines over the grid — one line every three device pixels at 4% of the foreground, painted over everything and never animated. Off by default: readability of the text under it outranks the atmosphere over it. |
...
| Key | Type | Default | |
|---|---|---|---|
| `prompt_editor` | bool | `true` | osiris edits the shell prompt. Off sends every key at the prompt to the shell, so its own line editor (ZLE, readline) owns editing. Shell integration stays on. |
| `tab_completion` | bool | `true` | osiris's completion menu on <kbd>⇥</kbd>. Off hands the key to the shell. Needs `prompt_editor`. |
| `history_search` | bool | `true` | osiris's fuzzy history on <kbd>⌃ R</kbd>. Off hands the key to the shell. Needs `prompt_editor`. |
...
| `link_file_open` | string | `internal` | What a file link opens: `internal` (osiris's editor, at the line), `system` (the OS file association), `command`. A file on another machine always uses `internal`. |
...
| `install_cli_on_path` | bool | `true` | Put the bundled `osiris` command on PATH at launch. |
...
| `http_proxy` | string | — | For osiris's *own* traffic only — update checks, downloads, remote-server installs. `http://…` or `socks5://…`. Programs in a pane are unaffected. |

[Updates →](/reference/updates)

## Keys osiris manages itself
...
<Note>
  If the file cannot be parsed, osiris starts on defaults, keeps your original at
