...
description: "What osiris injects into your shell, and what it buys you."
---

A terminal that only sees bytes cannot tell a prompt from output, or a finished
command from a hung one. osiris's shell integration closes that gap: the shell
...
|---|---|
| **zsh** | A throwaway `ZDOTDIR` whose files source yours first, then osiris's. Your `OSIRIS_USER_ZDOTDIR` is preserved. |
...
`OSIRIS_SHELL_INTEGRATION` is set once it is active, and guards against a second
injection when shells nest.

<Note>
  A shell launched with arguments *you* wrote — `shell.args`, or a
  `custom_shells` entry — is left alone, because osiris's injection would
  conflict with the flags you chose. Arguments osiris's own detection supplied
...
| Editing mode (vi / emacs) | `OSC 133;V` | Matching osiris's key handling to your shell's mode |
| Window title | `OSC 0` | Tab labels. Only PowerShell is given this — zsh, bash, and fish already set a title of their own, and osiris reads whatever they emit |

## What turns off without it

Run a shell osiris does not integrate with, and everything below still works —
...
- Exact working-directory tracking (osiris falls back to inspecting the process)
