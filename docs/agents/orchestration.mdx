...
another. osiris gives that loop a primitive instead of leaving it to screen
scraping.
...
oexe wait "$PANE" --until waiting,done --changed --timeout 600

# 4. read what happened
oexe capture "$PANE" --plain
...
<Warning>
  ...
  own root process *is* the command — what `oexe run` spawns — looks free the
  whole time it runs; wait on `oexe run` itself instead, it already blocks. And
...
```bash
oexe wait "$PANE" --until waiting --changed   # it needs something
oexe capture "$PANE" --plain | tail -20       # see what it is asking
oexe send "$PANE" --key down --key enter      # answer it
oexe send "$PANE" --key C-c                   # or stop it altogether
```
...
[full vocabulary](/cli/reference#osiris-send-pane-text-enter-key-key) is in the
reference.

## When an agent never moves

If `oexe wait` times out and `oexe agents` shows a status that never changes,
the likely cause is that the agent's status hooks are not installed or are out
of date — the agent is working fine, it just has no way to say so. `oexe doctor`
reports where every agent's hooks stand, and `oexe agents` names the specific
one when it can see the gap.

## Watching everything at once

```bash
oexe agents          # every agent on the machine: pane, agent, status, message
oexe agents --json   # the same, parseable
```
...
osiris installs nothing into `~/.claude` for it — no switch in **Settings →
...
```bash
npx skills add kingpiragua/osiris
```
...
- **Never `send` into a pane you did not open.** Check `oexe agents` first.
- **Never close a pane, tab, or workspace you did not create.**
- **Never `server stop` or `server restart`.** Every pane on the machine dies
  with it.
- **Clean up what you did create** — `oexe pane close %83` when you are done.
  An interrupted `run` leaves its pane behind; `oexe pane ls --all` shows those,
  and `oexe pane close --orphans` clears them. That last one is a human's
