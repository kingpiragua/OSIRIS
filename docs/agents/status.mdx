...
waiting for permission look identical from outside. osiris fixes that by letting
...
becomes **Reinstall** — or **Update**, against an **Outdated** state, when osiris
ships a newer hook.

<Note>
  The hooks only do anything inside osiris. Running the same agent in another
...
The same three states are what `oexe agents` reports as `working` / `waiting` /
`done`, and what [`oexe wait`](/agents/orchestration) blocks on. A fourth state,
...
osiris keeps a status item in the system tray (menu bar on macOS). It flips to an
...
If nothing recognisable is running, osiris says *"No running coding agent found"*
