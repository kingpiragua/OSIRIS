---
title: "Coding agents"
description: "What osiris does around Claude Code, Codex, and 16 others — without ever wrapping them."
---

osiris recognises coding agents running in a pane and builds around them. It does
not wrap them, proxy them, or replace their interface: the agent you start is
the agent you get, running in a normal PTY, with its own UI. osiris adds the
...
<Frame caption="Placeholder — screenshot: a sidebar of agent sessions across several repos, each with a brand avatar and status dot">
  <img src="/images/placeholder.svg" alt="Agent sessions in the osiris sidebar" />
</Frame>
...
itself, over a channel osiris installs into that agent's configuration. It powers
the status dots, the notifications, the tray icon, and `oexe wait`.
...
<CardGroup cols={3}>
  ...
  <Card title="Orchestration" icon="diagram-project" href="/agents/orchestration">
    One agent driving another with `oexe wait`.
  </Card>
</CardGroup>
