...
osiris puts an editor at the shell prompt. Nothing to install, no plugin to source
...
<Frame caption="Placeholder — video: typing a command with a ghost suggestion, accepting it, then opening the completion menu">
  <img src="/images/placeholder.svg" alt="The osiris prompt" />
</Frame>
...
When osiris has nothing useful to offer, the <kbd>⇥</kbd> falls through to your
...
The prompt features arrive through osiris's shell integration, which is injected
...
readout, the "command finished" notification, and `oexe procs` are built on.
...
it does outside osiris:

```zsh
# ~/.zshrc — works at an osiris prompt with the editor off
...
Settings while it is off — both are menus osiris opens inside that editor, so
...
branch readout, `oexe procs`. The setting moves the line editor, not the
integration.
