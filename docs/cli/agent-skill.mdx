...
description: "Teaching a coding agent to use osiris properly — including when not to."
---
...
The source lives at
[`skills/osiris/`](https://github.com/kingpiragua/osiris/tree/main/skills/osiris) in the
repository: a `SKILL.md` and a full command reference.

<Note>
  This is the only skill osiris has — nothing in **Settings → Agents** installs
  one for you. Using the CLI to run *other* agents — a worker pane, `oexe wait`,
...
command, and the skill is named `osiris`:

```bash
npx skills update osiris    # this one
...
has drifted. `npx skills remove osiris` takes it back out.
...
- **Never `send` into a pane you did not open.** Keystrokes land in the middle
  of whatever is happening there. Check `oexe agents` first.
...
- **Never `oexe server start` on your own initiative** when `doctor` says the
  server is unreachable — starting one the user did not ask for changes what
  their GUI attaches to. Tell them instead.
- **Clean up what you did create.** `oexe pane close %83` when the scratch pane
  is done with.
...
oexe wait %3 --until waiting,done --changed --timeout 600
```

## For humans writing their own tooling

The same material is worth reading even if you are not an agent — it is the
shortest description of how to use osiris as a job runner. Start with the
