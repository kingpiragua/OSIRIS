...
Set inside every osiris pane, inherited by anything launched from one.

| Variable | Meaning |
|---|---|
| `OSIRIS_PANE` | This pane's id, e.g. `71` or `%71` (both accepted). Default target of `split`, `send`, `capture`, `procs`, `wait`, `pane close`. |
| `OSIRIS_WS` | This pane's workspace id. Default for `run --keep`, `tab new`, `tab ls`, `ws tree`. |
| `OSIRIS_CONFIG_DIR` | The server's config dir — how the CLI finds the right server. You never pass a socket path. |

Outside an OSIRIS pane, address-taking verbs fail with
...
|---|---|
| `0` | Success |
| `1` | The command failed; one line on stderr, prefixed `osiris:` |
| `2` | Usage error — unknown verb, missing argument, bad type |
| `124` | `oexe wait` timed out (the `timeout(1)` convention) |
...
- `--keep` leaves the pane alive as a new tab afterwards. Needs a workspace, so
  it requires `--ws` or `$OSIRIS_WS` — without one it is an error, not a silent
  fallback.
...
nothing, so `oexe agents` shows it standing still and `oexe wait` sits there
...
prefix `oexe ls` prints). An ambiguous name or prefix is an error that lists the
candidates.
...
leaves an empty workspace you then have to populate, while `oexe new --json`
