---
title: "The osiris command"
description: "Driving the workbench from a script, a Makefile, or another agent."
---

`oexe` is a thin, non-interactive client of the OSIRIS server. Every verb runs and
...
any terminal on the machine. [Installation →](/getting-started/installation#the-osiris-command)
...
`OSIRIS_CONFIG_DIR` / `OSIRIS_WS` / `OSIRIS_PANE` are set — that is, whether you are
running *inside* an osiris pane.

Being inside a pane matters because the address-taking verbs (`split`, `send`,
`capture`, `procs`, `wait`, `pane close`) default to `$OSIRIS_PANE`, and
`run --keep` files its pane into `$OSIRIS_WS`. Outside one you must name a target,
...
```bash
oexe run -- cargo test           # streams to stdout, exits with cargo's code
oexe run --cwd /path -- make
oexe run --keep -- cargo build   # leaves the pane behind as a new tab
```
...
<Note>
  Everything after `--` belongs to the child: `oexe run -- cargo test --keep`
  passes `--keep` to cargo, not to osiris.
</Note>
...
```bash
PANE=$(oexe split --v)                    # or --h; prints "%83"
...
```bash
oexe new --json /path/to/repo    # {"id": "...", "pane": 83}
```
...
For agents specifically, use [`oexe wait`](/agents/orchestration) instead of
polling.
...
oexe ws tree api     # one workspace as a tree
oexe pane ls --all   # every pane, including orphans no workspace holds
oexe agents          # every coding agent and its status
oexe status          # server pid, uptime, pane count, build, socket
oexe machine ls      # this machine plus any linked remotes
oexe events          # stream server events until interrupted
```
