...
| Setting | Default | |
|---|---|---|
| **Font family** | Geist Mono | Or anything installed on your system |
...
## Two faces ship inside the binary

**Geist Mono** is the default — the terminal voice OSIRIS.EXE is written in,
under the SIL Open Font License. **Hack** stays embedded behind it as the
fallback anchor, so a config that names Hack keeps working and the chain always
ends on a face that cannot be missing.

Neither depends on a system install: a fresh machine renders exactly like the
one you set up last year, and both carry the block glyphs the home wordmark and
the block cursor are drawn from.
...
install it and osiris picks it up with no config change.
