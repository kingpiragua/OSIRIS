...
further: osiris runs a server on the far machine, and the whole workbench points
...
<Frame caption="Placeholder — screenshot: a remote workspace open, sidebar showing remote repos, with the machine name in the strip">
  <img src="/images/placeholder.svg" alt="A remote workspace in osiris" />
</Frame>
...
<Steps>
  ...
  <Step title="Pick a machine">
    osiris connects over the same SSH stack as everything else, so profiles,
  ...
  <Step title="Approve the server install, once">
    ...
    > osiris will write its server binary to *devbox* so this machine can host
...
| | |
|---|---|
| **What** | A single self-contained `osiris-server` binary |
| **Where** | `~/.local/share/osiris/bin/osiris-server-c<control>p<protocol>` |
...
that disagree never quietly half-work — osiris installs the matching one instead.
...
<AccordionGroup>
  <Accordion title="Update osiris's server on “devbox”?">
    The machine is serving sessions from a build whose protocol this client
    cannot speak. osiris has already installed a matching server, but the one
  ...
  <Accordion title="Restart osiris's server on “devbox”?">
...
<Warning>
  Both of these end other people's work if the machine is shared. osiris spells
...
- **Fork an agent session.** The fork command would run against the *local*
  agent, so osiris does not offer it.
...
```bash
oexe machine ls              # this machine plus every link the server holds
osiris -m devbox ls            # route any command to a linked machine
