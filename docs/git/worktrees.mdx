...
working tree. A git worktree is the fix, and osiris makes it a single dialog.
...
Confirm and osiris creates the worktree, opens a tab in it, and starts a shell
...
```
<repo>/.osiris/worktrees/<name>
```

`<repo>/.osiris/.gitignore` is created with `*` in it the first time, so the
