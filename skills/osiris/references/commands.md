...
Set inside every osiris pane, inherited by anything you launch from one.

| Variable | Meaning |
|---|---|
| `OSIRIS_PANE` | This pane's id, e.g. `71` or `%71` (both forms are accepted). The default target of `split`, `send`, `capture`, `procs`, `wait`, `pane close`. |
| `OSIRIS_WS` | This pane's workspace id. The default for `run --keep`, `tab new`, `tab ls`, `ws tree`. |
| `OSIRIS_CONFIG_DIR` | The server's config dir. How the CLI finds the right server's sockets — you never pass a socket path. |

Outside an OSIRIS pane the address-taking verbs fail with
`not inside an OSIRIS pane — pass an explicit %pane/@tab/workspace`.
...
|---|---|
| 0 | success |
| 1 | the command failed; the reason is one line on stderr, prefixed `osiris:` |
| 2 | usage error (clap) — unknown verb, missing argument, bad type |
| 124 | `oexe wait` gave up — the `timeout(1)` convention, so "not yet" is distinguishable from "broken" |
...
| *other* | only from `oexe run`, which passes the child's exit code through |
...
### `oexe run [--keep] [--cwd DIR] [--ws WORKSPACE] -- CMD...`
...
the child, so `oexe run -- cargo test --keep` passes `--keep` to cargo.

- `--keep` leaves the pane alive as a new tab afterwards. It needs a workspace,
  so it requires `--ws` or `$OSIRIS_WS`; without one it is an error, not a
  silent fallback.
- `--cwd` sets the working directory. `--ws` also sets the pane's `OSIRIS_WS`.
...
### `oexe split [%PANE] (--v|--h) [--ratio R]`
Alias of `pane split`. Splits `%PANE` (default `$OSIRIS_PANE`), spawning a shell
...
### `oexe send [%PANE] [TEXT] [--enter] [--key KEY]…`
...
none, so `oexe send %42 --enter` runs whatever pane 42 already has typed. With
one argument the text is the argument and the pane comes from `$OSIRIS_PANE` —
...
to do. `--enter` is that key only for the `%`-marked spelling: `oexe send 83
...
type an address-shaped string, name the pane as well: `oexe send %42 %3x`.
...
### `oexe capture [%PANE] [--plain] [--scrollback]`
...
`oexe wait --until free` blocks on.
...
### `oexe wait [%PANE] [--until STATE,…] [--changed] [--timeout SECS] [--interval MS]`
Blocks until the pane reaches one of the named states. The orchestration
primitive: `oexe wait %3 && oexe capture %3 --plain`.
...
- **`free` reads the process tree**, so a pane whose root process is the command
  itself (a `oexe run` pane) looks free while it runs, and a backgrounded job
...
reports no status, so `oexe agents` shows it frozen and `oexe wait` only ever
...
one by name, by full id, or by a unique id prefix (the 8-char prefix `oexe ls`
...
interrupted `oexe run`: that pane keeps running with nothing referencing it.
`pane ls --all` finds those.

Prefer `oexe new <path>` over `ws new` when you want something usable: `ws new`
leaves you with an empty workspace you then have to populate, while
`oexe new --json <path>` hands back `{"id","pane"}` — both addresses in one go.
...
free-floating `oexe run` before `--keep` files it), and `orphan: true` means
no workspace holds it. An interrupted `oexe run` is what leaves them.
...
| Command | Effect |
|---|---|
| `server status` | same as `oexe status` |
| `server logs` | tail the server log; prints the path, and says so when logging was never enabled (`OSIRIS_LOG=info` before the server starts) |
