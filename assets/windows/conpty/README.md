...
Microsoft's redistributable pseudoconsole, shipped beside `osiris.exe` so that
...
query: the sequence never reaches osiris's emulator and no reply is ever written
...
back to the client, which is all osiris needs — `terminal::view` already answers
...
(`src/win/psuedocon.rs`), so nothing in osiris loads these files explicitly. They
are found through the ordinary DLL search path, which starts at the directory of
the running executable — which is why they sit beside `osiris.exe` (the daemon
is `osiris.exe --daemon`) rather than in a subdirectory, and why `build.rs`
