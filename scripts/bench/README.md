...
Reproducible throughput/FPS/memory benchmarks for osiris against Alacritty,
...
| **Memory** | cold launch, default shell, idle 6 s | RSS in KB (osiris = GUI + daemon) |
...
cargo build --release                   # the osiris under test

scripts/bench/run_one.sh osiris io        # ALWAYS first: records the reference grid
scripts/bench/run_one.sh alacritty io   # matches osiris's grid automatically
scripts/bench/run_one.sh ghostty io
scripts/bench/run_one.sh kitty io
scripts/bench/run_one.sh osiris fire      # same order for the fire test
...
`$OSIRIS_BENCH_DIR`, the osiris binary with `$OSIRIS_BIN`).
...
- **osiris**: an isolated `--config-dir` under the work dir whose `config.json`
  sets `shell` to the script. GUI + daemon are launched fresh and killed
  (by config-dir-scoped `pkill`) after the run — a daily-driver osiris and its
...
Grid fairness: osiris has no size flag, so its default window is the reference —
run osiris first, and the driver reads the recorded `grid:` line to size the
...
| Test | osiris (before) | osiris (after) | Alacritty | Ghostty | Kitty |
| --- | ---: | ---: | ---: | ---: | ---: |
...
"run 1 fast, later runs slow" (the ring fills during run 1). `OSIRIS_TRACE=1`
...
¹ The osiris-before numbers were measured while the machine was busy (builds +
...
- **The day's fps band matters more than the run.** The same pre-pass binary
  ...
  reproduced within ±3%. osiris's fire number is drain-rate-bound and therefore
  sensitive to ambient machine load in a way the (slower) competitors aren't.
  Only compare osiris-vs-osiris fire numbers from the same session.
- **DOOM-fire is producer-bound, not terminal-bound, at this level.** Under a
  ...
  60 MB/s → 690 fps). osiris's steady seconds already drain at 93–98 MB/s; the
