// Fig autocomplete spec -> osiris signature JSON converter.
//
...
//
// The output schema mirrors osiris's `terminal::completion::spec` serde model
// one-to-one. Runtime JS (postProcess) is intentionally lost: osiris runs a
...
// last. `postProcess` (how to parse output) is a fn we always drop; osiris
...
function normArg(a) {
  ...
  return {
    ...
    template: strList(a.template), // "filepaths" | "folders" -> osiris path completion
