fn main() {
    #[cfg(windows)]
    {
        ...
        // What Explorer's Properties dialog, the taskbar and SmartScreen read.
        // winresource would otherwise derive these from the crate name — the
        // lowercase identifier (`osiris`), not the product's name. Upstream
        // credit lives in the copyright string: this is a fork of tty7
        // (Apache-2.0), and the binary should say so where Windows looks.
        res.set("ProductName", "OSIRIS.EXE");
        res.set("FileDescription", "OSIRIS.EXE — the archive terminal");
        res.set("CompanyName", "DSB Labs · Disk Darián");
        res.set(
            "LegalCopyright",
            "Apache-2.0 · a fork of tty7 (github.com/l0ng-ai/tty7)",
        );
        res.set("InternalName", "osiris.exe");
        res.set("OriginalFilename", "osiris.exe");
...
fn stage_bundled_conpty() {
    ...
    for name in ["conpty.dll", "OpenConsole.exe"] {
        ...
        for directory in [target_dir.clone(), target_dir.join("deps")] {
            ...
            // Re-copying would fail while a previously built osiris is running,
