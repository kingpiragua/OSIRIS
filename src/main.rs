...
fn spawn_config_watcher(cx: &mut App) {
    ...
    let handler = move |res: notify::Result<notify::Event>| {
        ...
        // event — see `osiris_core::host::is_content_change`.
        if !osiris_core::host::is_content_change(&event.kind) {
            return;
        }
        let hit = event
            .paths
...
fn forward_open_path_with(
    ...
    dispatch: impl FnOnce(Option<String>) -> std::io::Result<osiris_core::daemon::control::ReplyOk>,
) -> bool {
    use osiris_core::daemon::control::ReplyOk;
...
fn forward_open_path(open_path: Option<&std::path::Path>) -> bool {
    use osiris_core::client::ControlClient;
    use osiris_core::daemon::control::{ControlHello, ControlRequest};
    ...
    forward_open_path_with(
        ...
        |path| {
            let hello = ControlHello::host_rpc(
                format!("osiris-open-{}", std::process::id()),
                "this computer",
            );
...
fn main() {
        ...
        .any(|arg| arg == std::ffi::OsStr::new("--stop-daemon"))
    {
        ...
        if let Some(dir) = update_install_dir_from(args.iter().cloned()) {
            ...
            osiris_core::daemon::update_guard::hold_for_parent();
    ...
    // Give desktop toasts the osiris icon and name instead of notify-rust's
...
mod config_reload_tests {
    ...
    fn running_on(cx: &mut App, config: Config) {
        let dir = std::env::temp_dir().join(format!("osiris-reload-{}", std::process::id()));
