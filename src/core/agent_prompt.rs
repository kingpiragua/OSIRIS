...
fn capped(text: &str) -> String {
    ...
    format!(
        "{}\n[… truncated by osiris — ask for the rest if needed]",
        &text[..end]
    )
}
...
mod tests {
    ...
    fn selection_prompt_embeds_the_text_and_cwd() {
        let p = build_selection_prompt("error[E0308]: mismatched types", Some("/work/osiris"))
            .expect("non-empty selection builds");
        assert!(p.contains("error[E0308]"));
        assert!(p.contains("/work/osiris"));
        assert!(p.contains("```"));
    ...
    fn oversized_context_is_truncated_with_a_note() {
        ...
        assert!(p.contains("truncated by osiris"));
    }
