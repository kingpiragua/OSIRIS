//! Putting the bundled `osiris` CLI on PATH, without asking and without an entry
//! point to click.
//!
//! The GUI and the CLI ship as one artifact but are two binaries: the installer
//! lays `osiris` down beside `osiris` (inside `Contents/MacOS/` on macOS, in the
...
//! system resists all of it still has a working GUI, just no `osiris` on PATH.
//!
...
//! it (a dangling link still names `osiris`, so the next launch repoints it); a
...
pub enum Outcome {
    ...
    DevBuild,
    /// Already reachable as `osiris`, pointing at this install.
...
#[cfg(windows)]
const CLI_NAME: &str = "oexe.exe";
#[cfg(not(windows))]
const CLI_NAME: &str = "oexe";
...
pub fn install(enabled: bool) -> Outcome {
    ...
    match &outcome {
        ...
        Outcome::AlreadyInstalled(p) => log::debug!("osiris CLI already on PATH at {}", p.display()),
        Outcome::Installed(p) => log::info!("put the osiris CLI on PATH at {}", p.display()),
        Outcome::InstalledOffPath(p) => log::warn!(
            "installed the osiris CLI at {}, which is not on your PATH — add it to use `oexe` \
             outside an OSIRIS pane",
            p.display()
        ),
        Outcome::InstalledShadowed { ours, winner } => log::warn!(
            "installed the osiris CLI at {}, but `oexe` outside an OSIRIS pane still resolves to {} — \
             remove that one, or reorder your PATH, to reach the bundled CLI",
            ours.display(),
            winner.display()
        ),
        Outcome::Occupied(p) => log::info!(
            "leaving the existing `oexe` at {} alone; the bundled CLI was not installed",
            p.display()
        ),
        Outcome::Failed(e) => log::warn!("could not put the osiris CLI on PATH: {e}"),
    }
    outcome
}
...
fn install_inner(enabled: bool) -> Outcome {
    ...
    // holds a `osiris` too, so without this a `cargo run` would point the user's
    // real `osiris` at a binary the next `cargo clean` deletes — and the isolated
    ...
    // rather than placing a file, so it never collides with another `osiris` and
...
/// The first `osiris` the user's shell would find, if any.
///
...
/// `cargo run --release`, which would otherwise aim the developer's real `osiris`
...
/// an older install — inside an osiris pane, `osiris` should mean the osiris you are
/// sitting in.
///
...
fn platform_install(cli: &Path, user_path: &[PathBuf]) -> Outcome {
    ...
    for dir in &candidates {
        match place(dir, cli, mode) {
            ...
            // Someone else's `osiris` lives here. Keep going rather than giving
...
fn copy_marker(dir: &Path) -> PathBuf {
    dir.join(format!(".{CLI_NAME}.installed-by-osiris"))
}
...
fn place(dir: &Path, cli: &Path, mode: Mode) -> std::io::Result<Placement> {
    ...
    match std::fs::symlink_metadata(&target) {
        Ok(meta) if meta.file_type().is_symlink() => {
            ...
            // `osiris`. Anything else under this name was pointed somewhere
...
/// unlink-then-create leaves a window in which `osiris` resolves to nothing. The
...
fn write_atomically(dir: &Path, target: &Path, cli: &Path, mode: Mode) -> std::io::Result<()> {
    // The temp name carries the pid so two osiris instances starting together
    ...
    match mode {
        Mode::Copy => {
            let _ = std::fs::write(
                copy_marker(dir),
                format!(
                    "{} was installed by the osiris app, which replaces it on upgrade.\n\
                     Delete this marker to have osiris treat that binary as yours and leave \
                     it alone.\n",
                    target.display()
                ),
            );
...
mod tests {
    ...
    fn a_build_tree_binary_is_recognised_under_both_profile_layouts() {
        for p in [
            "/home/dev/osiris/target/debug/osiris",
            "/home/dev/osiris/target/release/osiris",
            "/home/dev/osiris/target/aarch64-apple-darwin/release/osiris",
        ] {
    ...
    fn a_directory_already_on_path_is_not_prepended_twice() {
        let current = std::env::join_paths(["/usr/bin", "/opt/osiris", "/bin"]).unwrap();
        assert!(path_with_dir_first(&current, Path::new("/opt/osiris")).is_none());
    ...
    fn the_user_path_gains_the_directory_once_and_losslessly() {
        ...
        let updated = user_path_with_dir(&existing, &wide("C:\\osiris")).expect("a new entry");
        assert_eq!(
            &updated[..existing.len()],
            &existing[..],
            "mangled the tail"
        );
        assert_eq!(&updated[existing.len()..], &wide(";C:\\osiris")[..]);

        // Idempotent, and insensitive to case and to a trailing separator.
        assert!(user_path_with_dir(&updated, &wide("C:\\osiris")).is_none());
        assert!(user_path_with_dir(&updated, &wide("c:\\OSIRIS")).is_none());
        assert!(user_path_with_dir(&updated, &wide("C:\\osiris\\")).is_none());
    }
    ...
    fn a_trailing_separator_does_not_become_an_empty_path_entry() {
        let updated = user_path_with_dir(&wide("C:\\bin;;"), &wide("C:\\osiris")).unwrap();
        assert_eq!(from_wide(&updated), "C:\\bin;C:\\osiris");

        let fresh = user_path_with_dir(&[], &wide("C:\\osiris")).unwrap();
        assert_eq!(from_wide(&fresh), "C:\\osiris");
    }
}
...
mod unix_tests {
    ...
    fn tmpdir(tag: &str) -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("osiris-cli-install-{tag}-{}", std::process::id()));
