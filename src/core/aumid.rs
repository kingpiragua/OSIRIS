...
//! touches at most the single per-user `osiris.lnk` that Inno's default install
//! owns anyway — never a second Start Menu entry beside an all-users install
//! (which would show "osiris" twice and outlive the uninstaller), and never
...
pub(crate) const AUMID: &str = "com.diskdarian.osiris";
...
fn decide() -> Result<Decision, String> {
    ...
    // An elevated install owns `%ProgramData%\...\osiris.lnk`, which we cannot
    // rewrite unelevated. A per-user twin beside it would list "osiris" twice in
...
/// `target[\<triple>]\{debug,release}\osiris.exe`. `CACHEDIR.TAG` is the
...
mod tests {
    ...
    fn start_menu_shortcut_lives_under_programs() {
        ...
        assert_eq!(lnk.file_name().and_then(|n| n.to_str()), Some("osiris.lnk"));
        assert_eq!(
            lnk.parent()
    ...
    fn cargo_layouts_are_recognized_as_build_output() {
        for exe in [
            r"C:\src\osiris\target\debug\osiris.exe",
            r"C:\src\osiris\target\release\osiris.exe",
            r"C:\src\osiris\target\x86_64-pc-windows-msvc\release\osiris.exe",
            r"C:\src\osiris\TARGET\Debug\osiris.exe",
        ] {
            assert!(
    ...
    fn install_layouts_are_not_build_output() {
        for exe in [
            r"C:\Program Files\osiris\osiris.exe",
            r"C:\Users\me\AppData\Local\Programs\osiris\osiris.exe",
            // Portable zip, extracted anywhere the user likes.
            r"D:\tools\osiris\osiris.exe",
            // A "release" *install* directory with no `target` above it.
            r"D:\osiris\release\osiris.exe",
        ] {
    ...
    fn same_path_ignores_case_for_paths_that_do_not_exist() {
        assert!(super::same_path(
            Path::new(r"C:\Program Files\osiris\OSIRIS-APP.EXE"),
            Path::new(r"c:\program files\osiris\osiris.exe"),
        ));
        assert!(!super::same_path(
            Path::new(r"C:\Program Files\osiris\osiris.exe"),
            Path::new(r"C:\src\osiris\target\debug\osiris.exe"),
        ));
    }
    ...
    fn a_written_shortcut_reads_back_as_ours() {
        let dir = std::env::temp_dir().join(format!("osiris-aumid-{}", std::process::id()));
        std::fs::create_dir_all(&dir).expect("create temp dir");
        let lnk = dir.join("osiris.lnk");
    ...
    /// what `decide()` chose and pops a real toast, which should carry the osiris
    ...
    fn sends_a_branded_toast() {
        ...
        crate::terminal::notify_desktop(Some("osiris"), "AUMID toast test");
