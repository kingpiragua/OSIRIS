...
impl WorkspaceStore {
    ...
    pub fn focus(cx: &mut gpui::App, id: WorkspaceId) {
        ...
        crate::ui::tree_sync::fire_workspace_op(cx, id, |ws| {
            osiris_core::daemon::control::ControlRequest::WorkspaceTouch { workspace: ws }
        });
    }
...
mod tests {
    ...
    fn restore_one_keeps_one_window_and_reports_the_rest(cx: &mut gpui::TestAppContext) {
        ...
        let _ = osiris_core::core::config::set_config_dir(
            std::env::temp_dir().join(format!("osiris-session-test-{}", std::process::id())),
        );
        cx.update(|cx| {
    ...
    fn claim_remote_remembers_the_route_and_refreshes_it_while_the_profile_lives(
        ...
        cx.update(|cx| {
            ...
            let mut profile = osiris_core::core::ssh_profile::SshProfile::new("lager");
