...
//!
//! The registry is user-visible system state, so osiris never writes these verbs
...
//!
//! Both verbs invoke the GUI-subsystem `osiris.exe` directly so Explorer never
...
const DIRECTORY_KEY: &str = r"Software\Classes\Directory\shell\osiris";
const BACKGROUND_KEY: &str = r"Software\Classes\Directory\Background\shell\osiris";
...
impl Location {
    ...
    fn label(self) -> &'static str {
        match self {
            Self::Directory => "Open in osiris",
            Self::Background => "Open osiris here",
        }
    }
...
fn replace_entry_with(
    ...
    // The verb root belongs exclusively to osiris. Replacing it at the operation
...
/// Remove only the two Explorer verb trees owned by osiris.
...
fn application_path() -> Result<PathBuf> {
    let app = std::env::current_exe().context("locating the running osiris application")?;
    app.parent()
        .context("the running osiris application has no parent directory")?;
    Ok(app)
}
...
mod windows {
    ...
    fn create_key(path: &str) -> Result<RegistryKey> {
        ...
        if code != ERROR_SUCCESS {
            return Err(io_error("creating the osiris Explorer registry key", code));
        }
        Ok(RegistryKey(key))
    }
    ...
    fn set_string(key: &RegistryKey, name: Option<&OsStr>, value: &OsStr) -> Result<()> {
        ...
        let byte_len = u32::try_from(value.len() * 2)
            .context("the osiris Explorer registry value is too long")?;
        ...
        if code == ERROR_SUCCESS {
            ...
            Err(io_error("writing an osiris Explorer registry value", code))
        }
    }
    ...
    fn delete_tree(path: &str) -> Result<()> {
        let path = wide(OsStr::new(path));
        // SAFETY: `path` is a live, NUL-terminated UTF-16 string. Only osiris's
        ...
        match code {
            ...
            other => Err(io_error("removing the osiris Explorer registry key", other)),
        }
    }
...
mod tests {
    ...
    fn registration_targets_both_directory_surfaces() {
        let app = Path::new(r"C:\Program Files\osiris\osiris.exe");
        let [directory, background] = registrations(app);

        assert_eq!(directory.location.key(), DIRECTORY_KEY);
        assert_eq!(directory.location.label(), "Open in osiris");
        assert_eq!(directory.icon, app.as_os_str());
        assert_eq!(
            directory.command,
            r#""C:\Program Files\osiris\osiris.exe" --open-path "%1""#
        );

        assert_eq!(background.location.key(), BACKGROUND_KEY);
        assert_eq!(background.location.label(), "Open osiris here");
        assert_eq!(background.icon, app.as_os_str());
        assert_eq!(
            background.command,
            r#""C:\Program Files\osiris\osiris.exe" --open-path "%V""#
        );
    }
    ...
    fn commands_quote_even_paths_without_spaces() {
        assert_eq!(
            quoted_open_command(Path::new(r"C:\osiris\osiris.exe"), "%1"),
            r#""C:\osiris\osiris.exe" --open-path "%1""#
        );
    }
    ...
    fn registration_replaces_the_owned_tree_before_writing() {
        let registration = Registration::new(
            Location::Directory,
            Path::new(r"C:\Program Files\osiris\osiris.exe"),
        );
