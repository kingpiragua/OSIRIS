...
use osiris_core::daemon::install::AssetFetcher as _;

use crate::core::config::{Config, UpdateChannel};
use crate::ui::i18n::{L10nKey, t, t_fmt};

const REPO: &str = "kingpiragua/osiris";
...
pub const RELEASES_URL: &str = "https://github.com/kingpiragua/osiris/releases/latest";
...
pub const NIGHTLY_RELEASE_URL: &str = "https://github.com/kingpiragua/osiris/releases/tag/nightly";
...
const WINDOWS_INNO_INSTALL_MARKER: &str = ".osiris-inno-install";
#[cfg(target_os = "windows")]
const WINDOWS_PORTABLE_MARKER: &str = ".osiris-portable";
#[cfg(target_os = "windows")]
...
#[cfg(target_os = "windows")]
const WINDOWS_PORTABLE_BACKUP_PREFIX: &str = ".osiris-update-backup-";
...
/// src/bin/osiris-updater.rs, which this duplicates like the markers above.
#[cfg(target_os = "windows")]
const WINDOWS_PORTABLE_BACKUP_INCOMPLETE: &str = ".osiris-replace-incomplete";
...
impl UpdateInstallHint {
    ...
    fn english(&self) -> String {
        match self {
            #[cfg(target_os = "macos")]
            Self::UnsupportedMacos => "This copy is not running from a writable osiris.app bundle, so replacing it would be unsafe. Move osiris to Applications or another writable folder, or open the release page to install the update.".to_string(),
            #[cfg(target_os = "linux")]
            ...
            #[cfg(target_os = "windows")]
            Self::WindowsAllUsersInstall => "osiris is installed for all users, which needs administrator rights to replace. osiris will not raise an elevation prompt on its own behalf, so open the release page and run the installer yourself to update it.".to_string(),
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            ...
            Self::MissingChecksums => "The release has no checksums.txt, so osiris refuses to install it automatically.".to_string(),
        }
    }
}
...
/// waits for a download and never answers a second question — they quit osiris
...
pub fn absorb_update_outcome_at_launch() {
    use osiris_core::daemon::install::outcome::{UpdateOutcome, read_outcome};
...
fn is_stage_name(name: &str) -> bool {
    name.starts_with(".osiris-update-") || name.starts_with("osiris-update-")
}

/// Deals with `.osiris-update-backup-*` directories a previous portable update
...
impl PendingUpdate {
    ...
    fn launch_elevated(&self) -> Result<ElevationStart> {
        ...
        let installed_updater =
            bundled_updater().context("osiris-updater.exe is not installed beside this app")?;
        ...
        let mut watcher = osiris_core::core::proc::hide_console(&mut watcher)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
...
/// (`--config-dir "D:\osiris\"`) escapes its own closing quote and swallows the
...
/// `osiris-26.8.2-nightly.202608071800-macos-arm64.zip`.
///
/// The platform segment is a closed set, which is what makes the split
/// unambiguous — the version is everything between the `osiris-` prefix and the
/// platform marker. `osiris-server-linux-x86_64-musl` matches that shape too but
...
fn version_from_assets(assets: &[GitHubAsset]) -> Option<String> {
    assets
        .iter()
        .filter_map(|asset| {
            let rest = asset.name.strip_prefix("osiris-")?;
...
fn build_http_client(manual_proxy: Option<&str>) -> Result<ReqwestClient> {
    let user_agent = concat!("osiris/", env!("CARGO_PKG_VERSION"));
    ...
    if let Some(proxy) = manual_proxy
        .and_then(osiris_core::daemon::install::proxy::normalize_manual)
...
fn package_for_current_install(version: &str) -> Result<PackageOffer, UpdateInstallHint> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if !layout.directory().join("osiris-updater.exe").is_file() {
            return Err(UpdateInstallHint::UnsupportedWindows);
        }
...
fn updater_speaks_elevation(install_dir: &Path) -> bool {
    let updater = install_dir.join("osiris-updater.exe");
    let mut command = Command::new(&updater);
    command
        .arg("capabilities")
        .stdin(Stdio::null())
        .stderr(Stdio::null());
    let output = osiris_core::core::proc::hide_console(&mut command).output();
...
fn prepare_update(
    ...
    let cfg = Config::load();
    let fetcher =
        osiris_core::daemon::install::download::HttpsFetcher::new(cfg.http_proxy.as_deref());
...
impl PreparedUpdate {
    ...
    fn launch(&self) -> Result<()> {
        ...
        osiris_core::core::proc::hide_console(&mut command)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            ...
            .context("launching osiris-updater")?;
        Ok(())
    }
}
...
fn update_outcome_path() -> Option<PathBuf> {
    crate::core::config::config_path(osiris_core::daemon::install::outcome::OUTCOME_FILE_NAME)
}
...
/// spawning process's environment, so a `OSIRIS_CONFIG_DIR` set there would
...
fn prepare_macos_update(
    ...
    let current =
        current_macos_app_bundle().context("osiris is not running from an application bundle")?;
    let parent = current
        .parent()
        .context("osiris.app has no parent directory")?;
    let updater = bundled_updater().context("osiris-updater is not bundled with this app")?;
...
fn prepare_linux_update(
    ...
    let current = current_appimage().context("osiris is not running from an AppImage")?;
    let parent = current
        .parent()
        .context("the AppImage has no parent directory")?;
    let bundled = bundled_updater().context("osiris-updater is not bundled with this image")?;
    let staging = update_staging_dir(parent)?;
    let dir = staging.path().to_path_buf();
    let archive = write_staged_asset(&dir, asset_name, archive)?;
    let checksums = write_staged_asset(&dir, "checksums.txt", checksums)?;
    ...
    let updater = dir.join("osiris-updater");
...
fn prepare_windows_update(
    ...
    let layout = current_windows_update_layout()
        .context("osiris is not running from a recognized Windows installation")?;
    ...
    let bundled = bundled_updater().context("osiris-updater.exe is not bundled with this app")?;
    ...
    let expected_sha256 = if matches!(layout, WindowsUpdateLayout::Inno(_)) {
        ...
        Some(
            osiris_core::daemon::install::checksums::expected_digest(manifest, asset_name)
                .map(|digest| osiris_core::daemon::install::checksums::hex(&digest))
    ...
    let updater = dir.join("osiris-updater.exe");
    std::fs::copy(&bundled, &updater)
...
fn bundled_updater() -> Option<PathBuf> {
    let updater = current_macos_app_bundle()?.join("Contents/MacOS/osiris-updater");
...
#[cfg(target_os = "linux")]
fn bundled_updater() -> Option<PathBuf> {
    let updater = std::env::current_exe().ok()?.parent()?.join("osiris-updater");
    updater.is_file().then_some(updater)
}
...
fn linux_package_for(
    ...
    if !is_appimage {
        return Err(UpdateInstallHint::LinuxManualPackage(format!(
            "osiris-{version}-linux-{arch}.tar.gz"
        )));
    }
    let name = format!("osiris-{version}-linux-{arch}.AppImage");
    if can_self_update {
...
fn windows_package_for_layout(version: &str, layout: &WindowsUpdateLayout) -> Option<String> {
    ...
    Some(match layout {
        WindowsUpdateLayout::Inno(_) => format!("osiris-{version}-windows-{arch}-setup.exe"),
        WindowsUpdateLayout::Portable(_) => format!("osiris-{version}-windows-{arch}.zip"),
    })
}
...
fn windows_directory_is_writable(directory: &Path) -> bool {
    tempfile::Builder::new()
        .prefix(".osiris-update-write-test-")
...
///     Neither outcome is one osiris should produce on its own initiative.
...
/// `C:\Program Files\osiris\` a different place from `C:\Program Files\osiris`.
...
/// The `{app}` directory of an all-users osiris installation, read from the
...
fn windows_all_users_install_path() -> Option<PathBuf> {
    ...
    // out-parameter, wrapped only when the call reports success. The osiris
...
fn run_updater(updater: &Path, args: impl IntoIterator<Item = PathBuf>) -> Result<()> {
    let mut command = Command::new(updater);
    command.args(args);
    let output = osiris_core::core::proc::hide_console(&mut command)
        .output()
        .context("running osiris-updater verification")?;
    if !output.status.success() {
        anyhow::bail!(
            "osiris-updater verification failed: {}",
...
mod tests {
    ...
    fn release_asset_requires_the_platform_package_and_checksums() {
        let name = "osiris-27.1.0-macos-arm64.zip";
    ...
    fn release_without_the_exact_platform_package_is_never_guessed() {
        let selected = select_release_asset_for(
            Ok(PackageOffer::plain(
                "osiris-27.1.0-macos-arm64.zip".to_string(),
            )),
            &[
                github_asset("osiris-27.1.0-macos-x86_64.zip"),
    ...
    fn an_appimage_that_can_replace_itself_is_offered_the_appimage() {
        ...
        assert_eq!(offer.name, "osiris-27.1.0-linux-x86_64.AppImage");
    }
    ...
    fn linux_installs_that_cannot_self_update_name_their_package() {
        ...
        assert_eq!(
            ...
            UpdateInstallHint::LinuxManualPackage("osiris-27.1.0-linux-x86_64.AppImage".to_string())
        );
        // A tarball (or distro-packaged) install is never guessed at.
        assert_eq!(
            linux_package_for("27.1.0", "x86_64", false, false).unwrap_err(),
            UpdateInstallHint::LinuxManualPackage("osiris-27.1.0-linux-x86_64.tar.gz".to_string())
        );
    ...
    fn appimage_selection_matches_the_published_asset_names() {
        let name = "osiris-27.1.0-linux-x86_64.AppImage";
        let offer = linux_package_for("27.1.0", "x86_64", true, true).unwrap();
        let assets = [
            github_asset("osiris-27.1.0-linux-x86_64.tar.gz"),
            github_asset(name),
            github_asset("checksums.txt"),
        ];
    ...
    fn portable_backup_scan_separates_interrupted_from_finished() {
        ...
        std::fs::write(root.path().join(".osiris-update-backup-not-a-dir"), b"file").unwrap();
    ...
    fn version_is_recovered_from_nightly_asset_names() {
        let assets = [
            github_asset("checksums.txt"),
            github_asset("osiris-26.8.2-nightly.20260807-macos-arm64.zip"),
            github_asset("osiris-26.8.2-nightly.20260807-windows-x86_64-setup.exe"),
        ];
        assert_eq!(
    ...
    /// `osiris-…-linux-…` shape, but have no version in front of the platform.
    ...
    fn remote_server_assets_are_not_mistaken_for_a_version() {
        let assets = [
            github_asset("checksums.txt"),
            github_asset("osiris-server-linux-x86_64-musl"),
            github_asset("osiris-server-linux-aarch64-musl"),
        ];
        ...
        let mixed = [
            github_asset("osiris-server-linux-x86_64-musl"),
            github_asset("osiris-26.8.2-nightly.202608071800-linux-x86_64.tar.gz"),
        ];
        assert_eq!(
    ...
    fn the_newest_asset_version_wins_when_two_nights_overlap() {
        let assets = [
            github_asset("osiris-26.8.2-nightly.202608062200-macos-arm64.zip"),
            github_asset("osiris-26.8.2-nightly.202608062200-linux-x86_64.tar.gz"),
            github_asset("checksums.txt"),
            github_asset("osiris-26.8.2-nightly.202608071800-macos-arm64.zip"),
            github_asset("osiris-26.8.2-nightly.202608071800-linux-x86_64.tar.gz"),
        ];
        assert_eq!(
            version_from_assets(&assets).as_deref(),
    ...
    fn update_state_round_trips_and_defaults() {
        ...
        UpdateState {
            pending: Some(PendingUpdate {
                ...
                plan_version: PLAN_VERSION,
                updater: PathBuf::from("/tmp/osiris-updater"),
                command: "install".into(),
                rest: vec![PathBuf::from("/tmp/stage/osiris.zip")],
                config_dir: None,
    ...
    fn a_failure_lets_the_version_prompt_again() {
        ...
        state.last_failure = Some(FailureRecord {
            version: "27.0.0".into(),
            detail: "downloading osiris-27.0.0-macos-arm64.zip: timed out".into(),
        });
    ...
    fn an_updater_outcome_is_absorbed_exactly_once() {
        use osiris_core::daemon::install::outcome::{UpdateOutcome, write_outcome};
    ...
    fn only_our_own_staging_directories_are_swept() {
        assert!(is_stage_name(".osiris-update-abc123"));
        assert!(is_stage_name("osiris-update-abc123"));
        assert!(!is_stage_name("osiris.app"));
        assert!(!is_stage_name(".Trash"));
        assert!(!is_stage_name("osiris-update"));
    }
    ...
    fn a_staged_plan_supplies_a_fresh_parent_pid() {
        ...
        assert_eq!(args[2], PathBuf::from("/tmp/stage/osiris.zip"));
    }
    ...
    fn windows_markers_distinguish_inno_portable_and_unknown_layouts() {
        let root = tempfile::tempdir().unwrap();
        let executable = root.path().join("osiris.exe");
    ...
    fn an_all_users_inno_install_is_never_updated_in_place() {
        let all_users = PathBuf::from(r"C:\Program Files\osiris");
        let per_user = PathBuf::from(r"C:\Users\someone\AppData\Local\Programs\osiris");

        // The machine-hive entry names this directory: elevation would be
        // required, so osiris declines however writable the directory looks.
        ...
        assert!(windows_inno_needs_elevation_for(
            Some(Path::new(r"C:\Program Files\osiris\")),
            &all_users,
            true
        ));
        assert!(windows_inno_needs_elevation_for(
    ...
    fn a_plan_from_another_protocol_version_is_not_usable() {
        let root = tempfile::tempdir().unwrap();
        let updater = root.path().join("osiris-updater");
        ...
        let plan = |plan_version| PendingUpdate {
            ...
            command: "install".into(),
            rest: vec![stage.join("osiris.zip")],
            config_dir: None,
            stage: stage.clone(),
            needs_elevation: false,
            expected_sha256: None,
    ...
    fn an_elevation_capable_updater_is_recognised_by_its_own_answer() {
        ...
        assert!(!capabilities_cover_elevation(
            b"usage: osiris-updater <command> [args]"
        ));
    ...
    fn an_elevated_plan_names_every_piece_the_chain_needs() {
        ...
        let stage = PathBuf::from(r"C:\Users\someone\AppData\Local\Temp\osiris-update-x");
        let plan = |needs_elevation, expected_sha256: Option<&str>| PendingUpdate {
            version: "27.0.0".into(),
            apply_on_launch: false,
            plan_version: PLAN_VERSION,
            updater: stage.join("osiris-updater.exe"),
            command: "install".into(),
            // The order `prepare_windows_update` pushes: installer, checksums,
            // asset name, install dir, version, log, stage.
            rest: vec![
                stage.join("osiris-27.0.0-windows-x86_64-setup.exe"),
                stage.join("checksums.txt"),
                PathBuf::from("osiris-27.0.0-windows-x86_64-setup.exe"),
                PathBuf::from(r"C:\Program Files\osiris"),
                PathBuf::from("27.0.0"),
                stage.join("update.log"),
                stage.clone(),
            ],
            config_dir: Some(PathBuf::from(r"C:\Users\someone\.config\osiris")),
            stage: stage.clone(),
            needs_elevation,
        ...
        assert_eq!(parts.asset_name, "osiris-27.0.0-windows-x86_64-setup.exe");
        assert_eq!(parts.install_dir, PathBuf::from(r"C:\Program Files\osiris"));
    ...
    fn the_elevated_command_line_quotes_every_argument() {
        let parts = ElevatedPlanParts {
            ...
            asset_name: "osiris-27.0.0-windows-x86_64-setup.exe".into(),
            install_dir: PathBuf::from(r"C:\Program Files\osiris"),
            version: "27.0.0".into(),
        ...
        let line = elevated_stage_arguments(
            &parts,
            Path::new(r"C:\Users\some one\.config\osiris\update-elevation.status"),
            Path::new(r"C:\Users\some one\.config\osiris\update-outcome.json"),
            Some(Path::new(r"C:\Users\some one\.config\osiris")),
        )
        .into_string()
        ...
        assert!(line.contains(r#""C:\Program Files\osiris""#), "{line}");
        assert!(
        ...
        assert!(
            line.contains(
                r#""--status-file" "C:\Users\some one\.config\osiris\update-elevation.status""#
            ),
            "{line}"
        );
        assert!(
            line.contains(r#""--config-dir" "C:\Users\some one\.config\osiris""#),
            "{line}"
        );
        assert!(
            line.contains(
                r#""--result-file" "C:\Users\some one\.config\osiris\update-outcome.json""#
            ),
            "{line}"
        );
    }
    ...
    fn a_trailing_backslash_does_not_escape_its_own_closing_quote() {
        ...
        assert_eq!(
            quote(r"C:\Program Files\osiris"),
            r#""C:\Program Files\osiris""#
        );
        // Doubled only in front of the closing quote.
        assert_eq!(quote(r"D:\osiris\"), r#""D:\osiris\\""#);
        assert_eq!(quote(r"D:\osiris\\"), r#""D:\osiris\\\\""#);
