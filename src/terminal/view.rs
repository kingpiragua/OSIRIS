...
pub(crate) const DEFAULT_TITLE: &str = "osiris";
...
pub struct TerminalView {
    ...
    /// session ends. "osiris" for a local shell; for an SSH pane it is the host
...
fn write_clipboard_image(img: &gpui::Image) -> Option<std::path::PathBuf> {
    ...
    let dir = std::env::temp_dir().join("osiris-clipboard");
...
/// and needs no such check: osiris reaches those distros by running `wsl.exe`
...
const REMOTE_CLIPBOARD_PATH: [&str; 3] = [".cache", "osiris", "clipboard"];
...
/// owner change its mode, so a directory osiris can chmod and then observe at
...
impl TerminalView {
    ...
    /// reports are on a third machine osiris has no link to, and neither this
    ...
    pub fn busy(&self) -> Option<PaneBusy> {
        ...
        // "osiris is still running. Closing ends it." about a folder. The
    ...
    fn on_key_down(&mut self, ev: &KeyDownEvent, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // expects osiris's menu. With the prompt editor off, the shell owning
    ...
    fn remote_shell_history_sources(
        ...
        // start from what osiris recorded for the inner target, like bare ssh.
    ...
    /// Moves this pane between osiris's inline editor and the shell's own line
    ...
    fn shell_owns_prompt(&self) -> bool {
        ...
        // exist to feed osiris's editor, and one of which erases the line with
    ...
    fn tab_pressed(&mut self, forward: bool, cx: &mut Context<Self>) {
        ...
        if let Some(reason) = self.link_inactive_reason(cx) {
            log::debug!(target: "osiris::completion", "Tab does nothing and the line stays: {reason}");
            return;
        }
        if let Some(reason) = self.input_inactive_reason() {
            log::debug!(target: "osiris::completion", "Tab goes straight to the PTY: {reason}");
    ...
    fn complete_tab(&mut self, forward: bool, cx: &mut Context<Self>) {
        ...
        if !cx.global::<Config>().tab_completion {
            log::debug!(target: "osiris::completion", "handing the line to the shell: tab_completion is off");
    ...
    fn spawn_remote_path_completion(
        ...
        log::debug!(target: "osiris::completion", "listing {dir} over the remote's own connection");
        cx.spawn(async move |this, cx| {
    ...
    pub fn commit_text(&mut self, text: &str, cx: &mut Context<Self>) {
        ...
        // taken when osiris is *not* driving the line — so scrolling up and
    ...
    /// one place — [`OsirisApp::change_font_size`](crate::ui::app::OsirisApp).
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .context_menu(move |menu, window, cx| {
                ...
                // `fork_label` is osiris-core's capability probe, and core has no
...
/// Whether a right-click belongs to osiris's own context menu rather than to the
...
/// is the escape hatch that reaches osiris, matching how Shift already overrides
...
mod tests {
    ...
    fn a_notification_title_keeps_at_most_two_segments() {
        ...
        assert_eq!(compose_notification_title(None, None, ws()), "osiris");
        assert_eq!(
            compose_notification_title(Some("Claude".into()), None, None),
            "Claude"
        );
        assert_eq!(compose_notification_title(None, None, None), "osiris");
    }
    ...
    fn a_wsl_pane_gets_the_automount_path_not_the_windows_one() {
        ...
        assert_eq!(
            staged_path_for_pane(
                r"C:\Users\me\AppData\Local\Temp\osiris-clipboard\paste-1.png",
                true
            ),
            "/mnt/c/Users/me/AppData/Local/Temp/osiris-clipboard/paste-1.png"
        );
        ...
        assert_eq!(
            staged_path_for_pane("/tmp/osiris-clipboard/paste-1.png", false),
            "/tmp/osiris-clipboard/paste-1.png"
        );
        assert_eq!(
    ...
    fn a_failed_staging_preparation_is_retried_rather_than_latched() {
        assert_eq!(
            staging_cache(&Ok("/home/me/.cache/osiris/clipboard".to_string())),
            Some("/home/me/.cache/osiris/clipboard".to_string())
        );
    ...
    fn staged_images_land_under_the_remote_users_own_home() {
        ...
        assert_eq!(dir, "/home/me/.cache/osiris/clipboard");
        assert!(
            !dir.starts_with("/tmp"),
    ...
    fn a_file_command_that_cannot_spawn_comes_back_as_an_error() {
        ...
        assert!(super::run_file_command("osiris-no-such-binary {path}", path, None, None).is_err());
    ...
    fn right_click_belongs_to_the_app_while_mouse_reporting_is_active() {
        assert!(
            ...
            "an unmodified right-click in mouse mode is the application's event, \
             so osiris must not also pop its own menu over it"
        );
    }
    ...
    fn shift_right_click_stays_the_escape_hatch_in_mouse_mode() {
        assert!(
            ...
            "Shift is the documented way to reach osiris's own menu inside a \
             mouse-reporting TUI"
        );
    }
...
mod gpui_tests {
    ...
    fn placed_at(anchor_row: i64) -> crate::terminal::images::PlacedImage {
        use osiris_core::core::kitty_graphics::{Image, WireFormat};
    ...
    fn a_relative_path_links_against_the_panes_own_directory(cx: &mut TestAppContext) {
        let dir = std::env::temp_dir().join(format!("osiris-view-link-{}", std::process::id()));
    ...
    fn a_pane_whose_paths_are_elsewhere_does_not_link_local_files(cx: &mut TestAppContext) {
        let file = std::env::temp_dir().join(format!("osiris-elsewhere-{}.txt", std::process::id()));
    ...
    fn title_events_drive_the_tab_title(cx: &mut TestAppContext) {
        ...
        settle(cx);
        window
            .update(cx, |view, _, _| assert_eq!(view.title, "osiris"))
            .unwrap();
    }
    ...
    /// SSH tab in the window read "osiris" until — and only if — the far shell
    ...
    fn shell_vi_mode_prompt_bypasses_the_local_editor(cx: &mut TestAppContext) {
        ...
        panic!("an emacs-mode prompt should re-enable osiris's local editor");
    }
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                assert_eq!(
                    view.cmd.text(),
                    "n",
                    "the first character on the fresh line belongs to osiris's editor"
                );
            })
            .unwrap();
        assert_eq!(
    ...
    fn tab_completion_off_sends_every_tab_to_the_shell(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                view.complete_tab(true, cx);
                assert!(view.completion.is_none(), "no osiris menu while opted out");
    ...
    fn insert_newline_action_extends_the_line_and_enter_submits_it(cx: &mut TestAppContext) {
        let dir = std::env::temp_dir().join(format!("osiris-covtest-{}", std::process::id()));
        std::fs::create_dir_all(&dir).ok();
        ...
        window
            .update(cx, |view, _, cx| {
                ...
                assert!(
                    ...
                    "no osiris menu while opted out"
                );
    ...
    fn prompt_editor_off_hands_typing_and_arrows_to_the_shell(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                assert_eq!(view.cmd.text(), "", "nothing was typed into osiris's editor");
                assert!(
                    view.history_nav.is_none(),
                    "Up never touched osiris's own history"
                );
            })
            .unwrap();
    ...
    /// notice that Ctrl-R raises when osiris *wanted* the key and could not have
    /// it.
    #[gpui::test]
    fn prompt_editor_off_hands_over_tab_and_ctrl_r(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                ...
                view.on_key_down(&ctrl_r, window, cx);
                assert!(view.completion.is_none(), "no osiris completion menu");
                assert!(view.reverse_search.is_none(), "no osiris history menu");
                assert!(
                    view.integration_notice.is_none(),
    ...
    fn submitted_command_backfills_its_exit_code(cx: &mut TestAppContext) {
        ...
        let marker = format!("osiris_gpui_exit_marker_{}", std::process::id());
        window
            .update(cx, |view, _, cx| {
                view.cmd.set(&marker);
    ...
    fn a_dropped_link_does_not_claim_the_process_exited(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, _, cx| {
                ...
                assert_eq!(view.title, "osiris — disconnected");

                view.set_workspace(None);
                view.handle_event(AlacEvent::Exit, cx);
                assert_eq!(view.title, "osiris — process exited");
            })
            .unwrap();
    }
    ...
    fn cmd_backspace_reaches_a_foreground_tui_as_ctrl_u(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |view, window, cx| {
                assert!(
                    !view.input_active(),
                    "the foreground process, not osiris's editor, owns input"
                );
                view.on_key_down(
                    &KeyDownEvent {
                        keystroke: gpui::Keystroke {
