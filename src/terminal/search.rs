...
/// Answers for paths on the machine osiris itself is running on.
...
impl LinkRoots {
    /// Roots on the machine osiris is running on, where `$HOME` means what it
    /// says.
...
mod tests {
    ...
    fn url_at_stops_at_unbalanced_close_paren_glued_after_url() {
        let url = "https://github.com/kingpiragua/osiris/pull/43";
    ...
    fn temp_file(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-link-test-{}", std::process::id()));
    ...
    fn tilde_expansion_prefers_home_inferred_from_the_pane_cwd() {
        let cwd = Path::new("/Users/alice/clone/osiris");
        assert_eq!(
            expand_home("~/clone/osiris/src/main.rs", Some(cwd), true),
            Some(PathBuf::from("/Users/alice/clone/osiris/src/main.rs"))
        );
    }
