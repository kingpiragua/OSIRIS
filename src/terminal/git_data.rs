...
pub fn git_watch_dirs(
    ...
    use osiris_core::host::default_join;
...
impl OsirisApp {
    ...
    /// As [`OsirisApp::scm_invalidate`], for a caller that knows a directory
    ...
    /// cannot supply — offer the one thing osiris has that a GUI does not: a
...
fn scm_watch_dirs(host: &dyn Host, root: &Path) -> Option<Vec<PathBuf>> {
    ...
    let heads = osiris_core::host::default_join(
        &osiris_core::host::default_join(&common_dir, "refs", sep),
        "heads",
        sep,
    );
...
mod tests {
    ...
    /// The same pin list `osiris_core::core::git::test_support` keeps for the
    /// four modules over there. It cannot be shared with them: those helpers
    /// are `#[cfg(test)]`, so they do not exist in the `osiris-core` this crate
