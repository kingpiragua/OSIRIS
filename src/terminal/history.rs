...
fn file_stem(label: &str) -> String {
    let mut safe: String = label
        ...
        .collect();
    safe.truncate(48);
    format!("{safe}-{:016x}", osiris_core::host::fnv1a64(label.as_bytes()))
}
...
mod tests {
    ...
    fn append_then_load_recovers_the_command_and_metadata() {
        ...
        let unique = format!("osiris_cov_marker_{}", std::process::id());
        append(
            &Scope::Local,
            &unique,
            Some(Path::new("/tmp")),
    ...
    fn concurrent_appends_never_interleave_records() {
        ...
        let tag = format!("osiris_race_{}", std::process::id());
    ...
    fn a_remote_scope_never_serves_the_local_machine_s_history() {
        ...
        let tag = format!("osiris_scope_{}", std::process::id());
    ...
    fn two_remotes_keep_their_own_stores() {
        ...
        let tag = format!("osiris_twohosts_{}", std::process::id());
    ...
    fn append_rejects_a_cwd_that_would_break_the_line_format() {
        ...
        let unique = format!("osiris_nlcwd_marker_{}", std::process::id());
        append(
            &Scope::Local,
            &unique,
