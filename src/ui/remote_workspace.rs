...
use osiris_core::host::remote::RemoteHost;
...
pub enum RemoteStatus {
    ...
    Failed(String),
    /// The machine answered, and answered as osiris — with a server the other
...
impl RemoteStatus {
    pub fn strip_message(&self, machine: &str) -> Option<String> {
        match self {
            ...
            // The protocol layer's wording reads like the far end is not osiris
...
/// updating osiris here is the first suggestion, replacing the server there the
...
impl OsirisApp {
    ...
    fn restart_remote_server(
        ...
        log::info!("restarting osiris's server on {label} at the user's request");
        ...
        cx.spawn(async move |this, cx| {
            ...
            let _ = this.update_in(cx, |this, window, cx| match outcome {
                ...
                Err(e) => {
                    log::warn!("could not restart osiris's server on {label}: {e}");
    ...
    fn replace_remote_server(
        ...
        let host_id = target.host_id();
        let target_for_error = target.clone();
        log::info!("replacing osiris's server on {label} at the user's request");
        let running = Arc::new(std::sync::atomic::AtomicBool::new(true));
        self.watch_for_restart_consent(host_id, running.clone(), cx);
        cx.spawn(async move |this, cx| {
            let for_task = label.clone();
            ...
            let _ = this.update_in(cx, |this, window, cx| match outcome {
                ...
                Err(e) => {
                    log::warn!("could not replace osiris's server on {label}: {e}");
                    this.report_remote_host_error(Some(&target_for_error), &label, &e, window, cx);
                }
            });
        })
    ...
    fn prompt_remote_daemon_mismatch_later(&self, cx: &mut Context<Self>) {
        cx.spawn(async move |this, cx| {
            let _ = this.update_in(cx, |_, window, cx| {
                OsirisApp::prompt_remote_daemon_mismatch(window, cx);
            });
        })
        .detach();
    }
...
mod tests {
    ...
    fn a_dialect_refusal() -> String {
        "build-box answered, but not as an osiris server: control peer (build 26.7.7-nightly) \
    ...
    fn a_create_refused_for_dialect_runs_once_the_machine_connects(cx: &mut gpui::TestAppContext) {
        ...
        let target = RemoteTarget::Alias {
            alias: "osiris-test-refused-box".into(),
        };
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            app.finish_connect(Ok(fake_connected("osiris-test-refused-box")), window, cx);
        });
    ...
    fn a_server_replacement_connects_back_for_the_parked_create(cx: &mut gpui::TestAppContext) {
        ...
        app.update(cx, |app, cx| {
            app.server_replaced(&target, "refused-box", "osiris-test-unknown-origin", cx);
    ...
    fn a_restart_retires_the_mismatch_note_it_answers(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            let origin = "osiris-test-stale-origin";
