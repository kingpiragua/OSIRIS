...
use gpui::{App, Global};
use osiris_core::core::machine::{LayoutDelta, Machine, PaneRecord, Tab, TabId, Workspace};
use osiris_core::daemon::control::{ControlRequest, ReplyOk};
...
impl MachineMirrors {
    ...
    /// by `oexe new` really has no name, and reads its directory on purpose.
    ...
    match delta {
        ...
        LayoutDelta::RatioChanged { tab, path, ratio } => {
            ...
            match t.root.descend_mut(path) {
                Some(osiris_core::core::machine::PaneNode::Split { ratio: r, .. }) => {
                    *r = *ratio;
                    true
                }
                _ => false,
...
/// `oexe tab ls`, which is why the reading itself lives in the core.
pub use osiris_core::core::tab_view::{TabLabel, TabView, tab_views_of};
...
/// `oexe new` does not look like it did nothing.
...
#[cfg(test)]
mod tests {
    use osiris_core::core::machine::{Axis, PaneNode, PaneSeed, Tab, TabId};
    ...
    fn a_named_workspace_reads_the_same_before_and_after_a_full_pull(
        ...
        cx.update(|cx| {
            ...
            // what `oexe new` leaves behind, and it reads as its directory.
    ...
    fn a_workspace_the_store_never_saw_is_still_listed_and_still_readable(
        ...
        cx.update(|cx| {
            ...
            // What `oexe new` leaves behind: on the machine, named, with a
    ...
    fn display_names_derive_from_the_tree_with_the_session_precedence() {
        ...
        ws.tabs[0].sidebar_group = Some("/repo/osiris".into());
        assert_eq!(
            display_name_of(&ws, &panes),
            "osiris",
