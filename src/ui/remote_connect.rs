...
use osiris_core::host::{Host as _, HostId};
...
pub const LOCAL_STDIO_ENV: &str = "OSIRIS_LOCAL_STDIO_SERVER";
...
fn client_hostname() -> String {
    ...
    NAME.get_or_init(|| {
        std::process::Command::new("hostname")
            ...
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| "an osiris client".to_string())
    })
    .clone()
}
...
pub fn rows_from_machine(machine: &osiris_core::core::machine::Machine) -> Vec<RemoteWorkspaceRow> {
...
/// layer's own wording, which reads like the far end is not osiris at all. It is —
...
mod tests {
    ...
    fn request() -> InstallRequest {
        InstallRequest {
            ...
            version: "0.9.1".into(),
            asset: "osiris-server-linux-x86_64-musl",
            source_url: "https://example.invalid/v0.9.1/osiris-server".into(),
            remote_path: "/home/me/.local/share/osiris/bin/osiris-server-0.9.1".into(),
    ...
    fn refusal(peer: u32, ours: u32) -> String {
        format!(
            "java answered, but not as an osiris server: control peer (build 26.7.7-nightly) \
    ...
    fn a_pane_and_its_workspace_resolve_to_the_same_machine() {
        ...
        let local = RemoteTarget::LocalStdio {
            program: "/opt/osiris-server".into(),
            args: vec!["--stdio".into()],
        };
    ...
    fn the_mismatch_prompt_names_the_host_and_both_versions() {
        ...
        let m = MismatchedRemoteDaemon {
            host: "me@build-box:22".into(),
            running_version: Some("0.8.0".into()),
            running_exe: Some("/home/me/.local/share/osiris/bin/osiris-server-0.8.0".into()),
            wanted_version: "0.9.1".into(),
        };
    ...
    fn rows_from_the_tree_sort_newest_first_and_derive_names() {
        use osiris_core::core::machine::{Machine, PaneRecord, Tab, Workspace};
