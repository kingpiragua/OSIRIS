...
pub(crate) fn menu_spec(snap: &TraySnapshot) -> Vec<SpecItem> {
    ...
    let mut items = vec![item("show", t(L10nKey::TrayShowOsiris)), SpecItem::Separator];
...
mod tests {
    ...
    fn attention_follows_waiting_and_tooltip_counts() {
        ...
        assert_eq!(TraySnapshot::default().tooltip(), "OSIRIS.EXE");
    }
...
fn deliver(action: TrayAction, workspace: osiris_core::core::session::WorkspaceId, cx: &mut App) {
    use crate::ui::windows::WindowRegistry;
