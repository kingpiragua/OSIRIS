...
// osiris-core next to the validation: a local, narrower pair used to push a
...
/// of the set fills.
///
/// Every icon osiris draws itself sits on the same optical bound — 3.4..20.6 of
...
const DOCS_URL: &str = "https://github.com/kingpiragua/osiris#readme";
// Upstream's community link would send OSIRIS.EXE users to tty7's Discord,
// which is not this project's room. The action keeps its name (it is a
// keybinding id users may have bound) and points at the archive instead.
const DISCORD_URL: &str = "https://osirisexe.com";
const ISSUES_URL: &str = "https://github.com/kingpiragua/osiris/issues/new";
...
pub struct Tab {
    ...
    /// while the tab is active: then the zoom lives in `OsirisApp::maximized`.
    ...
    pub(crate) tree_id: std::cell::Cell<osiris_core::core::machine::TabId>,
...
impl Tab {
    fn new(pane: Pane) -> Self {
        Self {
            ...
            tree_id: std::cell::Cell::new(osiris_core::core::machine::TabId::new()),
            last_used: std::cell::Cell::new(0),
        }
    }

    pub(crate) fn from_tree(tree: &osiris_core::core::machine::Tab, pane: Pane) -> Self {
        Self {
            pane,
...
pub(crate) struct Renaming {
    ...
    pub(crate) tab: osiris_core::core::machine::TabId,
...
pub struct OsirisApp {
    ...
    pending_tab: Option<osiris_core::core::machine::TabId>,
    ...
    pub(crate) tab_merge: Cell<
        Option<(
            osiris_core::core::machine::TabId,
    ...
    /// finishing is what completes it (see `OsirisApp::finish_connect`).
...
pub(crate) enum CloseTarget {
    Tab(osiris_core::core::machine::TabId),
    Pane,
}
...
impl OsirisApp {
    ...
    fn prompt_daemon_version_mismatch(window: &mut Window, cx: &mut Context<Self>) {
        ...
        let answer = window.prompt(
            ...
            Some(&detail),
            &[t(L10nKey::CmdQuitOsiris), t(L10nKey::RestartServer)],
            cx,
        );
        ...
        let (tabs, active) = match session {
            None => match new_terminal(
                ...
                Err(e) => {
                    ...
                    // everything, and it used to be what they saw when osiris
    ...
    pub(crate) fn detach_workspace(&self, cx: &mut App) {
            ...
            && crate::ui::tree_sync::workspace_is_disposable(cx, self.workspace)
        {
            crate::ui::tree_sync::fire_workspace_op(cx, self.workspace, |ws| {
                osiris_core::daemon::control::ControlRequest::WorkspaceRemove { workspace: ws }
            });
    ...
    /// by reading that window's `OsirisApp` back out of the registry. Called from
    ...
    pub(crate) fn sync_window_title(&self, window: &mut Window, cx: &App) {
        let title = WorkspaceStore::all(cx)
            ...
            .unwrap_or_else(|| "OSIRIS.EXE".to_string());
    ...
    fn tab_landing_rect(
        &self,
        id: osiris_core::core::machine::TabId,
        window: &Window,
    ) -> Option<(
        crate::ui::pane_drag::DropZone<gpui::EntityId>,
        Bounds<Pixels>,
    ...
    fn merge_tab(
        &mut self,
        source: osiris_core::core::machine::TabId,
    ...
    fn deliver_agent_prompt(&mut self, prompt: &str, window: &mut Window, cx: &mut Context<Self>) {
        let Some(target) = self.agent_target_leaf(cx) else {
            crate::terminal::notify_desktop(Some("OSIRIS.EXE"), t(L10nKey::AppNoRunningCodingAgent));
            return;
        };
    ...
    fn send_selection_to_agent(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some(selection) = selection else {
            crate::terminal::notify_desktop(Some("OSIRIS.EXE"), t(L10nKey::AppNothingSelected));
            return;
        };
    ...
    fn send_git_diff_to_agent(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Some((host, cwd)) = target else {
            crate::terminal::notify_desktop(Some("OSIRIS.EXE"), t(L10nKey::AppPaneNoKnownDirectory));
            return;
        };
        crate::ui::host_ops::HostOps::run_in(
            host,
    ...
    fn commit_http_proxy(&mut self, cx: &mut Context<Self>) {
        ...
        if !value.is_empty() && !osiris_core::daemon::install::proxy::is_valid_manual(&value) {
            cx.notify();
            return;
        }
    ...
    fn commit_working_directory_path(&mut self, cx: &mut Context<Self>) {
        ...
        // my project" reads as an osiris bug rather than a typo (#601). Refuse
    ...
    /// lives in `osiris-core` and cannot reach `src/ui/i18n` — it used to hand
    ...
    fn agent_hooks_outcome_msg(outcome: &crate::core::agent_hooks::HookOutcome) -> String {
        ...
        match outcome {
            ...
            O::NoOsirisHooks => t(L10nKey::AppAgentHooksNoOsirisHooks).to_string(),
        }
    }
...
fn pane_free_for(
    ...
    match alive.get(&id) {
        ...
        Some(Some(recorded)) => {
            ...
            // stamping its own name — `osiris` before this release wrote a
            // literal "osiris-cli" — and refusing on it strands every pane the
    ...
    for st in &session.tabs {
        ...
        tabs.push(Tab {
            ...
            tree_id: std::cell::Cell::new(
                st.tree_id
                    .unwrap_or_else(osiris_core::core::machine::TabId::new),
            ),
            last_used: std::cell::Cell::new(0),
        });
    }
...
fn watch_pane_focus(view: &Entity<TerminalView>, window: &mut Window, cx: &mut Context<OsirisApp>) {
...
mod tests {
    ...
    fn a_start_in_path_saves_only_when_it_names_a_real_directory() {
        ...
        let file = std::env::temp_dir().join("osiris-wd-saveable-probe");
    ...
    fn shell_arguments_round_trip_between_field_text_and_argv() {
        for case in [
            ...
            &[r"--dir", r"C:\Users\me"],
            &[r"--dir", r"C:\Program Files\osiris"],
            &["--tag", "#1"],
        ] {
            let argv = argv(case);
            let field_text = join_shell_args(&argv);
            assert_eq!(
                split_shell_args(&field_text).expect("osiris's own refill must parse"),
                argv,
    ...
    fn unbalanced_quotes_are_refused_not_shredded() {
        ...
        assert!(split_shell_args("--name 'osiris").is_err());
...
pub(crate) mod test_window {
    ...
    pub(crate) fn harness(cx: &mut TestAppContext) -> (Entity<OsirisApp>, VisualTestContext) {
        ...
        let window = cx.add_window(|window, cx| {
            let app =
                cx.new(|cx| OsirisApp::with_session(None, Some(Session::default()), window, cx));
        ...
        let app = window
            .update(cx, |root, _, _| {
                root.view()
                    ...
                    .downcast::<OsirisApp>()
                    .ok()
                    .expect("window root wraps a OsirisApp")
            })
            .unwrap();
...
mod ssh_rebuild_gpui_tests {
    ...
    use osiris_core::core::machine::{LayoutDelta, PaneNode, Tab as TreeTab};
...
mod keybinding_gpui_tests {
    ...
    fn begin_capture(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext, action: &str) {
...
mod restart_server_gpui_tests {
    ...
    async fn settling_after_a_restart_does_not_lease_the_window_twice(cx: &mut TestAppContext) {
        ...
        OsirisApp::settle_after_restart(app.downgrade(), Ok(()), &mut cx.to_async()).await;
...
mod shell_menu_gpui_tests {
    ...
    fn harness(cx: &mut TestAppContext) -> (Entity<OsirisApp>, VisualTestContext) {
        crate::core::config::pin_test_config_dir();
        cx.executor().allow_parking();
        cx.update(|cx| {
            gpui_component::init(cx);
    ...
    fn pump_until(
        app: &Entity<OsirisApp>,
        vcx: &mut VisualTestContext,
        done: impl Fn(&OsirisApp) -> bool,
    ) -> bool {
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
        loop {
            vcx.background_executor.run_until_parked();
