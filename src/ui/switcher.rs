...
use osiris_core::core::machine::TabId;
use osiris_core::core::session::{RemoteTarget, RouteSnapshot, WorkspaceId};
...
struct TabRow {
    ...
    git: Option<osiris_core::core::git::GitStatus>,
}
...
/// `oexe run` leaves behind, and what `oexe pane ls --all` points the CLI's
...
pub(crate) fn orphan_panes_of(
    listed: Vec<osiris_core::daemon::protocol::PaneInfo>,
    held: &HashSet<u64>,
) -> Vec<OrphanPane> {
    listed
        .into_iter()
...
fn default_workspace_name(chosen: Option<&HostChoice>, cx: &App) -> String {
    let host = match chosen {
        None => osiris_core::host::HostId::LOCAL,
...
impl OsirisApp {
    ...
    fn refresh_orphan_panes(&mut self, cx: &mut Context<Self>) {
        cx.spawn(async move |this, cx| {
            let listed = cx
                .background_spawn(async move { osiris_core::client::PaneClient::local().list() })
                .await;
            let listed = match listed {
                Ok(listed) => listed,
                Err(e) => {
                    log::warn!(target: "osiris::switcher", "orphan pane listing failed: {e}");
                    return;
                }
            };
        ...
        cx.spawn(async move |this, cx| {
            let listed = cx
                .background_spawn(async move {
                    let client = osiris_core::client::PaneClient::local();
                    client.kill(pane_id)?;
                    client.list()
                })
                .await;
            let listed = match listed {
                Ok(listed) => listed,
                Err(e) => {
                    log::warn!(target: "osiris::switcher", "closing orphan %{pane_id} failed: {e}");
                    return;
                }
            };
            let _ = this.update(cx, |this, cx| {
    ...
    fn switcher_groups(&self, cx: &mut Context<Self>) -> Vec<Group> {
        ...
        // person who ran `oexe new` like nothing happened at all. They open
    ...
    fn tab_rows_for(&self, id: WorkspaceId, cx: &App) -> Vec<TabRow> {
        ...
        let git = |cwd: Option<&str>| -> Option<osiris_core::core::git::GitStatus> {
...
/// `OsirisApp::tab_label` shows for local ones.
///
...
mod tests {
    ...
    fn orphan_panes_of_keeps_only_live_panes_no_workspace_holds() {
        use osiris_core::daemon::protocol::PaneInfo;
        ...
        fn info(pane_id: u64, alive: bool) -> PaneInfo {
            PaneInfo {
                ...
                osc_title: None,
                alive,
                owner: Some("osiris-cli".to_string()),
            }
        }
    ...
    fn a_tab_of_another_window_is_named_the_way_a_local_one_would_be() {
        ...
        let mut view = crate::ui::machine_mirror::TabView {
            ...
            cwd: Some("/Users/x/repo/osiris".to_string()),
        ...
        view.osc_title = Some("user@host:~/repo/025/osiris".to_string());
        assert_eq!(
            tab_view_label(&view, 0, None),
            crate::ui::tab_strip::short_title("user@host:~/repo/025/osiris", None),
        ...
        assert_eq!(
            tab_view_label(&view, 0, None),
            crate::ui::tab_strip::short_title("/Users/x/repo/osiris", None),
