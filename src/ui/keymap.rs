...
/// The bindings that were already in the keymap before osiris put its own
...
/// Replaces the keymap with the full set: the bindings osiris inherited, then
...
/// `init` bound them in, and gpui reads the map back to front, so osiris's
...
fn authored_entry(action: &str) -> Option<(CommandGroup, String)> {
    ...
    Some(match action {
        ...
        "About" => (
            CommandGroup::Application,
            t(L10nKey::CmdAboutOsiris).to_string(),
        ),
        "CheckForUpdates" => (
            CommandGroup::Application,
        ...
        "Quit" => (
            CommandGroup::Application,
            t(L10nKey::CmdQuitOsiris).to_string(),
        ),
...
mod gpui_tests {
    ...
    #[gpui::test]
    fn a_rebuild_keeps_the_bindings_osiris_did_not_install(cx: &mut TestAppContext) {
        cx.update(|cx| {
            ...
            assert!(
                !inherited.is_empty(),
                "gpui-component binds backspace before osiris touches the keymap"
            );
