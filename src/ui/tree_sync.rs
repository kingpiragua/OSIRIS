...
use osiris_core::core::machine::{
    AgentFacts, Axis as TreeAxis, LayoutDelta, Machine, PaneNode, PaneRecord, PaneSeed, Side,
    Tab as TreeTab, TabId, Workspace,
};
use osiris_core::daemon::control::{ControlClient, ControlRequest, ReplyOk};
use osiris_core::host::HostId;
...
fn classify_tree_link(client: Option<Arc<ControlClient>>) -> TreeLink {
    match client {
        Some(client)
            if client
                .hello()
                .has_feature(osiris_core::daemon::control::feature::MACHINE_TREE) =>
        {
...
pub(crate) fn sync_window(app: &OsirisApp, cx: &mut App) {
...
fn adopt_tab_ids(app: &OsirisApp, cx: &App) {
...
pub(crate) fn fresh_workspace_name(cx: &App, host: HostId) -> String {
    ...
    osiris_core::core::codename::unique(|name| taken.iter().any(|t| t == name))
}
...
pub(crate) fn session_from_tree(
    ws: &osiris_core::core::machine::Workspace,
    panes: &[PaneRecord],
) -> Session {
...
///
/// This is what a launch carrying a directory does — Explorer's "Open in osiris",
/// or `osiris <PATH>` with no window up. Both halves are wanted: the layout the
...
/// dialect behind reads as "osiris lost my tabs" with nothing anywhere to say
...
/// shell still on the machine: "osiris lost my session" for a request that
/// needed asking twice.
///
...
fn pull_workspace(
    ...
    let name = chosen.unwrap_or_else(|| osiris_core::core::codename::unique(|n| taken.contains(&n)));
    match client.call(ControlRequest::WorkspaceCreate {
        name: Some(name),
        workspace: Some(machine_ws),
    }) {
...
/// Someone else removed this workspace from its machine — `oexe ws rm`, or
/// another client.
///
...
pub(crate) fn note_instance(seen: &mut String, instance: &str) -> bool {
    ...
    if !before.is_empty() && before != instance {
        log::info!(
            "the osiris-server on this machine is a new process ({before} → {instance}); \
             its panes are gone"
        );
        return true;
    }
...
mod tests {
    ...
    fn a_hydrating_pull_settles_the_typed_name_too(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            ...
            let epoch = cx.default_global::<TreeSync>().windows[&ws].epoch;
            let pulled = Machine {
                workspaces: vec![osiris_core::core::machine::Workspace {
                    id: ws,
    ...
    fn primed_window(
        ...
        osiris_core::core::config::set_config_dir(
            std::env::temp_dir().join(format!("osiris-chosen-name-{}", std::process::id())),
        );
    ...
    fn a_peer_without_the_machine_tree_bit_classifies_as_unserved() {
        ...
        use osiris_core::host::local::LocalHost;
        use osiris_core::host::server::{Services, serve_with};
        ...
        let connect = |services: Services| {
            ...
            Arc::new(
                osiris_core::daemon::control::ControlClient::over_unix(
                    client,
                    &hello,
                    Box::new(|_| {}),
                )
        ...
        let dir = std::env::temp_dir().join(format!("osiris-treelink-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let store = osiris_core::core::machine::MachineStore::open(
            dir.join(osiris_core::core::machine::MACHINE_FILE),
        );
    ...
    ///
    /// The other half needs a live `OsirisApp` in a real window to reach, so it
    ...
    fn a_deletion_nothing_has_open_forgets_the_workspace_here_too(cx: &mut gpui::TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            let _ = osiris_core::core::config::set_config_dir(
                std::env::temp_dir().join(format!("osiris-deleted-test-{}", std::process::id())),
            );
            crate::ui::windows::WindowRegistry::init(cx);
    ...
    fn priming_teaches_the_mirror_the_name_the_machine_gave_the_workspace(
        ...
        cx.update(|cx| {
            osiris_core::core::config::set_config_dir(
                std::env::temp_dir().join(format!("osiris-prime-name-{}", std::process::id())),
            );
            let view = crate::core::session::WindowView::default();
            let ws = view.id;
            WorkspaceStore::install_for_test(
    ...
    fn the_backoff_count_ends_with_the_run_of_failures(cx: &mut gpui::TestAppContext) {
        cx.update(|cx| {
            let _ = osiris_core::core::config::set_config_dir(
                std::env::temp_dir().join(format!("osiris-backoff-count-{}", std::process::id())),
            );
            let view = crate::core::session::WindowView::default();
            let ws = view.id;
            WorkspaceStore::install_for_test(
    ...
    fn a_lowered_leaf_carries_its_pane_id_and_its_agent_whatever_live_says() {
        ...
        let ws = osiris_core::core::machine::Workspace {
            tabs: vec![TreeTab {
                id: tab_id,
                name: Some("build".into()),
