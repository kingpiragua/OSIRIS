...
use gpui::{App, Global};
use osiris_core::daemon::control::ControlClient;
                ...
                match connected {
                    Ok(client) => {
                        ...
                        if restarted {
                            ...
                            crate::ui::tree_sync::on_link_up(cx, osiris_core::host::HostId::LOCAL);
                        }
                    }
...
fn connect_blocking() -> std::io::Result<Arc<ControlClient>> {
    ...
    let sink: osiris_core::daemon::control::EventSink = Box::new(local_event_sink);
    #[cfg(unix)]
    let client = ControlClient::over_unix(
        std::os::unix::net::UnixStream::connect(osiris_core::host::server::control_socket_path()?)?,
        &hello,
        sink,
    )?;
    #[cfg(windows)]
    let client =
        ControlClient::over_tcp(osiris_core::host::server::connect_control()?, &hello, sink)?;
    Ok(Arc::new(client))
}
...
fn dialect_refusal(e: &std::io::Error) -> Option<osiris_core::daemon::control::DialectRefusal> {
    osiris_core::daemon::control::parse_dialect_refusal(&e.to_string())
}

fn local_event_sink(event: osiris_core::daemon::control::ControlEvent) {
    osiris_core::daemon::control::observe_event(osiris_core::host::HostId::LOCAL, event);
}
