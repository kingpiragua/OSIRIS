...
impl FileTreeState {
    pub(crate) fn new(window: &mut Window, cx: &mut Context<OsirisApp>) -> Self {
    ...
    fn sync_watch(&mut self, host: SharedHost, dirs: HashSet<PathBuf>, cx: &mut Context<OsirisApp>) {
        self.watched = dirs;
        ...
        if let Some(sub) = self.watch.clone() {
            ...
            HostOps::run(
                ...
                |app: &mut OsirisApp, result: std::io::Result<()>, cx| {
    ...
    fn sync_search(&mut self, query: &str, roots: &[PathBuf], cx: &mut Context<OsirisApp>) {
...
impl OsirisApp {
    ...
    fn file_tree_begin_edit(
        ...
        let sub = cx.subscribe_in(
            &input,
            window,
            |this: &mut OsirisApp, _input, ev, window, cx| match ev {
...
fn dotfiles_menu_item(show_hidden: bool, app: &gpui::WeakEntity<OsirisApp>) -> PopupMenuItem {
    let app = app.clone();
...
mod tests {
    ...
    fn the_tree_reads_the_same_listing_out_of_the_host() {
        let host = osiris_core::host::local::LocalHost::new();
        let tmp = std::env::temp_dir().join(format!("osiris-tree-host-{}", std::process::id()));
    ...
    fn a_symlink_cycle_cannot_make_the_search_walk_forever() {
        let host = osiris_core::host::local::LocalHost::new();
        let tmp = std::env::temp_dir().join(format!("osiris-tree-loop-{}", std::process::id()));
        let _ = host.remove(&tmp, true);
...
mod render_idle_gpui_tests {
    ...
    use osiris_core::core::config::RightPanelTab;
    ...
    pub(super) fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-idle-{name}-{}", std::process::id()));
    ...
    pub(super) fn rows(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext) -> usize {
    ...
    fn fs_event(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext, path: &Path) {
        app.update_in(vcx, |app, _, cx| {
            app.file_tree_apply_fs_events(HostId::LOCAL, &HashSet::from([path.to_path_buf()]), cx);
        });
    }

    pub(super) fn settle(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext, root: &Path) {
    ...
    fn git(root: &Path, args: &[&str]) -> bool {
        let mut full = vec![
            "-c",
            "user.name=osiris test",
            "-c",
            "user.email=test@osiris.invalid",
            "-c",
