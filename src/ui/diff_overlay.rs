...
/// The de-duplication sets on `OsirisApp` are keyed by `(HostId, PathBuf)`, so
...
mod overlay_gpui_tests {
    ...
    fn a_second_source_over_one_directory_is_a_second_overlay(cx: &mut TestAppContext) {
        ...
        let cwd = std::path::PathBuf::from("/no/such/osiris/repo");
    ...
    fn one_file_snapshot(source: DiffSource) -> DiffSnapshot {
        ...
        DiffSnapshot {
            root: std::path::PathBuf::from("/no/such/osiris/repo"),
            source,
            branch: "main".into(),
            files: vec![FileDiff {
                path: "a.rs".into(),
    ...
    fn show(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext, source: DiffSource) {
        let cwd = std::path::PathBuf::from("/no/such/osiris/repo");
...
///
/// The way in is ordinary: switch branches anywhere outside osiris — another
...
mod render_idle_gpui_tests {
    ...
    fn an_overlay_over_a_stale_branch_reaches_render_idle(cx: &mut TestAppContext) {
        let root = std::env::temp_dir().join(format!("osiris-diff-idle-{}", std::process::id()));
        ...
        // The cache as a branch switch outside osiris leaves it: a branch this
