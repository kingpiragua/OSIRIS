...
mod gpui_tests {
    ...
    fn harness(cx: &mut TestAppContext) -> (WindowHandle<OsirisApp>, VisualTestContext) {
        ...
        let window = cx.add_window(|window, cx| {
            OsirisApp::with_session(None, Some(Session::default()), window, cx)
        });
        window
    ...
    fn badges_shown(window: &WindowHandle<OsirisApp>, cx: &mut TestAppContext) -> bool {
        window
            .update(cx, |app, _, _| app.mod_hint_badges)
            .expect("the app window stays open")
    }

    fn wait_for_badges(window: &WindowHandle<OsirisApp>, cx: &mut TestAppContext) {
