...
pub fn translate_zh(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::QuitStopServerBody => {
            "这会退出 OSIRIS.EXE 并停止后台 server，所有仍在运行的 shell 都会被终止。你的标签页和布局会被保留，下次启动时以全新的 shell 重新打开。（关闭窗口只会把应用收起到托盘，shell 保持运行。）"
        }
        ...
        L10nKey::SettingsLanguageDesc => "选择 OSIRIS.EXE 界面使用的语言。",
        ...
        L10nKey::SettingsCustomThemesIntro => {
            "复制一个主题后可在此编辑其颜色，或者把自定义主题放入主题文件夹：OSIRIS.EXE YAML 主题或 iTerm2 的 .itermcolors 方案。"
        }
        ...
        L10nKey::SettingsInOsiris => "在 OSIRIS.EXE 中",
        ...
        L10nKey::SettingsTypeAddressToConnect => "输入地址即可立刻连接，之后 OSIRIS.EXE 会提示保存。",
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "重新读取文件并添加新内容。你在这里做的编辑由 OSIRIS.EXE 保存——不会写入该文件本身。"
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "有 {count} 个选项在 OSIRIS.EXE 中没有对应设置，仍留在文件里：{options}"
        }
        ...
        L10nKey::SettingsStartInDesc => "新 shell 的启动目录：OSIRIS.EXE 的启动目录、主目录或固定路径。",
        ...
        L10nKey::SettingsPromptIntro => {
            "shell 提示符处的 OSIRIS.EXE 自带编辑器与菜单。关闭某项即可把这部分交还给 shell。"
        }
        L10nKey::SettingsPromptEditor => "提示符编辑器",
        L10nKey::SettingsPromptEditorDesc => {
            "由 OSIRIS.EXE 接管你在 shell 提示符上敲的这一行：选择、撤销，以及下面这些菜单。关闭后，提示符处的每个按键、输入法上屏和粘贴都直接交给 shell，由它自己的行编辑器——zsh 的 ZLE、readline、fish——负责编辑，你在那里绑定的键位照常生效。两种模式下 shell 集成都保持开启。"
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "在提示符按 Tab 打开 OSIRIS.EXE 的补全菜单。关闭后 Tab 交由 shell 自身的补全处理。"
        }
        L10nKey::SettingsHistorySearch => "历史搜索",
        L10nKey::SettingsHistorySearchDesc => {
            "在提示符按 ⌃R 打开 OSIRIS.EXE 的模糊历史菜单。关闭后 ⌃R 交由 shell 处理——它自带的反向搜索，或你在那里绑定的其它功能（fzf、percol）。"
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "hook 集成让标签栏中的窗格实时显示这些 agent 的会话状态（进行中 / 等待中 / 已完成）。仅在 OSIRIS.EXE 内生效。"
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "OSIRIS.EXE 启动时的窗口状态。",
        L10nKey::SettingsRememberWindowSize => "记住窗口大小与位置",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "以 OSIRIS.EXE 上次退出时窗口的大小和位置重新打开。关闭时以默认大小居中打开。"
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => "下次启动 OSIRIS.EXE 时会自动装上。",
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "OSIRIS.EXE 是原地升级的，界面已经是新版本，但各个 pane 仍由旧版本的后台 server 托管。重启 server 才能用上新版本，代价是 pane 里正在跑的进程全部结束——shell、agent、SSH 会话都算。不急，挑个 pane 空闲的时候再重启。"
        }
        L10nKey::UpdateDialogTitle => "有可用更新",
        L10nKey::UpdateDialogDetail => {
            "OSIRIS.EXE {version} 已发布，你现在是 {current}。安装会重启应用；后台 server 不动，pane 里开着的东西都还在。"
        }
        L10nKey::UpdateDialogDetailWindows => {
            "OSIRIS.EXE {version} 已发布，你现在是 {current}。安装会重启应用和后台 server：pane 里正在运行的进程会被结束，标签页和布局会以全新的 shell 恢复。"
        }
        L10nKey::UpdateDialogDetailManual => "OSIRIS.EXE {version} 已发布，你现在是 {current}。{hint}",
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "这份 OSIRIS.EXE 是为所有用户安装的，开始安装前 Windows 会请求一次管理员批准。OSIRIS.EXE 本身不会以管理员身份运行——重启后仍以你的身份回来。"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "当前副本并非从可写的 osiris.app 包运行，直接替换并不安全。请将 OSIRIS.EXE 移到“应用程序”或其他可写文件夹，或者打开发布页面安装更新。"
        }
        ...
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "OSIRIS.EXE 是为所有用户安装的，替换它需要管理员权限。OSIRIS.EXE 不会自行弹出提权请求，请打开发布页面并自行运行安装程序进行更新。"
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "该版本缺少 checksums.txt，因此 OSIRIS.EXE 拒绝自动安装。"
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "启动时将自带的 OSIRIS.EXE 命令加入 PATH，让脚本和编码 agent 可在任意终端驱动 OSIRIS.EXE。在 OSIRIS.EXE 窗格内两种情况都可用。如果你自己构建或安装了 OSIRIS.EXE 且不希望被遮蔽，请关闭此选项。下次启动时生效。"
        }
        L10nKey::SettingsInstallCliOnPath => "将 `OSIRIS.EXE` 命令安装到 PATH",
        ...
        L10nKey::SettingsHowShellsWorkBody => {
            "你的 shell 跑在后台 server 里，不在这个窗口里。退出 OSIRIS.EXE 后它们照常运行——再打开时标签页、布局和工作目录都会回来，里面还是原来那些 shell。关闭标签页会结束它那个 shell；“重启 server”和“退出并停止 server”会结束全部。"
        }
        L10nKey::SettingsRestartServer => "重启 server…",
        L10nKey::SettingsAppHttpProxy => "更新代理",
        L10nKey::SettingsAppHttpProxyDesc => {
            "仅用于 OSIRIS.EXE 自身的更新检查和下载，不影响面板中运行的程序。留空则跟随系统代理。"
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "命令行工具 cli OSIRIS.EXE 路径 shell 命令 安装 符号链接 terminal command line tool"
        }
        ...
        L10nKey::DiffTruncatedBudget => {
            "内容未加载——此 worktree 已超出 OSIRIS.EXE 的 diff 预算。在终端中运行 git diff 查看此文件。"
        }
        ...
        L10nKey::DiffBudgetAndCap => "OSIRIS.EXE 的预算和单文件上限",
        L10nKey::DiffBudget => "OSIRIS.EXE 的预算",
        ...
        L10nKey::HomeIntegrityDetail => "{total} 个工作区中 {live} 个在运行 · {panes} 个面板",
        L10nKey::HomeIntegrityCeiling => "上限 —— 最后的 1/64 无法独自恢复",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "此窗口是 {machine} 上的工作区，但 OSIRIS.EXE 已没有它的连接详情——\
             请检查其 SSH 主机配置或 ~/.ssh/config 条目是否仍然存在。"
        }
        L10nKey::RemoteThisComputer => "本机",
        L10nKey::RemoteProfileGone => "已删除的配置",
        L10nKey::RemoteRestartTitle => "重启“{machine}”上的 OSIRIS server？",
        ...
        L10nKey::RemoteReplaceBody => {
            "OSIRIS.EXE 会在 {machine} 上安装匹配的 server 并启动它。\n\
             \n\
             {machine} 上运行的所有会话都会结束，包括此窗口未连接的会话。"
        }
        L10nKey::RemoteRestartFailedTitle => "“{machine}”上的 OSIRIS server 未被重启",
        ...
        L10nKey::RemoteInstallTitle => "在“{machine}”上安装 OSIRIS server？",
        L10nKey::RemoteInstallDetail => {
            "OSIRIS.EXE 会将其 server 二进制文件写入 {machine}，以便本机可以在那里托管\
        ...
        L10nKey::RemoteMismatchTitle => "更新“{machine}”上的 OSIRIS server？",
        L10nKey::RemoteMismatchDetail => {
            "{machine} 正在使用 {running} 提供 OSIRIS.EXE 会话，该版本使用的协议无法被\
             此客户端（{wanted}）识别。OSIRIS.EXE 已在那里安装了匹配的 server，\
             但正在运行的是你当前会话所在的版本。\n\
             \n\
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} 上的 OSIRIS server 太旧（{build}），当前这份 OSIRIS.EXE 连不上它。\
             更新它才能连接。"
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} 上的 OSIRIS server（{build}）比当前这份 OSIRIS.EXE 还新。\
             请更新本机的 OSIRIS.EXE，或把那边的 server 替换成匹配的版本。"
        }
        L10nKey::RemoteDaemonStartFailed => "无法启动 OSIRIS.EXE 本地 server：{error}",
        L10nKey::RemoteDaemonUnreachable => "无法连接到 OSIRIS.EXE 本地 server：{error}",
        L10nKey::RemoteDaemonTooOld => {
            "此机器上的 OSIRIS.EXE 守护进程版本较旧，无法重启 {machine} 上的 server。\
             请退出 OSIRIS.EXE（这会停止守护进程）并重新打开，然后重试。"
        }
        ...
        L10nKey::RemoteHostNotOsiris => "{machine} 已响应，但并非作为 OSIRIS server：{error}",
        L10nKey::RemoteWorkspaceListFailed => "已连接到 {machine}，但其工作区列表获取失败：{error}",
        L10nKey::RemoteServerRestartFailed => "无法重启 {machine} 上的 OSIRIS server：{error}",
        L10nKey::RemoteNoRouteToHost => "OSIRIS.EXE 已无法到达 {machine}",
        ...
        L10nKey::SwitcherRestartingServer => "正在重启 OSIRIS server…",
        L10nKey::SwitcherDownloadingServerWithTotal => "正在下载 OSIRIS server… {done} / {total}",
        L10nKey::SwitcherDownloadingServerNoTotal => "正在下载 OSIRIS server… {done}",
        L10nKey::SwitcherCopyingServer => "正在复制 OSIRIS server… {done} / {total}",
        ...
        L10nKey::AppRestartServerNoServer => {
            "{label} 上没有 OSIRIS.EXE 自己的 server 可重启——它是本机通过 --stdio 运行的程序。请改为停止其工作区。"
        }
        ...
        L10nKey::ConfigQuarantinedStartup => {
            "config.json 无法解析，OSIRIS.EXE 正以默认设置运行，也不会覆写该文件。原内容已保留为旁边的 config.json.corrupt——修好文件后会自动重载；在此之前，设置里的更改不会被保存。"
        }
        ...
        L10nKey::ConfigUnreadableStartup => {
            "config.json 读取失败，OSIRIS.EXE 正以默认设置运行，也不会覆写该文件——文件原样保留。修好它的权限或内容后会自动重载；在此之前，设置里的更改不会被保存。"
        }
        ...
        L10nKey::AppForkNoCommand => "OSIRIS.EXE 没有用于 {name} 的 fork 命令",
        L10nKey::AppForkLocalOnly => "{name} 会话只能从本地窗格 fork",
        L10nKey::AppForkNoSessionId => {
            "OSIRIS.EXE 尚未在此窗格中看到 {name} 的会话 ID——请在设置 → Agents 中安装其 hook"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "OSIRIS.EXE 无法确定这台计算机的主目录，因此没有可安装的位置。"
        }
        ...
        L10nKey::AppAgentHooksNoOsirisHooks => "没有找到 OSIRIS.EXE 钩子，无需移除",
        ...
        L10nKey::SettingsShellDetected => "OSIRIS.EXE 检测到的 shell",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "OSIRIS.EXE 是原地更新的，所以应用是新的，但你的面板仍由上一个版本在服务。\
             server 可以在不停止的情况下把自己换成新版本：你的 shell 和里面正在跑的东西会直接延续下来。\
             用 OSIRIS.EXE 内置 SSH 客户端的面板除外——那些连接会断开，需要重新打开。"
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "上方向键翻的是这个面板里跑过的命令，而不是所有面板混在一起的结果。\
             ...
             只对 OSIRIS.EXE 能接管的 bash 和 zsh 面板生效；用你自己参数启动的 shell 不受影响。"
        }
        L10nKey::IntegrationNoticeBlocked => {
            "此窗格中的 OSIRIS.EXE shell 集成被拦截——“{wrapper}”截获了 shell 上报，\
             内联补全和 Ctrl+R 菜单不可用。shell 自带的历史搜索仍可使用。"
        }
        L10nKey::IntegrationNoticeNotEngaged => {
            "此窗格中的 OSIRIS.EXE shell 集成尚未生效，内联补全和 Ctrl+R 菜单不可用。\
        ...
        L10nKey::PaletteTryDifferentSearch => "TYPE \"help\" — 或换个关键词。",
...
pub fn translate_variant_zh(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => "文件里的每个选项在 OSIRIS.EXE 中都有对应设置。",
        (L10nKey::SettingsImportIgnored, "one") => {
            "有 1 个选项在 OSIRIS.EXE 中没有对应设置，仍留在文件里：{options}"
        }
