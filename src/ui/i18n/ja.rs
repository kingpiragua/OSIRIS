...
pub fn translate_ja(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::Close => "閉じる",
        L10nKey::QuitStopServerTitle => "OSIRIS.EXE を終了してサーバーを停止しますか？",
        L10nKey::QuitStopServerBody => {
            "OSIRIS.EXE を終了してバックグラウンドサーバーを停止します。シェルで実行中のものはすべて終了します。タブとレイアウトは保持され、次回起動時に新しいシェルで開きます。ウィンドウを閉じただけではアプリはトレイに退避し、シェルは動き続けます。"
        }
        ...
        L10nKey::SettingsLanguageDesc => "OSIRIS.EXE の表示言語を選択します",
        ...
        L10nKey::SettingsCustomThemesIntro => {
            "テーマを複製して色を編集するか、テーマフォルダに自作テーマ（OSIRIS.EXE の YAML テーマまたは iTerm2 の .itermcolors スキーム）を置けます"
        }
        ...
        L10nKey::SettingsInOsiris => "OSIRIS.EXE 内",
        ...
        L10nKey::SettingsTypeAddressToConnect => {
            "アドレスを入力するとすぐに接続できます。OSIRIS.EXE はあとで保存するか尋ねます"
        }
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "ファイルを再読み込みして新しい項目を追加します。ここでの編集は OSIRIS.EXE が保存します — ファイル自体には書き込まれません"
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "OSIRIS.EXE に設定のないオプションが {count} 件あり、ファイルに残されています: {options}"
        }
        ...
        L10nKey::SettingsStartInDesc => {
            "新しいシェルの開始場所: OSIRIS.EXE の起動ディレクトリ、ホームフォルダ、または固定パス"
        }
        ...
        L10nKey::SettingsPromptIntro => {
            "シェルプロンプトでの OSIRIS.EXE 独自のエディターとメニュー。オフにするとその分がシェルに渡されます"
        }
        L10nKey::SettingsPromptEditor => "プロンプトエディター",
        L10nKey::SettingsPromptEditorDesc => {
            "シェルプロンプトで入力する行を OSIRIS.EXE が持ちます — 選択、取り消し、そして下のメニュー。オフにすると、プロンプトでのキー、IME の確定、貼り付けはすべてシェルへ直接渡り、シェル自身の行エディター（zsh の ZLE、readline、fish）が編集を担当するため、そこでバインドしたキーがそのまま動きます。どちらの場合もシェル統合は有効なままです"
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "プロンプトで Tab を押すと OSIRIS.EXE の補完メニューが開きます。オフの場合、Tab はシェル自身の補完に渡されます"
        }
        L10nKey::SettingsHistorySearch => "履歴検索",
        L10nKey::SettingsHistorySearchDesc => {
            "プロンプトで ⌃R を押すと OSIRIS.EXE のファジー履歴メニューが開きます。オフの場合、⌃R はシェルに渡されます — シェルの逆方向検索や、シェルでバインドしたもの（fzf、percol など）"
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "フック統合により、これらのエージェントを実行するペインのセッション状態（作業中 / 待機中 / 完了）がタブバーに表示されます。OSIRIS.EXE 内でのみ有効です"
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "OSIRIS.EXE 起動時のウィンドウ状態",
        L10nKey::SettingsRememberWindowSize => "ウィンドウサイズと位置を記憶",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "OSIRIS.EXE が最後に終了したときのサイズと位置で開き直します。オフならデフォルトサイズで中央に開きます"
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => "次回 OSIRIS.EXE を起動したときに適用されます。",
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "OSIRIS.EXE はその場で更新されたため、アプリは新しくなりましたが、各ペインは以前のビルドのサーバーが処理しています。サーバーを再起動すると新しいビルドに切り替わりますが、ペインで動いているプロセス（シェル、エージェント、SSH セッション）はすべて終了します。急ぐ必要はありません。ペインが空いているときに実行してください"
        }
        L10nKey::UpdateDialogTitle => "アップデートがあります",
        L10nKey::UpdateDialogDetail => {
            "OSIRIS.EXE {version} が利用できます（現在 {current}）。インストールするとアプリが再起動します。バックグラウンドサーバーは動いたままなので、ペインで開いているものはそのまま残ります。"
        }
        L10nKey::UpdateDialogDetailWindows => {
            "OSIRIS.EXE {version} が利用できます（現在 {current}）。インストールするとアプリとバックグラウンドサービスが再起動します。ペインで実行中のプロセスは終了し、タブとレイアウトは新しいシェルで復元されます。"
        }
        L10nKey::UpdateDialogDetailManual => {
            "OSIRIS.EXE {version} が利用できます（現在 {current}）。{hint}"
        }
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "この OSIRIS.EXE は全ユーザー向けにインストールされているため、インストール開始前に Windows の管理者承認が一度だけ求められます。OSIRIS.EXE 自体が管理者権限で実行されることはなく、あなたの権限のまま再起動します。"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "この OSIRIS.EXE は書き込み可能な osiris.app バンドルから実行されていないため、そのまま置き換えるのは安全ではありません。OSIRIS.EXE を「アプリケーション」など書き込み可能なフォルダへ移動するか、リリースページを開いてアップデートをインストールしてください"
        }
        ...
        L10nKey::SettingsUpdateUnsupportedWindows => {
            "Windows の自動更新は、認識可能な Inno Setup 版とポータブル ZIP 版に対応しています。この OSIRIS.EXE には有効なインストール情報・アップデーター・書き込み可能なポータブルディレクトリのいずれかが見つからないため、リリースページを開いて手動で更新してください"
        }
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "OSIRIS.EXE はすべてのユーザー向けにインストールされているため、置き換えには管理者権限が必要です。OSIRIS.EXE が自ら昇格を要求することはありません。リリースページを開き、インストーラーを手動で実行して更新してください"
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "このリリースには checksums.txt がないため、OSIRIS.EXE は自動インストールを行いません"
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "起動時に同梱の `OSIRIS.EXE` コマンドを PATH に入れ、スクリプトやコーディングエージェントが任意のターミナルから OSIRIS.EXE を操作できるようにします。OSIRIS.EXE のペイン内ではどちらでも機能します。自分でビルド・インストールした `OSIRIS.EXE` を上書きされたくない場合はオフにしてください。次回起動時に有効になります"
        }
        L10nKey::SettingsInstallCliOnPath => "`OSIRIS.EXE` コマンドを PATH にインストール",
        ...
        L10nKey::SettingsHowShellsWorkBody => {
            "シェルはこのウィンドウの中ではなく、バックグラウンドのサーバーで動いています。OSIRIS.EXE を終了してもシェルは動き続け、開き直せばタブ・レイアウト・作業ディレクトリが同じシェルのまま戻ります。タブを閉じるとそのシェルは終了し、「サーバーを再起動」と「終了してサーバーを停止」はすべてのシェルを終了します。"
        }
        L10nKey::SettingsRestartServer => "サーバーを再起動…",
        L10nKey::SettingsAppHttpProxy => "アップデート用プロキシ",
        L10nKey::SettingsAppHttpProxyDesc => {
            "OSIRIS.EXE 自身の更新チェックとダウンロードにのみ使用し、ペインで実行中のプログラムには影響しません。空欄ならシステムのプロキシに従います"
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "cli OSIRIS.EXE パス シェル コマンド インストール シンボリックリンク ターミナル iterm エージェント スクリプト command line tool"
        }
        ...
        L10nKey::DiffTruncatedBudget => {
            "差分の内容は読み込まれていません — このワーキングツリーは OSIRIS.EXE の Diff 予算を超えています。ターミナルでこのファイルの `git diff` を実行してください"
        }
        ...
        L10nKey::DiffBudgetAndCap => "OSIRIS.EXE の予算とファイルごとの上限",
        L10nKey::DiffBudget => "OSIRIS.EXE の予算",
        ...
        L10nKey::HomeIntegrityDetail => "{total} 個中 {live} 個のワークスペースが稼働中 · {panes} ペイン",
        L10nKey::HomeIntegrityCeiling => "上限 — 最後の 1/64 は独りでは戻せない",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "このウィンドウは {machine} 上のワークスペースですが、OSIRIS.EXE には接続情報がありません。SSH プロファイルか ~/.ssh/config に項目があるか確認してください"
        }
        L10nKey::RemoteThisComputer => "このコンピュータ",
        L10nKey::RemoteProfileGone => "削除されたプロファイル",
        L10nKey::RemoteRestartTitle => "「{machine}」上の OSIRIS.EXE サーバーを再起動しますか？",
        ...
        L10nKey::RemoteInstallTitle => "「{machine}」に OSIRIS.EXE サーバーをインストールしますか？",
        L10nKey::RemoteInstallDetail => {
            "OSIRIS.EXE はサーバーバイナリを {machine} に書き込み、{machine} でワークスペースをホストできるようにします。{machine} 上の他のものには触れず、sudo も使いません。\n\n{path_label}\u{2003}{path}\n{version_label}\u{2003}{version}\n{size_label}\u{2003}{size}\n{from_label}\u{2003}{from}\n{sha_label}\u{2003}{sha256}\n\n{silent_upgrades}"
        }
        ...
        L10nKey::RemoteMismatchTitle => "「{machine}」上の OSIRIS.EXE サーバーを更新しますか？",
        L10nKey::RemoteMismatchDetail => {
            "{machine} は {running} から OSIRIS.EXE セッションを提供していますが、このクライアント（{wanted}）はそのプロトコルを理解できません。OSIRIS.EXE は対応するサーバーをそこにインストール済みですが、セッションは実行中のサーバー上にあります。\n\n{replace_server}\u{2003}を選ぶと {wanted} に置き換えられ、そのサーバー上のセッションはすべて終了します。\n{cancel}\u{2003}を選ぶと {machine} はそのままです。このウィンドウは接続しません"
        }
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} の OSIRIS.EXE サーバーが古く（{build}）、この OSIRIS.EXE からは通信できません。更新すると接続できます"
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} の OSIRIS.EXE サーバー（{build}）は、この OSIRIS.EXE より新しいバージョンです。このコンピューターの OSIRIS.EXE を更新するか、向こうのサーバーを対応するものに置き換えてください"
        }
        L10nKey::RemoteDaemonStartFailed => {
            "OSIRIS.EXE のローカルサーバーを起動できませんでした: {error}"
        }
        L10nKey::RemoteDaemonUnreachable => {
            "OSIRIS.EXE のローカルサーバーに到達できませんでした: {error}"
        }
        L10nKey::RemoteDaemonTooOld => {
            "このマシンの OSIRIS.EXE デーモンは古いビルドのため、{machine} 上のサーバーを再起動できません。OSIRIS.EXE を終了（デーモンが停止します）して開き直し、再試行してください"
        }
        ...
        L10nKey::RemoteHostNotOsiris => {
            "{machine} は応答しましたが、OSIRIS.EXE サーバーとしては応答しませんでした: {error}"
        }
        L10nKey::RemoteWorkspaceListFailed => {
            "{machine} に接続しましたが、ワークスペースの一覧を取得できませんでした: {error}"
        }
        L10nKey::RemoteServerRestartFailed => {
            "{machine} 上の OSIRIS.EXE サーバーを再起動できませんでした: {error}"
        }
        L10nKey::RemoteNoRouteToHost => "OSIRIS.EXE は {machine} に到達する手段を失いました",
        ...
        L10nKey::SwitcherRestartingServer => "OSIRIS.EXE のサーバーを再起動中…",
        L10nKey::SwitcherDownloadingServerWithTotal => {
            "OSIRIS.EXE のサーバーをダウンロード中… {done} / {total}"
        }
        L10nKey::SwitcherDownloadingServerNoTotal => "OSIRIS.EXE のサーバーをダウンロード中… {done}",
        L10nKey::SwitcherCopyingServer => "OSIRIS.EXE のサーバーをコピー中… {done} / {total}",
        ...
        L10nKey::AppRestartServerNoServer => {
            "{label} には再起動できる OSIRIS.EXE 自身のサーバーがありません。これはこのコンピュータが --stdio で実行しているプログラムです。代わりにそのワークスペースを止めてください"
        }
        ...
        L10nKey::AppForkNoCommand => "OSIRIS.EXE には {name} 用のフォークコマンドがありません",
        L10nKey::AppForkLocalOnly => {
            "{name} のセッションはローカルペインからしかフォークできません"
        }
        L10nKey::AppForkNoSessionId => {
            "OSIRIS.EXE はこのペインで {name} のセッション ID を確認できていません — 設定 → エージェントでフックをインストールしてください"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "OSIRIS.EXE はこのコンピュータのホームディレクトリを特定できなかったため、インストール先がありません"
        }
        ...
        L10nKey::AppAgentHooksNoOsirisHooks => {
            "OSIRIS.EXE のフックが見つからないため、削除するものはありません"
        }
        ...
        L10nKey::SettingsShellDetected => "OSIRIS.EXE が見つけたシェル",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "OSIRIS.EXE はその場で更新されたため、アプリは新しくても、ペインは前のビルドが提供したままです。\
             サーバーは停止せずに新しいビルドへ自分自身を置き換えられます。\
             シェルとその中で動いているものはそのまま引き継がれます。\
             OSIRIS.EXE 内蔵の SSH クライアントを使うペインだけは例外で、その接続は閉じられ、開き直しが必要です"
        }
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "上キーでたどるのは、すべてのペインが混ざったものではなく、このペインで実行したコマンドです。\
             ...
             OSIRIS.EXE が設定できる bash と zsh のペインが対象で、独自の引数で起動したシェルはそのままです"
        }
        L10nKey::IntegrationNoticeBlocked => {
            "このペインでは OSIRIS.EXE シェル統合がブロックされています。“{wrapper}”がシェルレポートを\
             横取りしているため、インライン補完と Ctrl+R メニューは利用できません。\
             シェル独自の履歴検索は引き続き使えます。"
        }
        L10nKey::IntegrationNoticeNotEngaged => {
            "このペインでは OSIRIS.EXE シェル統合が有効になっていないため、インライン補完と Ctrl+R \
        ...
        L10nKey::PaletteTryDifferentSearch => "TYPE \"help\" — 別のキーワードも試せます。",
...
pub fn translate_variant_ja(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => {
            "ファイル内のすべてのオプションに OSIRIS.EXE 側の設定があります"
        }
        (L10nKey::SettingsImportIgnored, "one") => {
            "OSIRIS.EXE に設定のないオプションが 1 件あり、ファイルに残されています: {options}"
        }
