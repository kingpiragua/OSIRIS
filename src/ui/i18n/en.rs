...
pub fn translate_en(key: L10nKey) -> &'static str {
    match key {
        ...
        L10nKey::NoMatchingCommands => "UNKNOWN COMMAND",
        ...
        L10nKey::QuitStopServerBody => {
            "This quits OSIRIS.EXE and stops the background server — anything still running \
        ...
        L10nKey::SettingsLanguageDesc => "Choose the language used for the OSIRIS.EXE interface.",
        ...
        L10nKey::SettingsCustomThemesIntro => {
            "Duplicate a theme to edit its colors here, or drop your own in the themes folder: an OSIRIS.EXE YAML theme or an iTerm2 .itermcolors scheme."
        }
        ...
        L10nKey::SettingsInOsiris => "In OSIRIS.EXE",
        ...
        L10nKey::SettingsTypeAddressToConnect => {
            "Type an address to connect now — OSIRIS.EXE offers to save it afterwards."
        }
        ...
        L10nKey::SettingsImportAliasesDesc => {
            "Re-reads the file and adds anything new. Edits you make here are stored by OSIRIS.EXE — the file itself is never written."
        }
        ...
        L10nKey::SettingsImportIgnored => {
            "{count} options have no setting in OSIRIS.EXE and were left in the file: {options}"
        }
        ...
        L10nKey::SettingsStartInDesc => {
            "What a fresh shell starts in: OSIRIS.EXE's launch directory, your home folder, or a fixed path."
        }
        ...
        L10nKey::SettingsTabCompletionDesc => {
            "Tab at the prompt opens OSIRIS.EXE's completion menu. When off, Tab goes to the shell's own completion instead."
        }
        L10nKey::SettingsHistorySearch => "History search",
        L10nKey::SettingsHistorySearchDesc => {
            "⌃R at the prompt opens OSIRIS.EXE's fuzzy history menu. When off, ⌃R goes to the shell instead — its own reverse-i-search, or whatever you've bound there (fzf, percol)."
        }
        ...
        L10nKey::SettingsAgentsIntroDesc => {
            "Hook integrations give panes running these agents live session status (working / waiting / done) in the tab bar. Only active inside OSIRIS.EXE."
        }
        ...
        L10nKey::SettingsStartupWindowDesc => "Window state when OSIRIS.EXE launches.",
        L10nKey::SettingsRememberWindowSize => "Remember window size & position",
        L10nKey::SettingsRememberWindowSizeDesc => {
            "Reopen at the size and position the window had when OSIRIS.EXE last quit. Off opens centered at the default size."
        }
        ...
        L10nKey::SettingsUpdateReadyNextLaunch => {
            "It will be installed the next time you start OSIRIS.EXE."
        }
        ...
        L10nKey::SettingsDaemonStaleDesc => {
            "OSIRIS.EXE was updated in place, so the app is new but your panes are still served by the previous build. Restarting the server picks up the new one and ends every process running in your panes — shells, agents, and SSH sessions alike. There is no hurry: pick a moment when your panes are idle."
        }
        L10nKey::UpdateDialogTitle => "Update available",
        L10nKey::UpdateDialogDetail => {
            "OSIRIS.EXE {version} is available — you're on {current}. Installing restarts the app; the background server keeps running, so whatever is open in your panes survives."
        }
        L10nKey::UpdateDialogDetailWindows => {
            "OSIRIS.EXE {version} is available — you're on {current}. Installing restarts the app and the background service: processes running in your panes are ended, and your tabs and layout come back with fresh shells."
        }
        L10nKey::UpdateDialogDetailManual => {
            "OSIRIS.EXE {version} is available — you're on {current}. {hint}"
        }
        ...
        L10nKey::UpdateDialogNeedsElevation => {
            "This copy of OSIRIS.EXE is installed for all users, so Windows will ask for administrator approval once before the install begins. OSIRIS.EXE itself never runs elevated — it comes back as you."
        }
        ...
        L10nKey::SettingsUpdateUnsupportedMacos => {
            "This copy is not running from a writable osiris.app bundle, so replacing it would be unsafe. Move OSIRIS.EXE to Applications or another writable folder, or open the release page to install the update."
        }
        ...
        L10nKey::SettingsUpdateWindowsAllUsers => {
            "OSIRIS.EXE is installed for all users, which needs administrator rights to replace. OSIRIS.EXE will not raise an elevation prompt on its own behalf, so open the release page and run the installer yourself to update it."
        }
        ...
        L10nKey::SettingsUpdateMissingChecksums => {
            "The release has no checksums.txt, so OSIRIS.EXE refuses to install it automatically."
        }
        ...
        L10nKey::SettingsCommandLineDesc => {
            "Put the bundled OSIRIS.EXE command on your PATH at launch, so scripts and coding agents can drive OSIRIS.EXE from any terminal. Inside an OSIRIS pane it works either way. Turn this off if you keep your own OSIRIS.EXE — one you built or installed yourself — and do not want it shadowed. Takes effect at next launch."
        }
        L10nKey::SettingsInstallCliOnPath => "Install the OSIRIS.EXE command on PATH",
        ...
        L10nKey::SettingsHowShellsWorkBody => {
            "Your shells run in a background server, not inside this window. Quitting OSIRIS.EXE \
        ...
        L10nKey::SettingsAppHttpProxyDesc => {
            "Used only for OSIRIS.EXE's update checks and downloads, not for programs in your panes. Empty follows the system proxy."
        }
        ...
        L10nKey::SettingsSearchCommandLineToolKeywords => {
            "command line tool cli OSIRIS.EXE path shell command install symlink terminal iterm agent script"
        }
        ...
        L10nKey::DiffTruncatedBudget => {
            "Body not loaded — this working tree is past OSIRIS.EXE's diff budget. Run git diff in the terminal for this file."
        }
        ...
        L10nKey::DiffBudgetAndCap => "OSIRIS.EXE's budget and the per-file cap",
        L10nKey::DiffBudget => "OSIRIS.EXE's budget",
        ...
        L10nKey::HomeIntegrity => "MEMORY INTEGRITY {pct}%",
        L10nKey::HomeIntegrityDetail => "{live} of {total} workspaces live · {panes} panes",
        L10nKey::HomeIntegrityCeiling => "ceiling — the last 1/64 is not yours to restore alone",
        ...
        L10nKey::RemoteNoConnectionDetails => {
            "This window is a workspace on {machine}, but OSIRIS.EXE has no connection \
        ...
        L10nKey::RemoteRestartTitle => "Restart OSIRIS.EXE's server on \"{machine}\"?",
        ...
        L10nKey::RemoteReplaceBody => {
            "OSIRIS.EXE will install a matching server on {machine} and start it.\n\
             \n\
             Every session running on {machine} ends, including any this window is not \
             connected to."
        }
        L10nKey::RemoteRestartFailedTitle => "OSIRIS.EXE's server on \"{machine}\" was not restarted",
        ...
        L10nKey::RemoteInstallTitle => "Install OSIRIS.EXE's server on \"{machine}\"?",
        L10nKey::RemoteInstallDetail => {
            "OSIRIS.EXE will write its server binary to {machine} so this machine can host \
        ...
        L10nKey::RemoteMismatchTitle => "Update OSIRIS.EXE's server on \"{machine}\"?",
        L10nKey::RemoteMismatchDetail => {
            "{machine} is serving OSIRIS sessions from {running}, which speaks a protocol \
             this client ({wanted}) cannot. OSIRIS.EXE has installed a matching server there, \
        ...
        L10nKey::RemoteServerOutdated => {
            "{machine} is running an old OSIRIS server ({build}) that this copy of OSIRIS.EXE \
             cannot talk to. Update it to connect."
        }
        L10nKey::RemoteServerTooNew => {
            "{machine} is running a newer OSIRIS server ({build}) than this copy of OSIRIS.EXE. \
             Update OSIRIS.EXE on this computer, or replace the server there with a matching one."
        }
        L10nKey::RemoteDaemonStartFailed => "OSIRIS.EXE's local server could not be started: {error}",
        L10nKey::RemoteDaemonUnreachable => "could not reach OSIRIS.EXE's local server: {error}",
        L10nKey::RemoteDaemonTooOld => {
            "this machine's OSIRIS daemon is an older build and cannot restart the server on \
             {machine}. Quit OSIRIS.EXE (which stops the daemon) and open it again, then retry."
        }
        ...
        L10nKey::RemoteHostNotOsiris => "{machine} answered, but not as an OSIRIS server: {error}",
        ...
        L10nKey::RemoteServerRestartFailed => {
            "could not restart OSIRIS.EXE's server on {machine}: {error}"
        }
        L10nKey::RemoteNoRouteToHost => "OSIRIS.EXE no longer has a way to reach {machine}",
        ...
        L10nKey::SwitcherRestartingServer => "Restarting OSIRIS.EXE's server…",
        L10nKey::SwitcherDownloadingServerWithTotal => {
            "Downloading OSIRIS.EXE's server… {done} / {total}"
        }
        L10nKey::SwitcherDownloadingServerNoTotal => "Downloading OSIRIS.EXE's server… {done}",
        L10nKey::SwitcherCopyingServer => "Copying OSIRIS.EXE's server… {done} / {total}",
        ...
        L10nKey::CmdQuitOsiris => "Quit OSIRIS.EXE",
        L10nKey::CmdQuitOsirisSubtitle => "shells keep running",
        ...
        L10nKey::AppRestartServerNoServer => {
            "OSIRIS.EXE has no server of its own to restart on {label} — it is a program this computer runs over --stdio. Stop its workspace instead."
        }
        ...
        L10nKey::ConfigQuarantinedStartup => {
            "config.json could not be parsed, so OSIRIS.EXE is running on default settings and will not write over the file. Its contents were kept beside it as config.json.corrupt — fix the file and it reloads itself. Until then, changes made in Settings are not saved."
        }
        L10nKey::ConfigQuarantinedReload => {
            "The edited config.json could not be parsed, so OSIRIS.EXE kept the settings it is already running on and set the file's contents aside as config.json.corrupt. Fix the file and it reloads itself; saving a setting before then replaces it with the settings in use."
        }
        L10nKey::ConfigUnreadableStartup => {
            "config.json could not be read, so OSIRIS.EXE is running on default settings and will not write over the file — it is left exactly as it is. Fix its permissions or contents and it reloads itself. Until then, changes made in Settings are not saved."
        }
        L10nKey::ConfigUnreadableReload => {
            "config.json could not be read, so OSIRIS.EXE kept the settings it is already running on and left the file exactly as it is. Fix its permissions or contents and it reloads itself; saving a setting before then replaces it with the settings in use."
        }
        ...
        L10nKey::AppForkNoCommand => "OSIRIS.EXE has no fork command for {name}",
        L10nKey::AppForkLocalOnly => "{name} sessions can only be forked from a local pane",
        L10nKey::AppForkNoSessionId => {
            "OSIRIS.EXE hasn't seen a {name} session id in this pane — install its hooks in Settings → Agents"
        }
        ...
        L10nKey::AppAgentHooksNoHomeDir => {
            "OSIRIS.EXE could not work out this computer's home directory, so there is nowhere to install to."
        }
        ...
        L10nKey::AppAgentHooksNoOsirisHooks => "No OSIRIS.EXE hooks found; nothing to remove",
        ...
        L10nKey::SettingsShellDetected => "Shells OSIRIS.EXE found",
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "OSIRIS.EXE was updated in place, so the app is new but your panes are still served by the \
             previous build. The server can replace itself with the new one without stopping: \
             your shells and whatever is running in them carry straight over. Panes on OSIRIS.EXE's \
        ...
        L10nKey::AppRestartServerBodyInPlace => {
            "The background server replaces itself with this build without stopping. Your shells \
             ...
             and the window reconnects to them a moment later. Panes on OSIRIS.EXE's built-in SSH \
        ...
        L10nKey::SettingsPerPaneHistoryDescription => {
            "Up walks through what you ran in this pane, instead of an interleaving of every \
             ...
             panes that OSIRIS.EXE can set up; a shell started with your own arguments is left alone."
        }
        L10nKey::IntegrationNoticeBlocked => {
            "OSIRIS shell integration is blocked in this pane — \u{201c}{wrapper}\u{201d} is \
        ...
        L10nKey::IntegrationNoticeNotEngaged => {
            "OSIRIS shell integration hasn't engaged in this pane, so inline completion and the \
...
pub fn translate_variant_en(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsImportIgnored, "zero") => "Every option in the file has an OSIRIS.EXE setting.",
        (L10nKey::SettingsImportIgnored, "one") => {
            "1 option has no setting in OSIRIS.EXE and was left in the file: {options}"
        }
