...
use osiris_core::core::git::log::{
    Commit, CommitPage, Edge, GRAPH_PAGE, GraphRow, GraphScope, Lane, MAX_GRAPH_COMMITS, RefDeco,
    RefKind,
};
use osiris_core::core::git::ops::{GitOp, ResetMode};

use crate::ui::app::{CONTENT_INSET, OsirisApp};
...
impl OsirisApp {
    ...
    fn scm_load_graph(&mut self, repo: &RepoKey, cx: &mut Context<Self>) {
        ...
        crate::ui::host_ops::HostOps::run(
            host,
            cx,
            move |h| osiris_core::core::git::log::load_page(h, &root, &query, want),
            move |this, page, cx| {
        ...
        h_flex()
            ...
            .context_menu({
                ...
                move |menu, _window, cx| {
                    ...
                    OsirisApp::graph_row_context_menu(menu, &repo, &oid, danger, &app)
                }
            })
            .into_any_element()
    }
...
impl OsirisApp {
    ...
    ) -> impl Fn(PopupMenu, &mut Window, &mut Context<PopupMenu>) -> PopupMenu + 'static + use<>
    {
        ...
        move |menu, _window, _cx| {
            ...
            let pick = |app: &gpui::WeakEntity<OsirisApp>, next: GraphScope| {
                let app = app.clone();
...
impl OsirisApp {
    ...
    fn graph_row_context_menu(
        ...
        let op = |app: &gpui::WeakEntity<OsirisApp>, repo: &RepoKey, build: fn(String) -> GitOp| {
            let app = app.clone();
            let repo = repo.clone();
...
mod tests {
    ...
    fn repo() -> RepoKey {
        RepoKey {
            host: HostId::LOCAL,
            root: PathBuf::from("/tmp/osiris-graph-test"),
        }
    }
    ...
    fn everything_past_the_cap_lands_in_the_overflow_column() {
        let cap = 4;
        for lane in 4u16..=osiris_core::core::git::log::MAX_LANES {
    ...
    fn switching_repositories_drops_the_previous_page(cx: &mut TestAppContext) {
        ...
        let other = RepoKey {
            host: HostId::LOCAL,
            root: PathBuf::from("/tmp/osiris-graph-test-other"),
        };
        app.update(&mut vcx, |app, cx| {
            app.scm.graph.requested = GRAPH_PAGE * 3;
            app.scm.graph.page = Some(Arc::new(empty_page()));
    ...
    fn commit_named(summary: &str, author: &str, oid: &str) -> Commit {
        use osiris_core::core::git::log::{OffsetTs, Signature};
        let who = Signature {
...
mod render_idle_gpui_tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-graph-{name}-{}", std::process::id()));
    ...
    /// from `render`.
    fn settle_graph(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext) -> Option<Arc<CommitPage>> {
