...
use osiris_core::core::git::diff::MAX_RENDERED_FILES;
use osiris_core::core::git::ops::GitOp;
use osiris_core::core::git::status::{
...
use crate::ui::app::{CONTENT_INSET, TILE_GLYPH_XS, TILE_SIZE_XS, OsirisApp};
...
/// osiris is `.appearance(false)`; a hairline here, and an accent ring on top of
...
impl OsirisApp {
    ...
    /// 0.20)` in osiris, so a "primary" button is a mid-grey block sitting in a
...
mod tests {
    ...
    use osiris_core::core::git::status::{ConflictKind, EntryKind, RepoPath};
...
mod render_idle_gpui_tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-scm-{name}-{}", std::process::id()));
    ...
    fn scm_panel_on(
        ...
        root: &Path,
        until: impl Fn(&OsirisApp, &gpui::App) -> bool,
    ) -> (
        Entity<OsirisApp>,
        VisualTestContext,
