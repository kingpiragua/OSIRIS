...
use osiris_core::core::git::diff::CommitLabel;
use osiris_core::core::git::log::{Commit, CommitFile, RefKind};
use osiris_core::core::git::status::DecoStatus;
...
impl OsirisApp {
    ...
    ///
    /// `theme.accent` is a neutral surface tint in osiris rather than the brand
...
mod tests {
    use super::*;
    use osiris_core::core::git::diff::FileStatus;
...
mod detail_gpui_tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-detail-{name}-{}", std::process::id()));
    ...
    fn settle(
        ...
        done: impl Fn(&OsirisApp, &gpui::App) -> bool,
    ) {
    ...
    fn paths(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext) -> Vec<String> {
    ...
    fn a_seeded_detail_only_asks_for_the_files(cx: &mut TestAppContext) {
        ...
        let mut seed = osiris_core::core::git::log::load_commit(
            &*osiris_core::host::local::LocalHost::new(),
            &root,
            &head,
        )
    ...
    fn a_detail_from_another_repository_gives_the_body_back(cx: &mut TestAppContext) {
        ...
        app.update_in(&mut vcx, |app, window, cx| {
            let stranger = RepoKey {
                host: HostId::LOCAL,
                root: PathBuf::from("/no/such/osiris/repo"),
            };
