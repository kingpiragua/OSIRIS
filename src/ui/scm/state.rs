...
//!
//! All of it is app-level, hanging off `OsirisApp.scm` rather than off a tab.
...
use osiris_core::core::git::log::{Commit, CommitFile, CommitPage, GraphScope};
use osiris_core::core::git::status::HeadState;
...
pub(crate) struct ScmPanelState {
    ...
    /// built by `Default` alongside the rest of `OsirisApp`.
