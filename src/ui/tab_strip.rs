...
use crate::ui::app::{SpawnWhere, TILE_GLYPH, TILE_SIZE, Tab, OsirisApp, tile_trailing_inset};
...
/// metadata can distinguish osiris launch defaults from user-authored arguments.
fn shell_spec(shell: &DetectedShell) -> ShellSpec {
    ShellSpec {
        program: shell.program.clone(),
        args: shell.args.clone(),
        args_are_osiris_defaults: shell.args_are_osiris_defaults,
    }
}
...
/// same titles.
pub(crate) use osiris_core::core::tab_view::strip_host_prefix;
...
pub(crate) fn workspace_avatar(
    ...
    div()
        ...
        .children(dot.map(|rgb| OsirisApp::status_dot(rgb, 0, size, cx.theme().popover, false)))
}
...
mod tests {
    ...
    fn short_title_keeps_an_ssh_address_whole() {
        ...
        // is the title osiris's own pwsh integration writes on Windows.
        assert_eq!(short_title(r"ann@BOX:C:/Users/app"), r"C:/Users/app");
    }

    #[test]
    fn short_title_truncates_deep_paths_to_trailing_segments() {
        assert_eq!(short_title("user@host:~/repo/025/osiris"), "…/repo/025/osiris");
    ...
    fn elide_path_fits_shallow_paths_untouched(cx: &mut TestAppContext) {
        ...
        assert_eq!(elide_path_keep_tail(&ts, &font, size, path, max), "~/osiris");
    }
    ...
    fn elide_path_shows_the_whole_deep_path_when_the_budget_allows(cx: &mut TestAppContext) {
        ...
        let path = "E:/work/toolbox/crates/osiris-core/src/client";
        let max = measure_text(&ts, &font, size, path) + 1.;
    ...
    fn configured_shell_arguments_remain_user_authored_in_the_menu() {
        ...
        assert!(!spec.args_are_osiris_defaults);
    }
}
