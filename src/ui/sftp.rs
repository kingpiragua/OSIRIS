...
use crate::ui::app::{CONTENT_INSET, TILE_GLYPH_SM, TILE_SIZE_SM, OsirisApp};
...
pub(crate) struct SftpPanelState {
    ...
    ///
    /// An upload is written to `<name>.osiris-upload-<hex>` and renamed into
...
impl OsirisApp {
    ...
    /// `<name>.osiris-upload-<hex>` and is renamed into place at the end, so the
...
mod gpui_tests {
    ...
    fn panel(app: &Entity<OsirisApp>, vcx: &mut VisualTestContext) -> (bool, RightPanelTab) {
        vcx.update(|_, cx| {
            let app = app.read(cx);
