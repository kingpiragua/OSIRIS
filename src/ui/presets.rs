...
impl Theme {
    ...
    /// Clears a seed colour for use as ink on any of osiris's neutral fills.
    ///
...
/// OSIRIS.EXE ships on its own theme, not on a stock light one. The archive
/// renders in phosphor on duat black; everything else in the list is a guest.
pub const DEFAULT_ID: &str = "osiris";
...
// ---- OSIRIS.EXE canon palette (canon-lock v4.1) ---------------------------
//
// Every color below is a token from the locked v3.0 system. Nothing here is a
// new hex, and nothing blends the two poles: phosphor green #00FF46 is the
// Signal, crimson #FF3A1A is the Network, and they meet only at hard contact
// edges — a gradient between them would be world-law deception, i.e. a visual
// bug. Banned on sight: #7dffb0.
//
//   --duat-black      #010103   --lake-midnight   #0B1226
//   --deep-indigo     #1B2A5E   --phosphor        #00FF46
//   --phosphor-dim    #0A8A2E   --crimson         #FF3A1A
//   --crimson-dim     #7A1A0C   --agi-gold        #FFC93C
//   --resurrection    #C9A227   --quantum-violet  #6B3FA0
//   --bone            #D8D2C4   --cream           #D5CDB8
//
// The ANSI slots are assigned so a program's own color choices stay inside the
// grammar: anything a shell prints in "red" reads as the Network, anything in
// "green" reads as the Signal, and there is no cyan pole to invent a third
// one — slot 6 is phosphor-dim.
const DUAT_BLACK: u32 = 0x010103;
const LAKE_MIDNIGHT: u32 = 0x0b1226;
const DEEP_INDIGO: u32 = 0x1b2a5e;
const PHOSPHOR: u32 = 0x00ff46;
const PHOSPHOR_DIM: u32 = 0x0a8a2e;
const CRIMSON: u32 = 0xff3a1a;
const CRIMSON_DIM: u32 = 0x7a1a0c;
const AGI_GOLD: u32 = 0xffc93c;
const RESURRECTION: u32 = 0xc9a227;
const QUANTUM_VIOLET: u32 = 0x6b3fa0;
const BONE: u32 = 0xd8d2c4;
const CREAM: u32 = 0xd5cdb8;

/// A locked token, split into the `(r, g, b)` triple the ANSI table wants.
/// Written as a `const fn` so every slot below is spelled with the token's
/// name rather than a loose hex — the one rule that kept three color systems
/// from growing back.
const fn token(c: u32) -> (u8, u8, u8) {
    (((c >> 16) & 0xff) as u8, ((c >> 8) & 0xff) as u8, (c & 0xff) as u8)
}

/// The 16 ANSI slots, shared by both OSIRIS registers so a pane's output means
/// the same thing whichever side of the world you are reading it from.
const OSIRIS_ANSI16: [(u8, u8, u8); 16] = [
    token(LAKE_MIDNIGHT),  // 0  black
    token(CRIMSON),        // 1  red            the Network
    token(PHOSPHOR),       // 2  green          the Signal
    token(RESURRECTION),   // 3  yellow
    token(DEEP_INDIGO),    // 4  blue
    token(QUANTUM_VIOLET), // 5  magenta
    token(PHOSPHOR_DIM),   // 6  cyan           no third pole
    token(BONE),           // 7  white
    token(CRIMSON_DIM),    // 8  bright black
    token(CRIMSON),        // 9  bright red
    token(PHOSPHOR),       // 10 bright green
    token(AGI_GOLD),       // 11 bright yellow
    token(QUANTUM_VIOLET), // 12 bright blue
    token(QUANTUM_VIOLET), // 13 bright magenta
    token(PHOSPHOR),       // 14 bright cyan
...
static BUILTINS: [BuiltinSpec; 14] = [
    ...
    // EYE OF HORUS.EXE — the Network's crimson apex interface. Same grammar,
    // opposite pole: chrome goes crimson, and the Signal stays green wherever a
    // program prints it, so the two never merge into one another.
    BuiltinSpec {
        id: "eye_of_horus",
...
mod tests {
    ...
    fn carets_stay_visible_on_the_background() {
        ...
        // The theme that used to fail: an orange caret on pure white read at
        // 2.07:1. Named outright rather than reached through `DEFAULT_ID` —
        // the default is OSIRIS now, whose phosphor caret needs no rescue
        // (15.3:1 on duat black), so it cannot stand in for the broken case.
        let light = builtins().into_iter().find(|t| t.id == "light").unwrap();
        assert_ne!(light.neutrals().caret, light.caret.unwrap());
        // And the default itself is authored well enough to pass through
        // untouched, which is the other half of the contract.
        let osiris = builtins().into_iter().find(|t| t.id == DEFAULT_ID).unwrap();
        assert_eq!(osiris.neutrals().caret, osiris.caret.unwrap());
    }
