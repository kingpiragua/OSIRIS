...
/// The smallest window osiris still holds its shape in.
///
...
impl WindowRegistry {
    ...
    pub fn open_windows(cx: &mut App) -> Vec<(WorkspaceId, WeakEntity<OsirisApp>)> {
        Self::sweep(cx);
        cx.global::<Self>()
            .windows
            .iter()
    ...
    pub fn app_in(cx: &mut App, window: &Window) -> Option<gpui::Entity<OsirisApp>> {
        Self::sweep(cx);
    ...
    pub fn app_for(cx: &mut App, workspace: WorkspaceId) -> Option<WeakEntity<OsirisApp>> {
        Self::sweep(cx);
        cx.global::<Self>()
            .windows
            .iter()
...
pub fn open_at_tab(cx: &mut App, workspace: WorkspaceId, tab: osiris_core::core::machine::TabId) {
...
pub fn open_at(
    ...
    let mut created: Option<gpui::Entity<OsirisApp>> = None;
    let opened = cx.open_window(options, |window, cx| {
        let app = cx.new(|cx| match initial_cwd.clone() {
            Some(cwd) => OsirisApp::for_workspace_at(workspace, Some(cwd), window, cx),
            None => OsirisApp::for_workspace(workspace, window, cx),
        });
    ...
    let Some(app) = created else {
        log::error!("opened a window but its OsirisApp was never built; not registering");
        return;
    };
...
/// its id, and no other window would do. Everything else is `osiris [PATH]`,
...
/// to disagree: "Open in osiris" from Explorer, or `osiris <PATH>`, skipped the
...
mod tests {
    ...
    fn a_request_carrying_a_path_restores_the_layout_and_brings_the_path_along(
        ...
        // The Explorer "Open in osiris" case with no window up: the folder is
    ...
    fn a_delete_reads_its_kill_list_before_the_removal_blanks_the_mirror(
        ...
        use osiris_core::core::machine::{Machine, PaneRecord, Tab, Workspace as TreeWorkspace};
    ...
    fn forgetting_a_workspace_never_tells_its_machine(cx: &mut gpui::TestAppContext) {
        ...
        use osiris_core::core::machine::{Machine, Workspace as TreeWorkspace};

        // `WorkspaceStore::remove` saves; a test has no business writing the
        // real views.
        let _ = osiris_core::core::config::set_config_dir(
            std::env::temp_dir().join(format!("osiris-windows-test-{}", std::process::id())),
        );
        cx.update(|cx| {
