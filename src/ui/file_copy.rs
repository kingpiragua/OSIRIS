...
fn free_name_beside(
    ...
    for n in 0..WORKING_NAME_TRIES {
        let candidate = match n {
            0 => format!(".osiris-{tag}-{name}"),
            n => format!(".osiris-{tag}-{n}-{name}"),
        };
...
mod tests {
    ...
    fn scratch(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("osiris-drop-{name}-{}", std::process::id()));
    ...
    fn leavings(dir: &Path) -> Vec<String> {
        let mut names: Vec<String> = std::fs::read_dir(dir)
            ...
            .filter(|name| name.starts_with(".osiris-"))
            .collect();
        names.sort();
        names
    }
