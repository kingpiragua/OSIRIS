...
mod macos {
    ...
    fn usage() -> String {
        "usage: osiris-updater verify <current.app> <archive.zip> <checksums.txt> \
         <asset-name> <stage-dir> <version>\n\
         or: osiris-updater install <parent-pid> <current.app> <archive.zip> <checksums.txt> \
    ...
    impl TailOptions {
        fn apply(&self) {
            ...
            osiris_core::core::config::set_config_dir(dir.clone());
            ...
            unsafe { std::env::set_var("OSIRIS_CONFIG_DIR", dir) };
        }
    }
    ...
    fn report_outcome(
        ...
        let outcome = osiris_core::daemon::install::outcome::UpdateOutcome {
            version: version.to_string(),
            ok: result.is_ok(),
            detail: result.as_ref().err().cloned(),
        };
        if let Err(error) = osiris_core::daemon::install::outcome::write_outcome(path, &outcome) {
            log_line(
                log,
                &format!(
    ...
    fn install_inner(plan: &InstallPlan) -> Result<(), String> {
        let replacement = plan.stage.join("unpacked/osiris.app");
        wait_for_exit(plan.parent_pid);
        log_line(&plan.log, "re-verifying staged osiris update");
    ...
    fn verify_archive(archive: &Path, checksums: &Path, asset_name: &str) -> Result<(), String> {
        ...
        osiris_core::daemon::install::checksums::verify(&manifest, asset_name, &bytes)
    ...
    fn verify_update(
        ...
        let executable = replacement.join("Contents/MacOS/osiris");
        let updater = replacement.join("Contents/MacOS/osiris-updater");
        if !replacement.is_dir() || !executable.is_file() || !updater.is_file() {
            return Err(
                "the staged bundle is missing osiris or osiris-updater under Contents/MacOS"
                    .to_string(),
            );
        }
    ...
    fn launch_app(app: &Path) -> Result<(), String> {
        let executable = app.join("Contents/MacOS/osiris");
    ...
    mod tests {
        ...
        fn successful_launch_commits_the_replacement() {
            ...
            let current = root.path().join("osiris.app");
            let stage = root.path().join("stage");
            let replacement = stage.join("osiris.app");
            bundle(&current, "old");
            ...
            assert!(!stage.exists());
            assert!(!root.path().join(".osiris.app.osiris-update-backup").exists());
        }
        ...
        fn replacement_does_not_remove_a_fixed_name_sibling() {
            ...
            let stage = root.path().join("stage");
            let replacement = stage.join("osiris.app");
            let sibling = root.path().join(".osiris.app.osiris-update-backup");
            bundle(&current, "old");
            bundle(&replacement, "new");
            bundle(&sibling, "keep");
        ...
        fn verify_rejects_a_bundle_without_the_helper() {
            ...
            fs::write(replacement.join("Contents/MacOS/osiris"), b"app").unwrap();

            let error = verify_update(&current, &replacement, "1.0.0").unwrap_err();
            assert!(
                error.contains("missing osiris or osiris-updater"),
                "{error}"
            );
        }
        ...
        fn archive_verification_rejects_bytes_that_do_not_match_the_manifest() {
            let root = tempfile::tempdir().unwrap();
            let archive = root.path().join("osiris.zip");
            ...
            fs::write(
                ...
                format!(
                    "{}  osiris.zip\n",
                    osiris_core::daemon::install::checksums::hex(
                        &osiris_core::daemon::install::checksums::sha256(b"published bytes")
                    )
                ),
            )
            .unwrap();

            let error = verify_archive(&archive, &manifest, "osiris.zip").unwrap_err();
        ...
        fn bundle_version_preserves_the_complete_nightly_identity() {
            let root = tempfile::tempdir().unwrap();
            let app = root.path().join("osiris.app");
...
mod linux {
    ...
    /// image. The root-level `osiris.desktop` is linuxdeploy's symlink to this
    /// file, and extracting a symlink alone yields a dangling link — so the
    /// real path is the one asked for.
    const DESKTOP_ENTRY: &str = "usr/share/applications/osiris.desktop";
    /// The helper inside the image, beside the app the way every platform
    /// ships it.
    const BUNDLED_UPDATER: &str = "usr/bin/osiris-updater";
    ...
    fn usage() -> String {
        "usage: osiris-updater verify <archive.AppImage> <checksums.txt> \
         <asset-name> <stage-dir> <version>\n\
         or: osiris-updater install <parent-pid> <current.AppImage> <archive.AppImage> \
    ...
    fn install_inner(plan: &InstallPlan) -> Result<(), String> {
        wait_for_exit(plan.parent_pid);
        log_line(&plan.log, "re-verifying the staged osiris update");
        let verification = verify_archive(&plan.archive, &plan.checksums, &plan.asset_name)
    ...
    fn extract_entry(appimage: &Path, stage: &Path, entry: &str) -> Result<PathBuf, String> {
        ...
        // a hand-run `osiris-updater verify ./pkg.AppImage …` would otherwise
    ...
    fn replace_and_relaunch(
        ...
        // user's choice of who may run their osiris is not this program's to
    ...
    mod tests {
        ...
        fn successful_launch_commits_the_replacement() {
            let root = tempfile::tempdir().unwrap();
            let current = root.path().join("osiris.AppImage");
            let stage = root.path().join("stage");
            fs::create_dir(&stage).unwrap();
            let replacement = stage.join("osiris-new.AppImage");
        ...
        fn archive_verification_rejects_bytes_that_do_not_match_the_manifest() {
            let root = tempfile::tempdir().unwrap();
            let archive = root.path().join("osiris.AppImage");
            let manifest = root.path().join("checksums.txt");
            fs::write(&archive, b"downloaded bytes").unwrap();
            fs::write(
                &manifest,
            ...
            let error = verify_archive(&archive, &manifest, "osiris.AppImage").unwrap_err();
            assert!(error.contains("failed sha256 verification"), "{error}");
        }
        ...
        fn verification_rejects_a_file_that_is_not_an_appimage() {
            ...
            let root = tempfile::tempdir().unwrap();
            let staged = root.path().join("osiris.AppImage");
...
mod windows {
    ...
    use osiris_core::daemon::install::outcome::UpdateOutcome;
    ...
    const PORTABLE_MARKER: &str = ".osiris-portable";
    ...
    const PORTABLE_BACKUP_INCOMPLETE: &str = ".osiris-replace-incomplete";
    ...
    const PORTABLE_MANAGED_ROOTS: [&str; 11] = [
        ...
        // `verify_portable_payload`: osiris runs without them, falling back to
    ...
    fn usage() -> String {
        "usage: osiris-updater verify <setup.exe> <checksums.txt> <asset-name> <version>\n\
         or: osiris-updater install <parent-pid> <setup.exe> <checksums.txt> <asset-name> \
         <install-dir> <version> <log-path> <stage-dir> \
         [--config-dir <dir>] [--result-file <path>]\n\
         or: osiris-updater verify-portable <archive.zip> <checksums.txt> <asset-name> \
         <version> <stage-dir>\n\
         or: osiris-updater install-portable <parent-pid> <archive.zip> <checksums.txt> \
         <asset-name> <install-dir> <version> <log-path> <stage-dir> \
         [--config-dir <dir>] [--result-file <path>]\n\
         or: osiris-updater cleanup <parent-pid> <stage-dir>\n\
         or: osiris-updater capabilities\n\
         or: osiris-updater elevated-stage <gui-pid> <setup.exe> <asset-name> \
         <install-dir> <version> <log-path> <stage-dir> \
         --expected-sha256 <hex> --status-file <path> \
         [--config-dir <dir>] [--result-file <path>]\n\
         or: osiris-updater install-elevated <parent-pid> <setup.exe> <checksums.txt> \
         <asset-name> <install-dir> <version> <log-path> <stage-dir> \
         --expected-sha256 <hex> [--config-dir <dir>] [--result-file <path>]\n\
         or: osiris-updater relaunch-watcher --status-file <path> --result-file <path> \
         --app-path <osiris.exe> --log <path> --expected-version <version> \
    ...
    /// inherit the spawning GUI's environment, so a `OSIRIS_CONFIG_DIR` set
    ...
    struct TailOptions {
        ...
        /// The `osiris.exe` the watcher relaunches.
    ...
    /// process must never spawn `osiris.exe`: the app it launched would
    ...
    fn install_inner(plan: &InstallPlan, completion: Completion) -> Result<(), String> {
        log_line(&plan.log, "waiting for the osiris GUI to exit");
        ...
        log_line(
            &plan.log,
            "stopping the osiris daemon and clearing installed-file locks",
        );
        // From here until the relaunch, a `osiris` CLI call or a manual launch
        ...
        osiris_core::daemon::update_guard::hold();
        if let Err(error) = osiris_core::daemon::spawn::stop_for_update(&plan.install_dir) {
            return recover_from_failed_update(plan, error, completion);
        }

        log_line(&plan.log, "running the osiris Windows installer");
        ...
        if completion == Completion::ReportToWatcher {
            log_line(
                &plan.log,
                "the Windows update completed; the watcher relaunches osiris",
            );
            ...
            osiris_core::daemon::update_guard::clear();
            queue_cleanup(&plan.install_dir, &plan.stage);
            return Ok(());
        }
        log_line(&plan.log, "the Windows update completed; relaunching osiris");
    ...
    fn elevated_stage_inner(plan: &ElevatedStagePlan) -> Result<(), String> {
        ...
        pin_helper_to_installed(&plan.stage.join("osiris-updater.exe"), &install_dir)?;
    ...
    /// at `{app}\osiris-updater.exe`, so this process's own image names the
    ...
    fn run_install_stage(
        ...
        let staged_updater = staging.join("osiris-updater.exe");
        ...
        fs::copy(plan.stage.join("osiris-updater.exe"), &staged_updater)
    ...
    /// `%ProgramData%\osiris\update-<pid>`, created with a DACL that lets no
    ...
    fn create_protected_staging() -> Result<PathBuf, String> {
        ...
        let root = program_data.join("osiris");
    ...
    /// Claims `%ProgramData%\osiris` itself with the admin-only DACL, taking
    ...
    fn pin_helper_to_installed(staged: &Path, install_dir: &Path) -> Result<(), String> {
        let installed = install_dir.join("osiris-updater.exe");
    ...
    fn file_digest(path: &Path) -> Result<String, String> {
        ...
        Ok(osiris_core::daemon::install::checksums::hex(
            &osiris_core::daemon::install::checksums::sha256(&bytes),
        ))
    }
    ...
    fn read_outcome_lossy(plan: &WatcherPlan) -> Option<WatchOutcome> {
        match osiris_core::daemon::install::outcome::read_outcome(&plan.result_file) {
    ...
    fn finish_watch(plan: &WatcherPlan, gui: &GuiProcess, end: WatchOutcome) -> Result<(), String> {
        ...
        if gui.alive() && !gui.wait_for_exit(GUI_EXIT_GRACE) {
            log_line(
                &plan.log,
                "the osiris that raised the prompt is still running; leaving the \
                 relaunch to it",
            );
            return Ok(());
        }
        if !recorded {
            ...
            if let Err(error) =
                osiris_core::daemon::install::outcome::write_outcome(&plan.result_file, &outcome)
            {
                log_line(
                    &plan.log,
                    &format!(
        ...
        if let Err(error) = health {
            ...
            let _ = osiris_core::daemon::install::outcome::write_outcome(
                &plan.result_file,
                &UpdateOutcome {
    ...
    fn install_portable_inner(plan: &PortableInstallPlan) -> Result<(), String> {
        ...
        log_line(
            &plan.log,
            "stopping the osiris daemon before replacing portable files",
        );
        ...
        log_line(&plan.log, "replacing the osiris Windows portable files");
        let report = |result: &Result<(), String>| {
            report_outcome(
                plan.result_file.as_deref(),
                &plan.log,
    ...
    fn verify_portable_payload(payload: &Path, expected_version: &str) -> Result<(), String> {
        ...
        if marker != PORTABLE_MARKER_CONTENT {
            return Err("the portable ZIP has an invalid .osiris-portable marker".to_string());
        }
        verify_binary_version(
            &payload.join("osiris.exe"),
            expected_version,
            "staged portable osiris.exe",
        )?;
        verify_binary_version(
            &payload.join("osiris-updater.exe"),
            expected_version,
            "staged portable osiris-updater.exe",
        )
    }
    ...
    fn verify_installed_payload(install_dir: &Path, expected_version: &str) -> Result<(), String> {
        ...
        for (name, label) in [
            ("osiris.exe", "installed osiris.exe"),
            ("osiris-updater.exe", "installed osiris-updater.exe"),
        ] {
    ...
    fn stop_daemon_from_payload(payload: &Path, install_dir: &Path) -> Result<(), String> {
        let executable = payload.join("osiris.exe");
        let status = Command::new(&executable)
            ...
            .map_err(|error| {
                format!(
                    "stopping the osiris daemon with {}: {error}",
                    executable.display()
                )
            })?;
        if status.success() {
            ...
            Err(format!("stopping the osiris daemon exited with {status}"))
        }
    }
    ...
    fn with_relaunch_failure(mut message: String, relaunch: Result<(), String>) -> String {
        if let Err(error) = relaunch {
            message.push_str(&format!("; relaunching the previous osiris failed: {error}"));
        }
        message
    }
    ...
    fn launch_app(install_dir: &Path) -> Result<(), String> {
        ...
        osiris_core::daemon::update_guard::clear();
        let executable = install_dir.join("osiris.exe");
        let mut child = Command::new(&executable)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
    ...
    fn queue_cleanup(install_dir: &Path, stage: &Path) {
        ...
        let cleaner = install_dir.join("osiris-updater.exe");
    ...
    mod tests {
        ...
        fn installed_payload_verification_requires_matching_app_and_updater() {
            let root = tempfile::tempdir().unwrap();
            let executable = std::env::current_exe().unwrap();
            fs::copy(&executable, root.path().join("osiris.exe")).unwrap();

            let error =
                verify_installed_payload(root.path(), env!("CARGO_PKG_VERSION")).unwrap_err();
            assert!(error.contains("osiris-updater.exe"), "{error}");

            fs::copy(&executable, root.path().join("osiris-updater.exe")).unwrap();
            verify_installed_payload(root.path(), env!("CARGO_PKG_VERSION")).unwrap();
        }

        #[test]
        fn silent_installer_arguments_keep_the_log_path_native() {
            let log = Path::new(r"C:\Users\测试 User\osiris update.log");
            ...
            let expected: OsString = OsString::from_wide(
                &OsStr::new(r"/LOG=C:\Users\测试 User\osiris update.log")
                    .encode_wide()
                    .collect::<Vec<_>>(),
            );
        ...
        fn the_installed_root_is_the_running_image_not_an_argument() {
            ...
            assert!(same_directory(
                Path::new(r"C:\Program Files\osiris"),
                Path::new(r"c:\program files\OSIRIS")
            ));
            assert!(!same_directory(
                Path::new(r"C:\Program Files\osiris"),
                Path::new(r"C:\Users\mallory\osiris")
            ));
        }
        ...
        fn report_outcome_records_the_terminal_result_for_the_next_gui() {
            ...
            let outcome = osiris_core::daemon::install::outcome::read_outcome(&outcome_path).unwrap();
            assert_eq!(
                outcome,
                Some(osiris_core::daemon::install::outcome::UpdateOutcome {
        ...
        fn archive_verification_rejects_tampered_installer_bytes() {
            let root = tempfile::tempdir().unwrap();
            let installer = root.path().join("osiris-1.0.0-windows-x86_64-setup.exe");
            let manifest = root.path().join("checksums.txt");
        ...
        fn update_verification_accepts_an_unsigned_matching_windows_binary() {
            ...
            let asset_name = format!(
                "osiris-{}-windows-x86_64-setup.exe",
                env!("CARGO_PKG_VERSION")
            );
        ...
        fn portable_archive_verification_extracts_a_complete_release_payload() {
            let root = tempfile::tempdir().unwrap();
            let asset_name = format!("osiris-{}-windows-x86_64.zip", env!("CARGO_PKG_VERSION"));
            ...
            write_test_zip(
                &archive,
                &[
                    ("osiris.exe", executable.clone()),
                    ("oexe.exe", executable.clone()),
                    ("osiris-updater.exe", executable),
        ...
        fn portable_replacement_preserves_unmanaged_user_files() {
            ...
            fs::write(install.path().join("osiris.exe"), b"old app").unwrap();
            ...
            fs::write(payload.path().join("osiris.exe"), b"new app").unwrap();
            ...
            replace_portable_and_relaunch(
                ...
                |directory| {
                    assert_eq!(
                        fs::read(directory.join("osiris.exe")).unwrap(),
                        b"new app"
                    );
                    Ok(())
                },
            ...
            assert_eq!(
                fs::read(install.path().join("osiris.exe")).unwrap(),
                b"new app"
            );
        ...
        fn a_backup_carries_the_incomplete_marker_exactly_while_files_move() {
                    ...
                    let backups: Vec<_> = fs::read_dir(&install_dir)
                        ...
                        .filter(|path| {
                            path.file_name()
                                ...
                                .is_some_and(|name| name.starts_with(".osiris-update-backup-"))
                        })
                        .collect();
            ...
            assert_eq!(
                leftovers,
                vec![std::ffi::OsString::from("osiris.exe")],
        ...
        fn portable_replacement_rolls_back_when_the_new_app_does_not_start() {
            ...
            let payload = tempfile::tempdir().unwrap();
            fs::write(install.path().join("osiris.exe"), b"old app").unwrap();
            fs::write(install.path().join("osiris.exe"), b"old cli").unwrap();
            fs::write(install.path().join("notes.txt"), b"user file").unwrap();
            fs::write(payload.path().join("osiris.exe"), b"new app").unwrap();
            fs::write(payload.path().join("osiris.exe"), b"new cli").unwrap();
...
fn main() {
    eprintln!("osiris-updater is only available on macOS, Windows and Linux");
    std::process::exit(1);
}
